...
| REQ-030 | REQ | 🟡 部分收口 | P2 RAG 自动质量评估新口径（semantic embedding + LLM-as-judge） | 分支 `feat/req-030-new-quality-metric`；脚本支持四口径（substring/semantic/semantic_embedding/llm_judge）+ ScenarioRun 字段 + 报告新增 REQ-030 章节 + dry-run 通过 + 真 LLM 报告生成。**semantic_emb 全 0**：硅流 embedding API 在 10×4×10 batch 下持续挂起，httpx 30s timeout 不足。**LLM-judge AC-5 1/10 达标**。已登记 REQ-031 接力 | [REQ-030](../../01-product-planning/05-requirements/REQ-030-p2-rag-new-quality-metric.md) / [Report](../../02-delivery-plans/01-specs/2026-06-20-req-030-new-quality-metric-report.md) / [REQ-031](../../01-product-planning/04-backlog.md) |
