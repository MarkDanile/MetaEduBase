...
- 支持新建数据库（仅 admin / data_admin），填写名称 / code / 描述 / 图标 / 主题色 / entity_types 白名单。
- 数据集上传时必选所属数据库；3 种数据源类型（imported_dataset / direct_db / mcp）统一接入。
...
| 数据集接入 | 3 种数据源类型统一接入 | imported_dataset（上传 CSV/Excel，✅ 首期）/ direct_db（连接外部 PG，V1）/ mcp（MCP 服务映射，V1） |
...
| ID | 内容 |
|----|------|
| AC-1 | `metaedu.data_catalogs` 表创建，支持 CRUD + entity_types 白名单 |
| AC-2 | `datasets` / `semantic_models` / `knowledge_nodes` / `query_audit_log` 加 `catalog_id` FK，迁移后现有数据归入默认库 |
| AC-3 | 仅 admin / data_admin 可创建数据库；普通用户只能浏览使用（权限测试覆盖） |
| AC-4 | 数据集上传时必选 catalog_id；entity_type 必须在该库的白名单内 |
| AC-5 | 语义层按 (catalog_id, entity_type) 路由；同 entity_type 在不同库下可独立配置 |
| AC-6 | QueryPlanner / QueryService.ask 按 catalog_id 路由到正确的 semantic_model |
| AC-7 | 前端数据库列表卡片 + 详情页 tab（数据集 / 语义层 / KG / 问数） |
| AC-8 | 知识图谱按数据库聚合展示（V1 标签聚合，跨 dataset 节点可见） |
| AC-9 | 现有教育数据集自动归入"中高职教育数据库"，用户无感迁移 |
| AC-10 | 3 种数据源类型（imported_dataset / direct_db / mcp）统一接入，至少 imported_dataset 完整可用 |
