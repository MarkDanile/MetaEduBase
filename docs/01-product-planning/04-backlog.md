...
| REQ-006 | REQ | 🟡 Doing | P1 | P1 | P1 知识资产处理链路最终演示验收 | 已建 [Spec](../02-delivery-plans/01-specs/2026-W23-req-006-p1-final-demo.md) / [Plan](../02-delivery-plans/02-plans/2026-W23-req-006-p1-final-demo-plan.md) 骨架（[PR #115](https://github.com/MarkDanile/MetaEduBase/pull/115) 已合并）；Stage 1.0 收口：[PR #117](https://github.com/MarkDanile/MetaEduBase/pull/117) 合并 `tests/e2e/test_p1_demo.py` 3 步 e2e 脚本（上传 / 解析 / `processing` 中间态 / 幂等性）跑通 + UI 演示手册骨架；Stage 1.0 探查发现并入账 TD-036（`metaedu_test` schema drift）+ TD-037（e2e 沙箱 Celery broker 缺）；Stage 1.5 待实施（接入 `extract_template` / KG / RAG / sources AC-3 ~ AC-6）；Stage 2 待文档回填翻 `🟢 Done`。 |  |
...
| DOC-051 | DOC | ⚫ Candidate | P2 | P1 | 一次性收口历史 plan 残留 TBD / `TD-???` / `未回填` 占位 | 5 处 TBD（`2026-W23-req-003-rag-quality-gate-plan.md:643` 声明"无 TBD"但文件内另有 TBD 上下文；`2026-W23-req-004-template-match-explainability-plan.md:571-573` 3 处 `TBD`；`2026-W23-req-008-req-004-quality-follow-up-plan.md:436, 442, 484-486` 4 处 `TBD` / `PR #TBD`）+ 3 处 `TD-???`（`2026-W23-req-003-rag-quality-gate.md:87` / `2026-W23-req-004-template-match-explainability.md:141` / `2026-W23-req-008-req-004-quality-follow-up.md:103`）→ TD-030 稳定编号 + 2 处 `未回填`（`2026-W23-req-003-rag-quality-gate.md:41` / `2026-W23-req-008-req-004-quality-follow-up.md:46`）改为具体失败条件描述 + W23 迭代卡状态与内表同步 + 迭代卡 Review 段 PG 连失败历史声明更新为"2026-06-09 沙箱已恢复（34 passed）"；解锁 `quality-gates.md#脚本门禁候选清单` 中"完成态占位扫描"从候选转已实现。 | [Plan](../02-delivery-plans/02-plans/2026-W23-req-003-rag-quality-gate-plan.md) / [Plan](../02-delivery-plans/02-plans/2026-W23-req-004-template-match-explainability-plan.md) / [Plan](../02-delivery-plans/02-plans/2026-W23-req-008-req-004-quality-follow-up-plan.md) / [Spec](../02-delivery-plans/01-specs/2026-W23-req-003-rag-quality-gate.md) / [Spec](../02-delivery-plans/01-specs/2026-W23-req-004-template-match-explainability.md) / [Spec](../02-delivery-plans/01-specs/2026-W23-req-008-req-004-quality-follow-up.md) / [W23 迭代卡](03-iterations/2026-W23-p1-final-gap-closure.md) |
| DOC-052 | DOC | ⚫ Candidate | P2 | P3 | 清理 `scripts/engineering/checks/_common.py` 中 `KNOWN_ISSUES` 残留的 TD-023 历史白名单 | TD-023 任务已 🟢 完成（PR 未配置 CI / 本地门禁已通过）；其 `KNOWN_ISSUES` 4 条白名单（`td-020-*` plan 2 条 + `td-020-*` spec 1 条 + `td-023` 1 条）已无拦截价值，长期保留会掩盖新漂移；逐条评估能否删除；删除后跑 `scripts/check-engineering-docs` 退出码 0。 | [`_common.py` KNOWN_ISSUES](../../scripts/engineering/checks/_common.py) / [TD-023 复核背景](../03-engineering-governance/04-retrospectives/2026-06-08-req-003-delivery-flow.md) |
...
| DOC-054 | DOC | ⚫ Candidate | P2 | P3 | 收口 review-score-log PR 字段与倒排顺序一致性 | DOC-049 评审行仍写 `本 PR`，应回填 PR #113；DOC-050 要求评分总账最新评审置顶，但当前同日行顺序未完全倒排；Metrics Snapshot 计数也需与现有行数同步。 | [Review Score Log](../03-engineering-governance/04-retrospectives/review-score-log.md) / [Review Scorecard](../03-engineering-governance/01-rules/review-scorecard.md) |
