...
| DOC-049 | DOC | ⚫ Candidate | P2 | P1 | 收口 REQ-005 完成态占位与验证声明漂移 | 修正 REQ-005 spec/plan 中 AC-8 浅拷贝口径、`未回填`、`TD-???`、TBD 提交占位和 document 测试范围描述；评估把完成态占位扫描加入 `scripts/check-engineering-docs`。 | [Spec](../02-delivery-plans/01-specs/2026-W23-req-005-structured-extraction-regression.md) / [Plan](../02-delivery-plans/02-plans/2026-W23-req-005-structured-extraction-regression-plan.md) |
