...
| REQ-006 | REQ | 🟣 Shaping | P1 | P1 | P1 知识资产处理链路最终演示验收 | 已建 [Spec](../02-delivery-plans/01-specs/2026-W23-req-006-p1-final-demo.md) / [Plan](../02-delivery-plans/02-plans/2026-W23-req-006-p1-final-demo-plan.md) 骨架（[PR #115](https://github.com/MarkDanile/MetaEduBase/pull/115) 已合并）；6 步闭环（上传 / 解析 / 模板抽取 / 知识图谱 / RAG 问答 / 来源展示）拆 AC-1 ~ AC-10；2026-06-09 沙箱探测 `metaedu_test` 已恢复（`PostgreSQL 16.13` + W23 复核 4 文件 34 passed in 9.86s），原 DB 阻塞已解除；Stage 1 待实施（端到端脚本 + UI 手册）；Stage 2 待文档回填。 |  |
