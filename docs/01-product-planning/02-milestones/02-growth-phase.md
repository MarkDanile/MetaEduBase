...
| TD-068 | 🔵 Ready | REQ-024 dry-run 暴露 query embedding 为空，需先确认向量通道是否真实参与召回，再继续 REQ-025 效果验收。 | [Technical Debt](../../03-engineering-governance/technical-debt.md#td-068) |
| REQ-025 | 🔵 Ready | graph_edge 进入 prompt 与真实 LLM 效果验收收口：至少 2 个真实弱召回样例需证明 edge evidence 进入 packed context 并改善最终回答。 | [Requirement](../05-requirements/REQ-025-p2-graph-edge-prompt-impact-and-real-llm-validation.md) / [Backlog](../04-backlog.md) |
