...
| TD-068 | 🟡 部分收口（docs-only 诊断完成，TD-069 接力） | AI Chat vector embedding 为空底层修复 (分支 `docs/td-068-slice2-diagnosis-td069-handoff`)。Slice 1 PR #355 squash merge `fdffd60`：trace 已透出 `embedding_fallback` metadata。Slice 2 诊断完成：provider 层 (`embedding_service.py` 多 provider fallback) 真实生成 4096 维向量，vector_fallback_count=0；但 dev DB `document_chunks.embedding` / `knowledge_nodes.embedding` 两列都是 `text` 类型不是 `vector`，pgvector `<=>` cosine 操作符不可用，schema 层限制超出本任务代码修复范围。代码修复暂不 merge，等 TD-069 schema migration 完成后再合并 | [Technical Debt](../../03-engineering-governance/technical-debt.md#td-068) / [Report](../../02-delivery-plans/01-specs/2026-06-18-req-024-p2-real-validation-report.md) / [PR #355](https://github.com/MarkDanile/MetaEduBase/pull/355) / [TD-069](../../03-engineering-governance/technical-debt.md#td-069) |
