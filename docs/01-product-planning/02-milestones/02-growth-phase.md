...
| REQ-016 | 🟢 Done | P2-NER：PR #328/#329/#330 已合并；HybridQueryUnderstandingService、expanded_query、AIChatService diagnostics 和 retriever 查询扩展已接入，真 PG 验收报告待环境执行后填充。 | [Requirement](../05-requirements/REQ-016-p2-llm-hybrid-ner-query-understanding.md) / [Backlog](../04-backlog.md) |
| REQ-017 | 🟡 Doing | P2-RRF：Slice 1-3 PR #325 已合并（配置入口 + fusion diagnostics + 通道降级）。weighted RRF 已接入；Slice 4 真实 PG 样例 RRF 排序分析待 REQ-015 PG 环境 backfill。 | [Requirement](../05-requirements/REQ-017-p2-rrf-weighted-fusion.md) / [Backlog](../04-backlog.md) |
