...
| BUG-010 | 🟡 Doing | AI Chat 自然问法未稳定命中函数参数正文 chunk | P2-NER 前置确定性切片：先用 query normalizer + 术语拆分修复 A/B 等价问法召回不一致；LLM 混合 NER / Query Understanding 后续仍保留。 | [Bug](../05-requirements/BUG-010-ai-chat-query-normalizer-function-parameter-question.md) / [Backlog](../04-backlog.md) |
