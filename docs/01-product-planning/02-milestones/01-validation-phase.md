...
| REQ-010 | 🔵 Ready | P1 真实 RAG 证据治理与 AI Chat 溯源体验：把多源召回统一到可追溯证据单元（EvidenceItem），让 LLM 上下文以 `document_chunks.content` / 结构化字段 / 可回源知识关系为核心；MCP / KG 视图同步迁到新 retriever 接口；KG 抽取按 chunk 切片写入 `source_chunk_id`；前端 [1]/[2] 引用跳文件详情 + chunk 锚点。 | [Requirement](../05-requirements/REQ-010-p1-rag-evidence-governance.md) / [Spec](../../02-delivery-plans/01-specs/2026-06-10-req-010-rag-evidence-governance.md) / [Plan](../../02-delivery-plans/02-plans/2026-06-10-req-010-rag-evidence-governance-plan.md) / `docs/01-product-planning/04-backlog.md` |
