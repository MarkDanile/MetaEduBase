...
| REQ-010 | 🟢 Done | P1 真实 RAG 证据治理与 AI Chat 溯源体验（8 个 Slice 收口）：多源召回统一到 EvidenceItem，LLM 上下文以 `document_chunks.content` / 结构化字段 / 可回源知识关系为核心；MCP / KG 视图同步迁到新 retriever 接口；KG 抽取按 chunk 切片写入 `source_chunk_id`；前端 [1]/[2] 引用跳文件详情 + chunk 锚点。P1 RAG 基线：node_source_chunk 0% / chunk_embedding 100% / chunk_tsvector 93.55% / file_metadata 0%。follow-up：TD-045 (`_call_llm` 漏 await) + FU-A~E。 | [Requirement](../05-requirements/REQ-010-p1-rag-evidence-governance.md) / [Spec](../../02-delivery-plans/01-specs/2026-06-10-req-010-rag-evidence-governance.md) / [Plan](../../02-delivery-plans/02-plans/2026-06-10-req-010-rag-evidence-governance-plan.md) / `docs/01-product-planning/04-backlog.md` / [TD-044](../../03-engineering-governance/technical-debt.md#td-044) |
