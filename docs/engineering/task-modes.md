...
4. 如果使用插件生成 spec/plan，插件输出已登记到任务卡片的 `插件输出` 字段。
...
2. 验证结论是否来自真实命令输出；退出码非 0 的命令不得写“通过”。
3. 如果本次改动影响多个等价入口、对象类型、状态流或端点，是否已用覆盖矩阵确认测试或验收范围。
4. 如果仍有历史失败，是否绑定到已有或新增的 `TD-xxx`。
5. PR 描述、最终回复和任务卡片里的声明是否一致。
...
如果用户明确指定插件，例如 superpower 或 compound-engineering-plugin，执行者仍必须以 `docs/engineering/current-work.md` 为入口，并把插件输出登记到任务卡片。
