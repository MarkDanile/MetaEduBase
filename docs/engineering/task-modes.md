...
每次进入提交、PR 或声明任务完成前，执行者必须完成 `docs/engineering/rules/quality-gates.md#完成门禁`。如果任务触发专项风险，再读取对应专项门禁：

- 覆盖多个等价入口、对象类型、状态流或端点：使用覆盖矩阵。
- 声明“零业务逻辑变更”“仅格式化”“仅 lint 修复”：执行行为变化声明检查。
- 涉及 composable、Vue Query、请求 service、轮询或 mutation 刷新：执行前端请求生命周期等价矩阵。
