...
本文件用于把常见开发工作接入同一套流程。它关注的是“这类任务该怎么开工、该读什么、怎样算完成”，不是重复展开完整工作流。跨 AI IDE / 插件交接流程仍以 `docs/engineering/workflow.md` 为主。
...
4. 如果插件生成了 spec / plan，规范副本已迁移或镜像到 `docs/specs/*` / `docs/plans/*`。
5. 开发结束后会回写状态、验证结果和下一步。

## 通用收尾回查

每次进入提交、PR 或声明完成前，必须执行 `docs/engineering/rules/quality-gates.md#完成门禁`。如果任务触发专项风险，再补读对应专项门禁：

- 覆盖多个等价入口、对象类型、状态流或端点：使用覆盖矩阵
- 声明“零业务逻辑变更”“仅 lint 修复”等：执行行为变化声明检查
- 涉及 composable、Vue Query、轮询或 mutation 刷新：执行前端请求生命周期等价矩阵
- 涉及 API / DTO / shared schema：读取 `docs/engineering/rules/contracts.md`
...
| 跨 3 个以上文件、复杂 UI、新 API、新数据模型、权限 / 多租户 / 数据一致性变化 | `superpower` 优先 | 先产出或更新 `docs/specs/*` 和 `docs/plans/*`，再进入开发 |
...
| `按流程开发这个新需求: ...` | 新需求开发 | 判断是否需要 spec/plan；需要时进入 `docs/specs/*` 和 `docs/plans/*` |
...
适用：处理 `docs/engineering/technical-debt.md` 中的 `TD-xxx`。
...
- 对应 `docs/plans/*`
- 必要时读 `ARCHITECTURE.md`
- 涉及 API / DTO / shared schema 时读 `docs/engineering/rules/contracts.md`
...
- 必要时读 `docs/engineering/rules/git-workflow.md`
...
- 必要时读 `docs/engineering/rules/contracts.md`
- `docs/engineering/rules/quality-gates.md`
- 必要时读 `ARCHITECTURE.md`
