...
- Spec: `docs/specs/2026-06-05-doc-012-engineering-doc-gates-and-workbench-slimming.md`
- 任务卡片: `docs/engineering/current-work.md` 的 DOC-012 候选行
- 类型: 文档 / 工程规范 / 工具链
- 当前执行模式: plan-do
...
- [x] 解析 `docs/engineering/current-work.md` 的三个区域。
...
- [x] 扫描 `docs/engineering/*.md`、`docs/specs/*.md`、`docs/plans/*.md` 的 Markdown 链接。
- [x] 对本仓库内相对路径做存在性校验。
- [x] 忽略外部 URL、锚点-only 链接和明确历史兼容链接。

### 5. 检查 work-log 追加式索引风险

- [x] 在脚本中检查当前 diff 中 `docs/engineering/work-log.md` 的删除行。
- [x] 如果删除或替换任务索引行，要求提交说明或任务文档中有明确原因。
- [x] 失败信息提示“默认新增索引，不要覆盖旧行”。

### 6. 检查验证声明证据格式

- [x] 扫描 `docs/engineering/*.md`、`docs/specs/*.md`、`docs/plans/*.md` 中的强通过声明。
...
- [x] 新增或更新 `docs/engineering/rules/workbench.md`，承载状态流、保留策略和任务卡片模板。
...
- [x] `rg -n "check-engineering-docs|工程文档门禁" docs/engineering AGENTS.md CLAUDE.md` 命中新入口。
