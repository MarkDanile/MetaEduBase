...
**布局**：左右分栏（`xl:grid-cols-[1fr_340px]`）

**左列 — 模板配置表单**：
- 模板名称（`liquid-input`）
- 关联文档类型（多选下拉，`DOC_TYPE_OPTIONS` 基础上支持自定义）
- 字段列表（拖拽排序 `vuedraggable`）：
  - 每行：展开/折叠图标 + 字段类型标签 + 字段名/标签 + 操作按钮
  - 展开时：显示完整字段编辑表单（key、label、type、description、children/columns）
  - 类型切换时有条件渲染（table → 显示 columns 编辑器，object/array → 显示 children 编辑器）
...
- 按钮：「生成字段结构」
- 加载状态：`LoadingSpinner` + 进度提示
- 结果预览：返回的 fields 以卡片列表展示，点击「应用」填充到左列
