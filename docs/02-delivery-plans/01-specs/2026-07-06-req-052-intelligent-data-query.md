...
国资国企过去多年投入了大量信息化系统，沉淀了招商、资管、合同、财务、工单、OA、报表和台账数据。AI 应用的关键价值不是只接外部工具，也不是只做文档 RAG，而是把**内外部结构化数据**真正整合激活，让业务人员能用自然语言查询、解释和复用经营数据。

企业 360 背调（REQ-046）是首个高价值场景：需要同时整合**外部数据**（企查查等第三方 MCP 提供的企业工商/司法/风险事实）和**内部数据**（本园区合同/账单/工单/租约/招商等信息化沉淀），才能回答"这家公司在外部是什么样 + 和本园区发生过什么 + 履约如何 + 欠费如何 + 工单投诉如何"。这要求智能问数能力不只接内部数据，而是统一整合内外数据源。

当前后端有 `structured_data` context（`datasets` + `dataset_rows` 表 + 前端"数据库"视图），已支持用户上传结构化数据集（内部数据源之一）。但缺少：
- 统一数据源接入层（直连数据库 / 导入数据集 / 第三方 MCP 三种数据源统一适配）
...
建设可复用的**内外数据整合激活原子能力**——不只接入内部数据，而是统一整合 3 种数据源（直连数据库 / 导入数据集 / 第三方 MCP），通过统一语义层激活：
...
**首期只实现 `ImportedDatasetAdapter`**（JSONB 查询路径），接口先行；`DirectDBAdapter` 和 `MCPAdapter` 留 V1，但接口已定义，后续扩展只需加 adapter 实现。**最终目标：3 种数据源统一通过语义层激活，业务人员不需要知道数据来自哪里。**

核心目标：

- **整合内外数据**：统一接入直连数据库（内部系统）、导入数据集（本系统上传）、第三方 MCP（企查查/自家 MCP）3 种数据源，不只接入内部数据。
- 将自然语言问题映射到受治理的数据源、指标、维度、过滤条件和查询计划。
- 通过语义层屏蔽物理数据源差异（导入数据集 / 直连数据库 / MCP），避免大模型直接猜表、猜字段、猜口径。
- 在查询前做权限、敏感字段、查询安全和成本控制。
- 在查询后给出结果解释、数据来源、指标口径和异常提示。
- 支撑企业 360 背调（内外数据整合）、续约风险、报送材料、经营分析等上层应用。
...
## 11. 首期能力边界（诚实登记，避免预期错位）

**核心原则**：首期 JSONB 查询路径**只能支持 80% 简单场景**，剩下 20% 复杂场景（跨行关联/复杂聚合/跨 dataset JOIN）必须依赖阶段 2+ 解决。**绝不允许让用户预期"问数能查任何问题"，结果上线后发现只能查简单问题。**

### 11.1 JSONB 查询能力边界

| 查询类型 | 复杂度 | 首期支持 | 实现路径 | 阶段 2+ 升级 |
|----------|--------|----------|----------|--------------|
| 列等值（`company_name = "X"`） | 简单 | ✅ | JSONB `data['col'].astext = 'X'` | — |
| 范围（`amount > 1000 AND amount < 5000`） | 简单 | ✅ | JSONB + GIN 索引 | — |
| 模糊匹配（`company_name LIKE '%科技%'`） | 简单 | ✅ | JSONB `astext LIKE` | — |
| 时间窗口（`date >= '2023-01-01' AND date < '2024-01-01'`） | 简单 | ✅ | JSONB `astext >=` | — |
| 多条件 AND/OR 组合 | 中等 | ✅ | JSONB 组合查询 | — |
| 单维度聚合（`COUNT/SUM/AVG` + `GROUP BY`） | 中等 | ✅ | PG 原生聚合 | — |
| 多维度聚合（`GROUP BY` 多列 + `HAVING`） | 中等 | ✅ | PG 原生聚合 | — |
| TopN 排序（`ORDER BY ... LIMIT N`） | 简单 | ✅ | SQLAlchemy `order_by().limit()` | — |
| **跨行关联**（"A 企业的所有合同 + 所有账单"） | 复杂 | ⚠️ 部分 | 首期仅同 dataset 内；跨 dataset 留阶段 2 | 阶段 2 跨 dataset JOIN |
| **时间窗口 + 聚合 + 排序**（"最近 3 个月按企业分组求 sum"） | 复杂 | ⚠️ 性能风险 | JSONB 聚合大数据集慢；需 GIN 索引 + 预聚合 | 阶段 2 物化视图 |
| **跨 dataset JOIN**（合同 + 账单 + 工单） | 复杂 | ❌ 不支持 | JSONB 路径无法跨 dataset | 阶段 2 跨 dataset JOIN |
| **子查询 / 窗口函数** | 复杂 | ❌ 不支持 | JSONB 不支持子查询 | 阶段 2 |
| **跨表复杂计算**（如 NOI = 收入 - 成本） | 复杂 | ❌ 不支持 | 需业务定义 metric + 自定义函数 | 阶段 2/3 |
| **全文模糊搜索**（"找涉及'知识产权'的合同"） | 复杂 | ❌ 不支持 | JSONB 不擅长全文 | 阶段 3 引入 PG `tsvector` |

**首期 JSONB 路径支持率估计**（基于典型业务问题分布）：
- 简单查询（单条件/单聚合）：~70%
- 中等查询（多条件 + 单聚合）：~15%
- 复杂查询（跨行/跨 dataset/子查询）：~15%
- **首期合计覆盖：~80%**，**剩余 20% 必须升级到阶段 2+ 才能支持**
...
|----------|--------------|------|
| < 1,000 行 | ✅ 毫秒级 | 默认 |
| 1,000 - 10,000 行 | ✅ 秒级 | 加 GIN 索引 (`CREATE INDEX ON metaedu.dataset_rows USING GIN (data)`) |
| 10,000 - 100,000 行 | ⚠️ 几秒到几十秒 | 限制 limit ≤ 100 + GIN 索引 + 物化视图 |
| > 100,000 行 | ❌ 慢（JSONB 全表扫） | 必须物化视图 / 聚合表（阶段 2 改造） |

**首期数据量假设**：用户在 spec §3 提到的"样例库"——预计 100-10,000 行/数据集，**首期可工作**。生产环境 100,000+ 行数据集**需阶段 2 物化视图**才能支持。
...
|------|-----------|------|
| USCC 完全匹配 | 0% | 唯一标识符 |
| 企业全称匹配 | 2-5% | 拼写错误/简称/曾用名 |
| 同义词匹配（column_mapping.synonym） | 5-10% | 同义词不完整 |
| 跨业务系统匹配（资管 vs 招商 vs 财务） | 10-20% | 各系统企业名称格式不一致 |

**缓解**：
- ✅ USCC 优先（无错误）
- ✅ 同义词配置 + 模糊匹配（ilike/contains）降错率
- ✅ 人工确认兜底（企查查多候选场景）
- ⚠️ 错误率不能降至 0%——必须告知用户"这是近似匹配，不是精确匹配"

### 11.4 AI Chat tool calling 调优期

**问题**：AI Chat 加 tool calling 是**框架级扩展**，不是 trivial 集成。

**预期挑战**：
- 意图判断（什么时候用工具 vs 直接答）需要 2-3 轮 prompt 调优
- 工具调用失败 fallback 策略（超时/超时/拒答）需设计
- 多轮对话 + 工具结果融合需多轮测试
- 工具结果与 RAG 文档矛盾时仲裁

**预期时间**：2-3 周调优期（不是 spec 完成后立即完美）。
- 首期：基础 tool calling 框架 + 1 个工具（query_internal_data）
- 1 个月后：根据用户反馈调优 prompt + 边界
- 3 个月后：稳定 + 可考虑加更多工具
...
| 期望 | 现实（首期） |
|------|------------|
| "问数能查任何问题" | ❌ 80% 简单场景 ✅；20% 复杂场景 ❌（需阶段 2+） |
| "匹配企业 100% 准确" | ❌ 5-10% 错误率（同义词/简称/USCC 不一致） |
| "AI Chat 问数立即准确" | ⚠️ 1-2 个月调优期（需 few-shot + 多轮测试） |
| "大数据集快" | ⚠️ < 10K 行秒级；> 100K 行需物化视图 |
| "跨 dataset 查询" | ❌ 首期不支持（阶段 2） |

**对用户的承诺（必须提前说）**：
- ✅ 首期可演示：简单查数（"这企业欠费多少"）+ AI Chat 问数 + 前端面板
- ⚠️ 复杂查询需阶段 2（数据建模师配合 + 物化视图 + 跨 dataset JOIN）
- ⚠️ 主体匹配偶有错误（人机协同：机器找候选 + 人工确认）
- ⚠️ AI Chat 调优期 1-2 个月（不是 spec 完成后立即完美）

### 11.6 后续能力扩展（V1 优先）

按"统一语义层的检查和修正"能力（用户提的需求）：

| 扩展能力 | 价值 | V1 优先 |
|----------|------|---------|
| 语义层初始化（半自动生成草稿） | 高——减少手动配置 | ✅ V1 |
| 增量检测（数据源漂移） | 高——防"暗字段" | ✅ V1 |
| 主动元数据（扫描未登记字段） | 中——治理工具 | V1.5 |
| 跨 dataset JOIN | 高——解锁 20% 复杂场景 | ✅ V1 |
| 物化视图（性能优化） | 高——大数 | V1 |
| 本体图谱（阶段 3 Palantir 对标） | 长期价值 | 阶段 3 |
| Agentic 触发行动（阶段 4） | 高——主动预警 | 阶段 4 |

## 12. 安全与合规（首期必须扩展项，区别于 §11 V1 扩展）

**核心定位**：本节登记的是**首期上线时国资安全审查必查项**——不是 V1，是首期（Slice 0-4）就必须具备的。缺这些会被国资安全审批直接打回。

**与 §11 区别**：§11 登记"能力边界"（首期 JSONB 路径不能查的查询类型）；§12 登记"安全审查边界"（必须实现的能力）。

### 12.1 权限与多租户治理（🔴 首期必须）

| 能力 | 说明 | 实现 |
|------|------|------|
| **角色 RBAC** | 5 类角色：普通员工 / 部门经理 / 园区领导 / 数据管理员 / 审计员。每类角色对每个 entity_type 有不同权限（read / masked_read / none） | 复用现有 `users` 表 + 新增 `role_permissions` 表 |
| **跨租户访问控制** | 默认严格隔离（某租户用户只能看本租户数据）。例外需审批流（"申请查看 X 公司 X 字段"） | `tenant_access_grants` 表 + 审批流（V1） |
| **字段级权限** | 同一字段不同人看不同形态：员工看 masked（"138****1234"），部门经理看原文，审计员看变更历史 | `column_mapping.sensitive` + `column_mapping.visibility_rules`（按角色） |
| **操作审计** | 记录 user_id / tenant_id / role / question / query_plan / data_source / result_count / timestamp / ip / business_purpose（用户输入的查询背景） | `query_audit_log` 表 + 不可篡改（append-only） |

**AC-1 扩展**：原 AC-1 必须包含"权限模型 + 跨租户控制 + 字段级权限 + 操作审计"4 项检查。

### 12.2 数据安全与合规（🔴 首期必须）

| 能力 | 说明 | 实现 |
|------|------|------|
| **PII 自动识别** | 自动检测身份证 / 手机号 / 银行卡 / 邮箱 / 地址 / 财务数据（合同金额、银行账户）。即使 column_mapping 没标记 sensitive，PII 识别器也强制脱敏 | PII 检测模块（正则 + LLM 双引擎，交叉验证） |
| **强制脱敏** | 角色敏感 → 部门经理→原文 → 审计员→明文+变更历史。**默认 PII 字段强制脱敏** | SqlGuard 在结果层做 |
| **数据保留与销毁策略** | query_audit_log 保留 ≥ 3 年（国资合规要求）；数据缓存 LRU + TTL | 自动归档 + 清理 |
| **出境控制** | LLM 在内网（公司专网）部署；查询结果不外发；LLM provider 全部走内网代理 | 网络层 + 代码层双重控制 |
| **合规报告生成** | 季度合规报告（谁查过什么、异常查询、可疑访问模式） | 自动生成 + 审计员审 |

**PII 自动识别是关键**——如果 query_plan 查了身份证字段但 column_mapping 没标记 sensitive，**直接是数据安全事件**。PII 识别器作为最后防线（即使 schema 配置错误也不暴露 PII）。

### 12.3 LLM 输出的可靠性（🔴 首期必须）

| 能力 | 说明 | 实现 |
|------|------|------|
| **query_plan 二次校验** | SQL Guard 字段白名单只防"非法字段"；还需防"合法但错字段"（查错企业）。每次 query_plan 生成后，跑"沙箱 query"用已知数据集验证结果合理性 | 校验脚本（首期实现：随机抽 1 条结果与人工标注对比） |
| **LLM 降级策略** | provider 挂了 → 模板化问数（预设 query_plan）+ 人工兜底。永不返回"系统错误" | 异常处理 + 预设模板 |
| **LLM 成本控制** | 每月费用监控 + 单用户每日限流 + 异常使用告警 | rate limit + cost tracking |
| **数据一致性校验** | 跨数据源查询结果的一致性（如同一企业的欠费金额在资管 vs 财务系统是否一致） | V1 |
| **结果审计** | 随机抽样 1% 查询人工审计（看结果是否合理） | 审计员 dashboard（V1） |
...
| # | 审查项 | spec 当前 | 补救 |
|---|--------|---------|------|
| 1 | 多租户数据隔离 | ✅ §3.1 / §5.1 | 已有 |
| 2 | RBAC 角色权限 | ❌ | §12.1 补 RBAC |
| 3 | 字段级敏感数据保护 | ✅ §5.5（schema 标记） | 需补 PII 强制识别（§12.2） |
| 4 | 数据脱敏规则 | ✅ §5.5 | 已有 |
| 5 | 操作审计日志 | ✅ §5.5（审计日志） | 需补 business_purpose 字段 |
| 6 | 查询业务背景记录 | ❌ | §12.1 补 business_purpose |
| 7 | 跨租户访问审批 | ❌ | §12.1 补审批流 |
| 8 | LLM 调用可追溯 | ⚠️（API key + logs） | 需补完整 LLM 调用审计 |
| 9 | PII 自动识别 | ❌ | §12.2 补 |
| 10 | 数据保留与销毁 | ❌ | §12.2 补 |
| 11 | 出境控制 | ⚠️（provider 走 .env） | §12.2 补内网代理 |
| 12 | 合规报告 | ❌ | §12.2 补 |

**当前 spec 覆盖 5/12 项**——7 项缺失。如要通过国资安全审查，需补全 12 项。

### 12.5 国资场景特有考虑

| 考虑 | 说明 |
|------|------|
| **专网部署** | LLM 必须公司内网（不可调外网 API）。本地 LLM 选型（Qwen / DeepSeek / 商用版） |
| **数据出境合规** | 《数据出境安全评估办法》要求关键数据出境需评估。本系统数据为国资经营数据，**默认不出境** |
| **国产化要求** | 操作系统 + 数据库 + LLM 全部国产化（麒麟 OS + 达梦/OceanBase + Qwen/DeepSeek） |
| **审计上报** | 查询日志可向集团审计部门开放；季度合规报告自动生成 |
| **应急预案** | LLM 服务异常时降级到预设模板问数；查询超时告警；数据访问异常自动封号 |

### 12.6 安全与合规与现有 AC 关系

| 现有 AC | 安全与合规扩展 |
|---------|--------------|
| AC-1（数据源登记） | + 权限模型 + 跨租户控制 + 字段级权限 + 操作审计 |
| AC-3（query_plan 记录） | + business_purpose 必填 + LLM 调用完整审计 |
| AC-4（SQL Guard） | + PII 自动识别 + 强制脱敏 |
| AC-5（结果返回） | + 字段级脱敏 + 跨数据源一致性校验 |
| AC-7（回归样例） | + 安全测试样例（敏感字段、PII、越权） |

### 12.7 实施时序（首期 Slice 0-4 必须包含安全与合规）

| Slice | 安全与合规内容 |
|-------|----------------|
| Slice 0 | RBAC schema 设计 + role_permissions 表 + tenant_access_grants 表 + query_audit_log 表 + 字段级 visibility_rules schema |
| Slice 1 | SqlGuard 集成 PII 自动识别 + business_purpose 必填 + LLM 调用审计 + 降级策略 |
| Slice 2 | 前端问数面板集成（business_purpose 输入 + 角色脱敏展示） + 合规报告 dashboard |
| Slice 3 | AI Chat tool calling 集成安全与合规（业务背景传递 + 角色脱敏 + 审计） |
| Slice 4 | REQ-046 集成（跨租户访问 + 字段权限继承 REQ-046 报告） |

### 12.8 不补这些能力的代价

| 不补 | 后果 |
|------|------|
| RBAC + 跨租户控制 | 任何用户能查所有租户数据 → 国资安全审查**直接打回** |
| PII 自动识别 | 即使 schema 配置错误也暴露 PII → 数据安全事件 |
| 操作审计 + business_purpose | 无法追溯"谁查了什么、为什么查" → 不符合国资合规要求 |
| 出境控制 | LLM 调外网 API → 数据出境 → 违反《数据出境安全评估办法》 |
| LLM 降级 | LLM 挂了 → 系统不可用 → 业务停摆 |
| 字段级脱敏 | 员工能看到合同金额 → 内控失效 |

**不补这些能力的 spec 即使技术上完美，**也无法通过国资安全审批上线**。**安全与合规不是 V1 加分项，而是首期上线必查项**。