...
`docs/03-engineering-governance/current-work.md`，技术债进入 `technical-debt.md`，长期
spec / plan 进入 `docs/02-delivery-plans/01-specs/*` 和 `docs/02-delivery-plans/02-plans/*`。
...
- 检查 `docs/03-engineering-governance/current-work.md` 的区域约束：
  - “下一批候选任务”只允许 1 到 3 个未完成候选。
  - 候选区不得出现 `🟢 完成`。
  - “最近完成”最多 5 行。
- 检查已完成计划文件不得残留活动式 `- [ ]`，但允许明确标记为 out of scope
  或已绑定后续任务的未完成项。
- 检查关键 Markdown 链接或任务事实源路径，至少覆盖 `docs/02-delivery-plans/02-plans/*`、
  `docs/02-delivery-plans/01-specs/*`、`docs/03-engineering-governance/*.md` 内的相对链接。
...
- 将 `current-work.md` 的长规则说明迁移或压缩到更合适的规则文档，例如
  `docs/03-engineering-governance/01-rules/workbench.md`。
...
专项细节进入 `docs/03-engineering-governance/01-rules/*`；脚本作为完成门禁的一部分。
...
- `rg -n "check-engineering-docs|工程文档门禁" docs/03-engineering-governance AGENTS.md CLAUDE.md` 能命中新命令入口。
