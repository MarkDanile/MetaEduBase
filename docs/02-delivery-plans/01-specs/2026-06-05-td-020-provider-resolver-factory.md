...
> 领域：Backend / AI / 可维护性
> 类型：技术债
> 事实源：[technical-debt.md#td-020-统一-llm-provider-resolver-与-factory-优先级事实源](../../03-engineering-governance/technical-debt.md)
> 计划：[plans/2026-06-05-td-020-provider-resolver-factory-plan.md](../02-plans/2026-06-05-td-020-provider-resolver-factory-plan.md)
...
参见 `docs/02-delivery-plans/02-plans/2026-06-05-td-020-provider-resolver-factory-plan.md`。
...
- 补一条长期事实源 ADR（`docs/03-engineering-governance/decisions/td-020-resolver-order.md`）记录该决策。

路线 B 仍能消除"互相矛盾"这一条完成标准，但"策略漂移"风险未根除，仍会出现在下一个 AI 接手时。

## 8. 状态同步

- 开工：移动 `docs/03-engineering-governance/current-work.md` 的 `TD-020` 到 `当前进行中`，新增任务卡片。
- 收尾：把状态改成 `🟢 完成`，并按 `docs/03-engineering-governance/01-rules/quality-gates.md#完成门禁` 收口。
