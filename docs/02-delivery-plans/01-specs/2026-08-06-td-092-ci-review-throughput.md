...
可迭代高风险范围包含 migration、`app/composition` Agent 组合根、`agent_workspace`、`agent_execution` 的生产实现。直接修改 `test_*.py` 在 Draft/Ready 均按 test-only targeted 运行该测试与 smoke；`conftest.py` 和 direct-root 内生 helper 运行其测试根目录，`tests/contexts` 非测试 helper、跨域复用的 `tests/shared` helper 与顶层全局 fixture 仍 full。Git rename 必须按删除+新增同时分类，不能只按目标路径降级。始终全量范围包含 CI/workflow、选择器、shared runtime、identity/security、全局 fixture、依赖锁和未知路径。
