...
> 6. **日志脱敏注入（R1-AC10）**：`caplog` 捕获 purge 路径日志 + 断言 operation/checkpoint/fence 字段与日志不含正文、ref 原值、Runtime session ref、CoT、secret、自由文本 reason（spec §9.3）；reason 必须落在冻结 code 集合。**ledger 例外（冻结）**：external ledger（`agent_external_object_refs`）按设计**必须**保存 `ref_scheme`/`ref_value` 作为外部对象唯一身份（E-1 ledger 是唯一事实源），**仅此身份字段例外**——ledger 不得承载正文、Runtime session ref、CoT、secret、自由文本 reason（`blocked_reason` 必须是冻结 code 集合值）。**禁止承载的敏感字段**：正文、Runtime session ref、CoT、secret、自由文本 reason；这些 sentinel 在 operation/checkpoint/fence/ledger/日志 全部不得出现。
...
> | 混合 conversation 多族同时故障 | 同一 conversation 内 external ref + runtime binding + transport outbox 同时故障，**三族并发执行**（external/runtime/transport 各真实调用 `erase_external_payload`/`erase_runtime_session`/`erase_transport_owner`，transport **不得**仅用 workspace outbox 作 external ref 种子冒充 transport 覆盖） | 各 owner 独立 blocked/unknown/erased，`checkpoint.reason_code` 逐 owner 精确（F-2a）；`operation.failure_code` 按 **F-2a 严重度优先级确定性聚合**（与提交顺序无关）；`Conversation.purge_state='blocked'`；partial ACK 不 completed | 混合种子（external ref + runtime binding + transport outbox inline）+ **`asyncio.gather` 并发**三族各注入**不同** reason 的故障；断言逐 owner checkpoint/fence/source ledger/binding 五方一致 + `operation.failure_code` 恒等于最高严重度 reason（重复跑确定性）+ 未 ACK owner 保持；变异：把 `_record_blocked` 改回 last-committer-wins -> 聚合断言非确定 -> 红 |
> | 日志脱敏（R1-AC10） | purge 路径触发（含故障），记录日志/operation/checkpoint | operation/checkpoint/fence/日志 不含正文/ref 原值/Runtime session ref/CoT/secret/自由文本 reason；**external ledger 仅允许契约要求的 external ref identity（`ref_scheme`/`ref_value`），不得含正文/runtime ref/CoT/secret/自由文本 reason**（F-3 机制 6 ledger 例外） | 用**不同 sentinel** 分别种正文、external ref、runtime ref、CoT、secret、自由文本 reason；断言 operation/checkpoint/fence/日志 不含任何 sentinel；ledger 仅含 external ref identity（不含正文/runtime ref/CoT/secret/自由文本 reason）；reason 字段落冻结 code 集合；变异：把 ref 原值写入 reason/failure_code -> 红 |
