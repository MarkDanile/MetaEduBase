...
#### R1-S6 契约冻结（contract-first，2026-08-19；承接 S4-F F-5 分工穷举 + S5-SCH-0/2 写者矩阵 + Spec §3/§8/§10/§11 + R1-AC1..12）

> Status: Draft（本批次仅纯文档契约冻结 + 审计事实对账；不写代码/测试/schema/migration/registry/CI；不启动 S6 实现/C1；S5 production wiring 不在本任务）
> 分支：`feature/req041-047-r1-s6-retention-clocks-fault-matrix-contract`
> 依据：S4-F F-5 已穷举 S6 交付项（plan 行 1316）；S5-SCH-0/2 已冻结 coordination 表写者矩阵；Spec §8 三 job 骨架、§10 发布顺序、§11 验收标准、§3 时间基线、§5.3 legal hold 生命周期。**本契约不复制**已冻结规则，全部以精确指针引用；只冻结 S6 新增事实与 S5 契约未覆盖的边界。
> 停止条件（任一命中即停止并报告）：需修改 CR-1/2/4/10/13/16；需新增 Conversation-owned writer 但无稳定 owner key；需修改已冻结 S5 状态机或写者矩阵（本契约 S6-4/S6-10 显式登记的例外除外）；需翻转 external/runtime production capability；需依赖不存在的 Pi Worker/云 adapter 宣称生产完成；真实基础设施 drill 无法执行且未登记生产门禁；发现任何 P0/P1；需修改门禁脚本/KNOWN_ISSUES/阈值/CI 配置通过检查。
> 首轮三面复审原始计数（保留不覆盖）：数据/状态机 P0=1/P1=4/P2=5/P3=4 + 并发/锁序 P0=0/P1=8/P2=7/P3=7 + 测试/运维/文档 P0=0/P1=0/P2=5/P3=6 → 合计 **P0=1/P1=12/P2=17/P3=17**，按族 1~10 统一返修一次（本版）：族1 schema 守卫冲突（P0-1→043 冻结需求）、族2 时钟/谓词、族3 SSE 语义方向、族4 audit 前置/删除集合、族5 矩阵遗漏/门禁、族6 T2 重验与裁决二、族7 retention 锁域/hold 检查、族8 restore 重放、族9 对账一致性、族10 巡检形态。P0 与 P1 全部闭环（返修后）。

##### S6-0 横向事实对账（冻结，截至 `main@cda7ea47`）

**审计结论总表（复用/新增/迁移/S5 修改/验证层级）**：

| 审计面 | 已有实现可复用 | S6 新代码 | 新 schema/migration | 需修改 S5 共享事实源 | 真实 PG-only | fake 仅 contract-tested |
|--------|---------------|-----------|--------------------|--------------------|-------------|----------------------|
| retention clocks | `purge_after` 30 天全链路 DB clock（agent_control_plane.py:614-621 写 / repository.py:348/353、bridge.py:664-672、conversation_purge_scheduler.py:214 读）；`persisted_at` DB clock（execution_repository.py:920/120-124）；SSE 410/409 映射（router.py:147-177）；`read_event_replay_window` 钳位（execution_query_repository.py:63） | `run_event_retention`（payload tombstone + envelope prune + `first_available_event_seq`/`event_log_complete` 推进）；`run_audit_retention`（365 天 prune）；hold 到期裁决 | **migration 043：append-only guard 白名单扩展**（放行 `expired`/`archived` tombstone 写 + 已 tombstone 行 DELETE，P0-1 评审裁决，expand-only 可逆；039/041 现拒 DELETE 与 non-redacted 写） | claim 谓词 + `has_active_legal_hold` 增加 `expires_at` 比较（S6-1 裁决一，R1-AC7 缺口）；settlement `_verify_t2_tokens` 补 `checkpoint.state` 重验（S6-1 裁决二，S5-SCH-2 冻结清单落地缺口） | payload/prune/audit 全部裁决 | retention worker 无 adapter 依赖 |
| writer 全集 | S5-SCH-0/2 矩阵（operation/checkpoint/fence/ledger/binding/Conversation purge 字段）；本契约 S6-4 扩展全表 | writer conformance suite（枚举门禁）+ 本契约自身四写者登记（run_event_retention / run_audit_retention / event-gap 巡检写者 = 三 N 类 + restore 重放执行器 = 一 M 类，均 owner=execution.core.v1，S6-4 增补行） | 无 | 无（矩阵扩展为纯登记） | — | — |
| 故障入口 | 双连接 gather（test_s4f_fault_matrix.py:165-171/1269+）、DB 篡改（:196-304/650-688）、`_CrashOnFirstCall`（test_s4eb2:1037、test_s4ec:171）、锁外 crash（test_s5_sch_d_settlement.py:931-989）、blocking adapter（test_s4f:618-628）、lease takeover（test_s5_sch_a:455-516）、writer-pause race（test_s3e_execution_race.py:361-545）、partial ACK 负向（test_s4f:1984-2083）、pg_locks 断言（test_s5i2_coordinator.py:748-820） | 9 类新注入（S6-5 清单：F2/F4/F5/F7/F8/F9/F10/F11/F13）：claim/acquire 半提交、outbox claim 短事务 crash、单 owner 分步 ACK、ACK 落账后聚合前 crash、hold create 与 entry 真并发、mutate-during-lookup、T1/T2 间 hold 推进、first_available_event_seq 写侧+SSE 组合、进程级 kill（等价注入裁决） | 无 | 无（除 retention clocks 行已登记的裁决一/裁决二两处修改点外，无新增） | 全部真实 PG | 进程级 kill 不存在，用租约过期+中途 raise 等价注入（S6-5 裁决四，等价性域 = DB 状态转移） |
| orphan 巡检 | transport reconcile 三态 ledger + 具名 issue + `unknown_scheme` 阻断（agent_transport_backfill.py:28-48/114-115、表 `agent_transport_scope_reconcile`/agent_transport_ledger.py:39）；`_find_event_gap`（execution_query_repository.py:94-152）；backfill verify（agent_transport_backfill.py:1144-1147） | body/ref 六类巡检（S6-6，只读为主 + gap 检出时幂等写 `event_log_complete=False`）；`event_log_complete=False` 写者（S6-6） | 无 | 无 | 巡检命令在真实 PG 验证 | — |
| 发布/迁移 | expand-only migrations 034-042（head=042）；fence backfill CLI（agent_erasure_backfill.py，幂等/分批/tenant 限流/退出码 0/1/2）；scope backfill CLI（agent_transport_backfill.py:1261，B2-B7）；registry snapshot/digest（agent_erasure_registry.py:130-198）；scheduler 三重 fail-closed（registry 4 True/2 False + 静态守卫 test_s5i2_production_wiring_boundary.py:41 + 无生产调用方） | writer capability 上报与 conformance suite（S6-7 承载 = owner_version 代码级，无新列）；missing-fence/body-scan 巡检命令；expand→capability→backfill→verify→canary 演练脚本 | 无（policy snapshot 裁决 = claim 时点快照已足够，S6-7 裁决六；无独立回填命令） | 无 | 演练在真实 PG + 本地脚本 | 基础设施 canary（多实例滚动）无法本地执行 → 登记生产门禁 |
| 备份恢复 | delete/restore/cancel-scheduled 全套（repository.py:319-437、erasure_repository.py:890-965、bridge.py:676-716）；operation/checkpoint/ref 持久化构成可重放账本（S5-SCH-0） | 账本**独立导出/归档机制**（快照后 purge 可重放，P1-7 裁决）+ 恢复重放执行器（含进行中 operation 处置，P1-8 裁决）+ restore-before-open runbook + body scan 复用（S5 终态扫描 + S6-6 巡检） | 无 | 无 | 重放机制在真实 PG 验证 | 完整基础设施 drill（pg_dump/恢复/流量开关）无法本地执行 → 明确登记生产门禁，完成声明降级（R1-AC12 字面） |

**关键事实（冻结）**：
- `run_event_retention` / `run_audit_retention` **当前不存在任何实现**；`expires_at` 全仓只写不读（恒 NULL，无调用方传值，domain/event.py:190/216 默认 None）；`payload_state='expired'/'archived'` 在 CHECK（models.py:737-740）中合法但**代码不可达**；`first_available_event_seq` 创建初值 1（run_coordinator.py:101）后**全仓无推进者**；`event_log_complete` 无置 False 路径；365 天 audit 时钟无任何代码（`ended_at` 唯一写点 execution_repository.py:795，落值 = DB clock）。**append-only 守卫（migration 039/041）现状**：`BEFORE UPDATE OR DELETE` 触发器只放行 `redacted` tombstone UPDATE（inline 或 ref 分支），**其余 UPDATE 与全部 DELETE 一律 RAISE**（migration 039 docstring 明文）——S6 的 `expired`/`archived` 写与 envelope/audit 删除必须经 **migration 043 扩展白名单**（P0-1 评审裁决，S6-10 冻结需求）。
- **legal hold 无 active→expired 迁移**：`has_active_legal_hold`（erasure_repository.py:1204-1220）与 claim `_has_active_hold`（conversation_purge_scheduler.py:532-545）只判 `state='active'`，**不比较 `expires_at`** —— 过期 hold 永久阻塞 purge（R1-AC7「expired hold」缺口）；hold create/release 时间戳用应用时钟（erasure_repository.py:1086/1186 `_utcnow`），purge/claim 裁决一律 DB clock。
- **S5 已合并代码本批次零改动**（registry 4 True/2 False、scheduler_composition.py 联合门禁、settlement T1/锁外/T2、claim/lease、rebuild、orchestrator 全部原样）；本契约登记**两处 S5 代码修改点**（均经 S6 复核证明为明确缺口，S6-10 例外清单）：裁决一 = claim 谓词 + `has_active_legal_hold` 增加 `expires_at` 比较（R1-AC7 过期 hold 缺口，最小谓词宽化）；裁决二族 = settlement `_verify_t2_tokens` 补 `checkpoint.state` 重验（S5-SCH-2 冻结清单明文要求但已合并代码未落地——T2 现只验 attempt/digest 不验 state，settlement.py:699-702；伪造 ACK 防线，R1-AC9 相邻）。

##### S6-1 Retention clocks 事实源与时钟边界（冻结）

1. **时钟事实源（全系统统一）**：所有 retention/restore/purge 裁决以 PostgreSQL UTC 时间裁决——`clock_timestamp()`（DB 事务时钟）。应用时钟只允许测试注入（`now=` 参数）与 erasure 操作行时间戳落值（`_utcnow`，不参与裁决）。冻结：**新增 retention worker 的到期裁决一律 `SELECT clock_timestamp()`**，禁止 API/Worker 本机时间。
2. **时间基线（Spec §3 表格，冻结默认值）**：Conversation recovery 30 天（`deleted_at` 起算，已实现）；RunEvent 热重放 90 天（`persisted_at` 起算）；AgentRun 终态/审计 envelope 365 天（`ended_at` 起算）；legal hold 至显式解除或 `expires_at`。tenant policy 只能延长（缩短需单独数据合规评审）；**当前无 tenant policy 表**——retention worker 时钟 = 冻结默认基线（S6-7 裁决六：claim 时点 `retention_policy_snapshot` 已足够承载 purge 侧策略；retention worker 侧无 policy 表，冻结默认值硬编码为 90/365 天）。
3. **裁决一（hold 到期读侧裁决，S6 契约冻结；S5 代码修改点 #1）**：hold 到期不新增状态机写者——`has_active_legal_hold` 与 claim `_has_active_hold` 的 active 判定从 `state='active'` 宽化为 `state='active' AND (expires_at IS NULL OR expires_at > now)`（now = DB clock）；**不 bump `hold_revision`**（hold_revision 仍只由 create/release 推进，I1 冻结语义不变，无 G2 drift 无 rebuild 触发）。理由：Spec §3「legal hold…至显式解除或 `expires_at`」与 §5.3「active hold 阻止…」——过期 hold 非 active，读侧宽化是 R1-AC7 缺口的直接修复；无新写者、无锁序变化、无 writer matrix 变化。**时钟边界（冻结）**：hold 时间戳（created_at/expires_at 落值）维持应用时钟现状（I1 冻结，erasure_repository.py:1086/1186），裁决一律 DB clock——到期边界受「应用↔DB 时钟偏斜」影响；冻结可接受偏斜上界 = 服务时钟 NTP 同步假设（±5 分钟，运维登记，不新增代码改动；30 天级 hold 周期下偏斜可忽略）。**判别测试范围（限定）**：过期 active hold 不再 defer claim/不再 block participant/不再 block 90/365 prune；未过期 hold 行为不变；`expires_at` NULL 仍 active；**宽化只解除读侧阻塞——已 G2-blocked 的旧 operation（hold_revision 已推进、单调不回落）仍走 S5 rebuild 通道，判别测试不得断言过期 hold 使旧 operation 恢复完成**。
4. **裁决二（T1/T2 间 hold 推进，S6 契约冻结）**：settlement `_verify_t2_tokens`（settlement.py:669-713）**不重验 hold_revision** —— **supersede 注记（冻结）**：本裁决**覆盖 S5-SCH-2 T2 token 清单的 `hold snapshot` 项**（S5-SCH-8「本行 supersede」先例同款登记）——T2 对 hold 只做单向回归检查（`hold_revision_snapshot > conversation.hold_revision` 才 fail closed，settlement.py:840 现状；hold **推进**放行、**回退/脏数据** fail），语义 = hold 只阻止 purge owner 从 `active` 进入 `erasing`（Spec §5.3），不阻止已进入 erasing 的收口；`purge completed` 后新增 hold 只能保存审计（既有 I2 拦截 test_s5i2_coordinator.py:952-1003）。**同族修正（S5 代码修改点 #2）**：S5-SCH-2 T2 清单的 `checkpoint state` 项**未在已合并代码落地**（`_verify_t2_tokens` 只验 attempt/checkpoint_digest，settlement.py:699-702 无 state 校验）——S6 实现阶段**补 `checkpoint.state == 'erasing'` 重验**（与 fence.state 校验同构，任一失败 fail closed 零写），判别测试：T2 前篡改 checkpoint.state → fail closed 零写（S6-F11 载体）。**判别测试（S6-5 S6-F10）**：T1 提交后、T2 前 create hold → T2 照常完成 erase（fence erased + checkpoint acked，零正文复活、hold 行保留为审计）→ **后续聚合投影 `blocked_hold_revision_changed`（G2 门禁，projection_calculator.py:319-326：`hold_revision_snapshot < conversation.hold_revision`）** → 经 S5-SCH-1.5 rebuild 通道最终 completed（SCH-C 前置；S6-F10 不得断言原 operation 直接 completed）。
5. **SSE 语义（冻结，代码事实为基准，不修改读路径）**：错误映射 `EventHistoryExpiredError`→410 `event_history_expired`；`EventGapDetectedError`→409 `event_gap_detected`；`EventCursorAheadError`→409 `event_cursor_ahead`（router.py:147-177）。**分支方向（run_query_service.py:262-272 冻结代码事实；本契约原稿方向相反，三面复审纠正）**：`after_seq < first_available_event_seq - 1` 时——`event_log_complete=True` → **409 `event_gap_detected`**；`event_log_complete=False` → **410 `event_history_expired`**（既有测试 test_run_api.py:616-642 即 `first_available=2 + event_log_complete=False → 410` 形态）。**S6 语义承接（冻结）**：retention 剪前缀推进 `first_available_event_seq` 时**同事务置 `event_log_complete=False`**（新增写者：retention worker，owner=execution.core.v1，Run 行锁内，S6-4 登记）——「历史已过期」显式标记，早于窗口请求返回**稳定 410**；巡检发现内部空洞同样置 False → 410；窗口内实际空洞仍由既有逐条校验给出 409（run_query_service.py:278-290）。R1-AC6「SSE 返回稳定 410/409」由该映射满足。

##### S6-2 run_event_retention（冻结）

1. **处理对象与谓词**：`agent_run_events` 行，谓词（锁内重读）=`payload_state IN ('inline','external') AND ((expires_at IS NOT NULL AND expires_at <= now) OR (expires_at IS NULL AND persisted_at + 90d <= now))`（now = DB clock；**expires_at 恒 NULL 现状下以 `persisted_at + 90d` 为主路径**，expires_at 保持「可选业务到期提示」语义、非保留裁决唯一依据——三面复审 P1-1 纠正）**AND 语句级 hold EXISTS**（Conversation 无 active hold，裁决一读侧语义——Spec §5.3「active hold 也阻止 90 天 prune」，三面复审 P1-5 补入）。**ref-bearing（external）行处置（冻结，定向复核 P1-2/P2-4 钉死）**：到期写 = 仅 `payload_state` 变化（inline 已 NULL 不动，`payload_ref` 保留至 external.payload.v1 清除——S4-E）；该写需 043(a) 新增独立分支「`OLD.payload_state='external' AND OLD.payload_inline IS NULL AND NEW.payload_state IN ('redacted','expired','archived') AND 其余列不变`」（仅 state 变化，payload_ref 保留）；其 envelope 不进入前缀 prune（payload_ref 未清不满足 043(b) 白名单），audit 删除前置含 `payload_ref IS NULL`（S6-3 item 1）。**已 tombstone 行（redacted/expired/archived）不在 retention 范围**——与 erasure participant 的 `_clear_event_payloads`（execution_erasure_participant.py:964-996）谓词隔离：erasure 处理 purge 后的行（bulk UPDATE、不取 Run 行锁），retention 处理时间到期的行，二者对同一行并发时（erasure 设 redacted / retention 设 expired）最终态均为「payload_inline NULL + tombstone state」，语义等价——冻结为允许并发（**幂等可交换 tombstone 写**；scan 零命中以 payload_ref 亦清为前提，而 ref 清除唯一者 = external participant，retention 禁清 ref）。
2. **payload expiry（第一阶段）**：`payload_inline → NULL + payload_state='expired'`（保留 seq/type/digest/size/media_type/classification/provenance/runtime **八元**）；**该写必须经 migration 043 guard 白名单扩展放行**（039/041 现仅放行 `redacted`，P0-1 裁决；external 行仅 state 变化分支见 item 1）；`payload_ref` 属于 `external.payload.v1` owner——retention **不得清除 payload_ref**（与 S4-E `_clear_source_ref` 分工：ref 清除唯一者 = external participant；043 白名单的 ref 分支保持 redacted-only）。
3. **envelope prune（第二阶段，与 payload expiry 同批）**：只删除**完整连续前缀**（从 `first_available_event_seq` 起连续存在且全部满足 predicate 2 的行——payload 已 tombstone（expired/redacted/**archived**）且 envelope 到期（`persisted_at + 90d <= now`））；**任何非连续/未到期/未 tombstone 行立即停止前缀推进**（不制造内部 gap，Spec §8/R1-AC6）。**DELETE 必须经 migration 043 白名单扩展放行（仅限已 tombstone 行：`OLD.payload_state IN (redacted/expired/archived) AND payload_inline IS NULL AND payload_ref IS NULL`，P0-1 裁决——guard 不得对 live 行开 DELETE 洞）**。删除后同事务原子推进 `agent_runs.first_available_event_seq = 删除末行 seq + 1`（单调，受既有 CHECK `first_available_event_seq >= 1 AND <= next_event_seq` 约束，models.py:300-306）**并置 `event_log_complete=False`**（「历史已过期」显式标记，S6-1 item 5 承接；同事务、Run 行锁内、新增写者登记 S6-4）。
4. **锁/事务/租约（裁决三，S6 契约冻结）**：**Run 行 FOR UPDATE**（与 `_append_event_locked` 同锁，execution_repository.py:981-989 `_require_run_for_update`）串行化 writer/retention/ingest；自管短事务分批（batch 冻结默认 100 行）；**不取 Conversation 行锁、不取 owner advisory、不取 fence**——retention 只写 tombstone 状态与推进 seq 指针，非正文写、无持有锁 adapter I/O；锁序无环（retention 不反取任何上游锁；hold 检查 = 语句级无锁 EXISTS，不取 Conversation 行锁）。多实例并发无害：谓词 + 行锁保证幂等（已处理行不命中）。与 worker 互斥：无独立 lease 表——**Spec §8「三 job 共用 bounded claim lease」字面偏离登记（supersede 注记）**：意图（实例间互斥 + 有界工作量）由 Run 行锁 + 幂等谓词满足（行级单写者、已处理行不命中）；retention 无 operation 账本故无 lease 表需求（新增表需 043+ 新表，违背最小化）；实现阶段若需实例级节流再评估。**索引评估（自检登记）**：批处理谓词列（payload_state/persisted_at）当前无专用索引（expires_at 亦无，models.py:827-829）——低频全表扫描批处理可承受；若实现阶段量级评估需索引，经独立 expand-only migration（043 之后，不预设）。
5. **审计/指标**：prune 计数（payload 数/envelope 数/推进的 first_available_event_seq）按 tenant 记录；**不得记录 payload 正文、event ref、Runtime session ref 或自由文本 reason**（R1-AC10；retention worker 的日志与指标只允许计数与 time，沿用 S4-F F-3 机制 6 的 sentinel 判别要求——新写者纳入脱敏判别）。

##### S6-3 run_audit_retention（冻结）

1. **处理对象与谓词**：`agent_runs` 行，谓词（语句级重读，全真才可清理）=`status ∈ TERMINAL_RUN_STATUSES`（completed/failed/cancelled/expired，domain/run.py:37-44）AND `ended_at <= now - 365d`（ended_at 落值 = DB clock，execution_repository.py:795）AND **语句级 hold EXISTS**（Conversation 无 active hold，裁决一读侧语义——**并入 DELETE WHERE，语句级重验**，消除「检查后、删除前 hold 提交」的提交间隙，三面复审 P2-6 裁决）AND **被删 run 的 events payload 全部已 tombstone**（`payload_state IN ('redacted','expired','archived') AND payload_inline IS NULL AND payload_ref IS NULL`；90 天 retention 先行或 purge 已处理——audit 不越权清 payload，worker 间顺序约束；**ref-bearing 行须经 external.payload.v1 清除 ref（S4-E）后方可删——external 生产不可达 → 该情形结构上仅存于测试，判别测试注入，定向复核 P1-2 钉死**）。**blocked（零写，进入下一轮）**：run 非终态（ACTIVE_RUN_STATUSES + queued）；outcome_unknown（`TOOL_OUTCOME_UNKNOWN` 事件无后续 resolve，或 run 停于 `resume_required`（ACTIVE 态）的 unknown 分支——`resume_required` 非终态，三面复审措辞纠正）；未解决审批（`approval.requested` 事件无对应 `approval.resolved/expired`——判定基于**现存 envelope 的 seq 比较**；相关 envelope 已随连续前缀剪除 → 无法判定 → **fail closed blocked**（同 `unresolved_action` 业务保留语义，人工处置，三面复审裁决））；projection reconcile 未完成（`output_publish_state ∈ ('pending','dead_letter')`（CHECK 值域无 'claimed'——claimed 是 outbox 行状态，三面复审纠正）或对应 outbox 行存在非终态（pending/claimed/dead_letter），或 inbox 存在非终态 receipt 行）；**event ref 未清**（存在 payload_ref 非空的 ref-bearing event 行——external.payload.v1 未清除，定向复核 P1-2 补入）；**存在存活子 run**（`parent_run_id` 引用本 run 的子 run 行，FK `fk_agent_run_parent` NO ACTION models.py:284-288——父 run 仅在其全部子 run 亦到期可删后方可清理，子 run 按各自 365 天时钟独立处理，定向复核 P1-3 补入）；业务保留例外（**结构性无**：当前无 tenant policy 表/无标记列，未解决审批等 unresolved 条件即承担业务保留语义（blocked 即保留）；tenant policy 落表时（043 区间契约修订）worker 读该表——本契约不预设）。
2. **删除集合（冻结）**：同事务删除（**children-first，显式顺序**）`agent_turn_inputs`（FK NO ACTION，models.py:597-600）→ `agent_run_events`（FK NO ACTION，models.py:673）→ `agent_compatibility_outputs`（**显式删，不依赖 CASCADE 语义**；FK CASCADE 存在 models.py:515-523，显式删消除歧义）→ `agent_runs` 行（最后）；**自引用 `parent_run_id`（FK `fk_agent_run_parent` NO ACTION，models.py:284-288）不在删除集合——父 run 仅当无存活子 run 时满足谓词（item 1 blocked 清单），跨 run 删除顺序不适用**（定向复核 P1-3 钉死）；`runtime_session_bindings` **不随 audit prune 删除**（binding 生命周期归 runtime.private.v1 与 execution 生命周期，audit 不越权——CR-1 context ownership）；outbox/inbox 已终态行不随 run 删（transport 账本由 S4 矩阵管理；**删除事务与 claim_output_outbox（SKIP LOCKED 不取 Run 锁）并发可产生孤儿 outbox 行——由 orphan 巡检（S6-6 第六类 orphan transport 行）处置，冻结该边界，定向复核 P1-4 钉死**）。**删除前置（与 purge 路径区分）**：被删 run 的 events payload 必须全部已 tombstone（item 1 谓词）；terminal_output/context_snapshot/actor 列**随 run 行同集合删除**（无孤儿——terminal_output_ref/compat 同事务删、context_snapshot_ref 随行删；audit 删除 = 策略性移除，**无 ACK/scan 记账语义**（与 purge 的六 owner scan-zero ACK 不同），删除即终态，不存在「audit envelope 含可恢复正文」残留——三面复审 P1-3/P2-2 裁决：非 completed 终态 run 由 CHECK 强制 `output_publish_state='not_required'` + terminal_output 全空（models.py:341-376），completed run 的 terminal_output 随行删除）。**conversation 已 purge（purged_at 非空）→ 六 owner 已 ACK 扫描为零 → events payload 前置自动满足（purge 已 redacted 全量 payload），冻结为短路**。
3. **锁/事务/幂等（锁域复制 S6-2.4，裁决三）**：**Run 行 FOR UPDATE + tenant 谓词；不取 Conversation 行锁、不取 owner advisory、不取 fence**（hold 检查 = DELETE WHERE 内语句级 EXISTS——避免持 Run 锁等 Conversation 的 AB-BA（writer 持 Conv 等 Run），三面复审 P1-6 裁决）；自管短事务分批；先查后删同事务（谓词即幂等键，重入不命中已删行）。与 `run_event_retention` 独立 worker；二者对同一 run 的并发由 Run 行锁串行。
4. **hold 联动**：audit prune 的 hold 检查 = 语句级 EXISTS（裁决一语义）；hold 只阻止 prune，不恢复已清正文（Spec §5.3 冻结）；已删数据在 hold 生效期间不回滚（Spec §5.3「不允许恢复已清除正文」）。

##### S6-4 Conversation-owned writer 全集与 conformance suite（冻结）

**S5-SCH-0/2 已冻结表**（operation/checkpoint/fence/ledger/binding/Conversation purge 字段）**不复制**，逐字引用。本矩阵扩展其余 Conversation-owned 表（截至 `main@cda7ea47` 审计事实）。写者行格式：`写者（文件:行）→ 触发链 → 锁序 → 事务 → tenant → revision/epoch → fence 状态`。fence 状态值域：`F`=fenced（owner lock + fence FOR UPDATE）、`O`=owner lock 但无 fence、`N`=无锁无 fence（非正文/维护路径，显式裁定）、`D`=死代码、`M`=维护路径（集合锁，故意）。

**agent_conversations（owner 矩阵引用 S5-SCH-0；新增行）**：
| 写者 | 触发 | 锁序 | 事务 | tenant | revision | fence |
|------|------|------|------|--------|----------|-------|
| create_conversation（repository.py:56-125） | POST /conversations | 无（INSERT 新行） | 调用方 | ✓ | creation_digest 幂等 | N（新行无 fence；同事务建 baseline fence） |
| set_title（repository.py:250-294） | PATCH | Conv FOR UPDATE→owner→fence | 调用方 | ✓ | revision CAS | F |
| archive（repository.py:296-317） | POST archive | Conv FOR UPDATE | 调用方 | ✓ | revision CAS | N（非正文） |
| soft_delete_after_guard（repository.py:408-437） | DELETE（Guard 后） | Guard→Conv | 调用方 | ✓ | purge_revision+1 | N（生命周期） |
| restore + cancel_scheduled（repository.py:319-406/erasure_repository.py:890-965） | restore | Guard→Conv→owner→fence 全集裁决 | 调用方同事务 | ✓ | 四元 CAS+purge_revision+1 | F 裁决（bridge.py:676-716） |
| reserve_user_turn seq/activity（repository.py:590-593） | submit_turn | Guard→Conv→owner→fence | 调用方 | ✓ | — | F |
| project_assistant_message seq（bridge_repository.py:791-794） | dispatch_output | Conv→owner→fence | 调用方 | ✓ | — | F |
| project_suppressed_output seq（bridge_repository.py:847-848） | suppress_output_projection | Conv（allow_purge_fenced=True） | 调用方 | ✓ | — | N（purge 期间唯一正文侧写路径，tombstone 契约） |
| coordinator 聚合投影（transactional_projection_coordinator.py:510-592） | scheduler | Conv→operation→checkpoint→fence 只读 | 独立事务 | ✓ | CAS | O（fence 只读） |
| create/release_legal_hold bump（erasure_repository.py:1103-1110/1193-1200） | I1 producer | Conv→hold 行 | 调用方 | ✓ | hold_revision 原子 bump | F（I1 冻结） |
| purge_rebuild（purge_rebuild.py:223-229） | SCH-C | Conv 锁内 | rebuild 单事务 | ✓ | purge_revision+1 | O（S5-B case E） |
| _erase_conversation_title（workspace_erasure_participant.py:987-998） | orchestrator | Conv→owner→fence→aggregate | entry 事务 | ✓ | — | F |
| _anonymize_conversation_actors（workspace_erasure_participant.py:1000-1027） | 同 | 同 | 同 | ✓ | — | F |

**agent_messages / agent_message_parts**：reserve_user_turn INSERT（repository.py:552-589，F）；project_assistant_message（bridge_repository.py:759-790，F）；project_suppressed_output tombstone（bridge_repository.py:827-846，N allow_purge_fenced）；claim_turn_outbox 毒事件（bridge_repository.py:180-193，N claim 短事务）；acknowledge_turn_outbox（bridge_repository.py:271-273，**N 独立事务无 Conv/fence——dispatch 状态写**）；record_turn_delivery_failure（:313-319，N）；retry_turn_dispatch（:359-361，F Guard 内）；abandon_or_reconcile_turn（:434-456，F）；terminalize_turn_epoch_rejected Tx2（:1180-1182，N claim CAS）；_redact_messages（workspace_erasure_participant.py:1029-1078，F）；_delete_message_parts（:1080-1095，F，物理删除）。

**agent_conversation_user_state**：set_pinned（repository.py:439-474，**N 无锁无 fence 无 revision**——非正文，purge 后 created_by 匿名化间接阻断，S6 裁定维持现状）；_delete_user_states（workspace_erasure_participant.py:1097-1107，F）。`last_read_message_seq` **全库无写者**（仅 INSERT 默认 0）——登记 P3。

**agent_workspace_outbox / inbox**：add_turn_outbox（bridge_repository.py:62-120，F，producer_purge_revision 锁内真实值）；claim_turn_outbox（:134-196，N claim 短事务，全局 skip_locked 无 tenant 谓词——设计如此）；acknowledge_turn_outbox（:238-275，N）；record_turn_delivery_failure（:277-321，N）；retry_turn_dispatch（:323-363，F）；abandon_or_reconcile_turn（:405-458，F）；terminalize_turn_epoch_rejected（:1116-1183，N claim 四元 CAS）；erase_transport_body（workspace_transport_erasure_participant.py:220-229/243-281，F）；external _clear_source_ref（external_ref_erasure_participant.py:975-995，F，ref 唯一清除者）；create_output_receipt_rejected（bridge_repository.py:593-678，F Tx1）；begin_output_receipt（:680-719，F）；_consume_output_receipt（:927-948，F）；recompute_projection（agent_transport_ledger_service.py:79-117，M 集合锁）；backfill（agent_transport_backfill.py:505-517，M）。

**agent_runs / agent_run_events / agent_turn_inputs**：create_run_with_root（execution_repository.py:164-235，F 上游 Guard+Conv+fence）；start_run（:275-333，F，Conversation 前缀锁）；transition_run（:368-428，F/Run 行锁）；mark_run_resume_required（:430-496，F）；resume_run（:498-562，F）；commit_terminal（:718-891，F，ended_at 唯一写点 :795，producer_purge_revision 锁内真实值，COMPLETED 同事务 outbox INSERT :869-889）；reserve_cancel_intent（:245-273，**D 死代码无 caller**——若复活必须走 fenced port，矩阵登记警告）；_append_event_locked（:908-967，**run_events payload 唯一写者**，7 条路径，seq 在 Run 行锁内单调）；bridge 投影族（bridge_repository.py:396-409/449-450/529-534/564-565/591-592/622-623/670-671/762-764/842-844，O outbox→Run 锁序 _lock_output_then_run :897-933）；execution erasure participant（_clear_terminal_outputs :851-913 / _clear_context_snapshots :915-938 / _anonymize_actors :1010-1068 / _clear_event_payloads :964-996，F）；external _clear_source_ref（external_ref_erasure_participant.py:963-977，F）；_clear_compatibility_outputs（execution_erasure_participant.py:940-962，F）；stage_with_created（compatibility_output_service.py:62-115，**N 零行锁，依赖调用方 fenced_stage 裁决——裸调用即未栅栏写，矩阵登记**）。

**agent_runtime_session_bindings（关键缺口族，S6 裁定）**：create_runtime_binding / activate_runtime_binding / claim_ingest_stream（repository.py:182-203/218-241/243-278，经 execution_identity_service.py）——**N：无 Guard/Conv 行锁、无 owner advisory、无 fence FOR UPDATE**（仅 binding 行锁 + epoch/revision CAS）；ingest ack 推进（execution_repository.py:711-714）；runtime_erasure_participant Tx2 _clear（runtime_erasure_participant.py:935-980/1108-1120，F）。**裁定（冻结）**：binding 生命周期写者 = `runtime.private.v1` 域内写者，R1-AC3 要求与 purge 共用 owner lock/fence；当前 Runtime 通道**无生产调用方**（fenced_ingest_runtime_event/identity service 均无 API/调度接线），与 external/runtime erase capability 同构——**实现阶段这些写者必须接入 fence 协议（或保持生产不可达）后才能启用 Runtime**；本契约冻结：conformance suite 必须列出该缺口族并以「生产不可达 + 接线门禁」标记，**不宣称已修复**（等价 external/runtime False 后置姿态）。

**S6 本契约新增写者（矩阵登记，owner=execution.core.v1，N 类——非正文 tombstone/删除维护路径，显式裁定）**：`run_event_retention`（写 run_events payload_state/payload_inline + agent_runs first_available_event_seq/event_log_complete；锁 = Run 行 FOR UPDATE；事务 = 自管短事务分批；tenant ✓；revision 无；**不取 Conv/owner/fence——S5-SCH-0「Conversation-first」不变量覆盖域 = operation/fence/checkpoint 行锁，Run 行锁不在该域，文字确认边界**）；`run_audit_retention`（删 agent_runs/run_events/turn_inputs/compat_outputs；锁 = Run 行 FOR UPDATE + children 行级锁；事务 = 自管短事务分批；tenant ✓；revision 无）；`event-gap 巡检写者`（置 agent_runs.event_log_complete=False；锁 = Run 行 FOR UPDATE；tenant ✓）；`restore 重放执行器`（S6-8 item 3；对恢复库执行账本记录的清除步骤；锁 = **M 类集合锁**；事务 = 一次性维护事务；tenant ✓；revision 无；**与 retention/audit jobs 互斥（重放期间暂停）**；不重复 adapter 调用，external/runtime 未 acked 项记 blocked+reconcile）。前三个均在 S6-2/S6-3/S6-6 冻结谓词内幂等。

**conformance suite 门禁（冻结；验证层级 = 静态枚举（非真实 PG）+ fence 判别（真实 PG））**：实现阶段新增 `writer_conformance` 套件——静态枚举本矩阵全部写者（**含 S6 自身三 N 类写者 + 一 M 类写者**，遗漏任一 → 门禁失败）；对每个 `F` 写者断言 fence 协议（owner lock + fence FOR UPDATE + 同事务 commit，R1-AC3 判别：fence 非 active 拒写）；`N`/`M`/`D` 写者逐项断言裁定理由（非正文/维护路径/死代码）；`O` 写者断言锁序合法；**`stage_with_created`（N 类正文写者）增加调用方门禁断言——生产调用方集合 ⊆ fenced port（execution_fenced_port.py:420 现状唯一生产调用方；断言防止未来裸调用回归，三面复审 P2-4 裁决）**。**审计未发现无稳定 owner 的正文写者**（binding 族归 runtime.private.v1；dispatch 元数据非正文；catalog 表 agent_definition_versions/agent_runtime_profiles 非 conversation-owned 单列不纳入）——停止条件 2 不触发。

##### S6-5 真实故障矩阵（冻结）

**故障点清单（S6 覆盖全集，含复用与新增）**：

| # | 故障点 | 注入机制 | 期望行为 | 复用/新增 |
|---|--------|---------|---------|----------|
| S6-F1 | Worker kill（claim 后、聚合前） | claim 事务提交后中途 raise/进程死 → 租约到期 → takeover | lease_epoch CAS +1 单调、强制聚合、零残留（复用 test_s5_sch_a:455-516 骨架 + 双连接 gather 升级） | 复用+新增（既有 SCH-A claim-crash 已覆盖串行场景，本行升级为双连接 gather 并发验证） |
| S6-F2 | claim/acquire 半提交（_claim_fresh 建行后 acquire 前死） | **claim 为单事务（建行+acquire 同事务，conversation_purge_scheduler.py:329-368），raise 不可达——SQL 篡改：保留 operation/checkpoint 行 + 重置 lease_epoch=0、lease_expires_at=NULL**（_ACQUIRE_SQL 谓词依赖） | 全 owner checkpoint 行存在但无 lease → 幂等 claim 收敛为单一写者（SCH-2 判别） | 新增 |
| S6-F3 | lease/ACK 丢失 | checkpoint 退回 pending + 清 ack_digest（test_s4da:1239-1277 手法） | 重放修复 acked，无分叉 | 复用 |
| S6-F4 | 单 owner 内分步 ACK（部分 ref 先 ACK、部分失败） | 多 ref adapter 逐 ref 故障注入 | 已 ACK ref 保持、失败 ref 重放继续收口、attempt 不重复推进 | 新增 |
| S6-F5 | ACK 落账后、operation 聚合前 crash | checkpoint/fence 已写后 raise | takeover/重入按 checkpoint 态恢复账本，不重跑已 acked owner，coordinator 聚合完成 | 新增 |
| S6-F6 | seq gap | **测试事务内 `ALTER TABLE agent_run_events DISABLE TRIGGER ALL` 临时窗口 raw DELETE 中间行**（隔离测试库单事务，re-enable；043 白名单对 live 行不开 DELETE 洞，故需临时禁触发器）+ 篡改 | `_find_event_gap` 检出；巡检置 `event_log_complete=False` → 早于窗口 410；窗口内空洞 409（既有逐条校验） | 复用+新增 |
| S6-F7 | first_available_event_seq 写侧推进 + SSE 重放组合 | retention prune（置 event_log_complete=False）后重放早于窗口 | **410 `event_history_expired` 稳定**（S6-1 item 5 承接）；推进单调、无内部 gap | 新增 |
| S6-F8 | outbox claim 短事务内 crash | claim 后 raise | 行锁 SKIP LOCKED 语义下重入重取；已 claimed 行由消费事务重验 | 新增 |
| S6-F9 | hold create 与 participant entry 真并发 | 双连接 gather（非 drift 预置） | entry 在 create 前 → 完成；entry 在 create 后 → hold 快照校验 fail closed / blocked（S5-SCH 冻结语义） | 新增 |
| S6-F10 | T1/T2 间 hold 推进 | 锁外阶段第二连接 create hold | T2 完成 erase（fence erased + checkpoint acked，零复活正文）→ 后续聚合投影 `blocked_hold_revision_changed`（G2，projection_calculator.py:319-326）→ rebuild 通道（S6-1 裁决二判别，不得断言原 operation 直接 completed） | 新增 |
| S6-F11 | mutate-during-lookup | 锁外 lookup 时第二连接篡改 checkpoint.state/attempt/digest/源行 | T2 重验失败 → fail closed 零写（裁决二冻结；**checkpoint.state 重验 = S5 代码修改点 #2 判别载体**） | 新增 |
| S6-F12 | writer/purge pause race（fence check 后暂停） | test_s3e_execution_race.py:361-545 事件 barrier 手法 | writer 先提交 → purge 锁等待后清除全量；purge 先提交 → writer 拒写 | 复用 + **新增**（扩展 = retention vs writer 由 Run 行锁串行断言，S6-2 锁域判别） |
| S6-F13 | 进程级 kill | **裁决四**：真实 SIGKILL 在本仓测试不存在且不引入（不可复现/CI 不稳定）；以「租约过期 + 中途 raise + 双连接」等价注入覆盖。**等价性域（限定）**：只覆盖 DB 状态转移等价（事务回滚/租约到期/token 失效），**不覆盖 SIGKILL 独有现象**（连接中断、半提交 TCP、会话内共享锁残留）——后者属真实基础设施 drill 范畴，登记生产门禁（三面复审 P3-4 裁决） | 同 F1/F5（等价域内） | 裁决（等价注入） |
| S6-F14 | 跨 tenant / 伪造 ACK / revision 重放 | S4-F F-6 已冻结机制（双 tenant 种子 + 篡改 + gather） | 零写 fail closed | 复用 |

**注入机制（冻结）**：真实 PostgreSQL 双连接 `asyncio.gather`（test_s4f_fault_matrix.py:165-171 `_make_factory` NullPool）；DB 篡改（raw SQL UPDATE，:196-304/650-688）；crash 注入（`_CrashOnFirstCall` / `_FirstLookupCrashAdapter` 模式）；blocking adapter（asyncio.Event barrier）；租约过期（`_expire_lease`）；pg_locks 直接断言（test_s5i2_coordinator.py:748-820 手法，锁外 0 行锁）；**全部真实 PG，SQLite/mock 只覆盖纯状态转换**（plan §6）。fake Runtime/external adapter 只标 contract-tested。

##### S6-6 body/ref orphan 巡检（冻结）

**六类巡检 + 分类处置（fail closed）**：

| 类 | 检什么 | 证据 | 处置 |
|----|--------|------|------|
| tenant mismatch | 双 tenant 同 ref 值/跨 tenant 引用 | 源行 tenant 谓词对账 | 登记 reconcile ledger（`cross_tenant_mismatch` 具名 issue 已有基座）+ 阻止对应 conversation purge/SSE 放行 |
| digest conflict | checkpoint_digest/ack_digest/ingress_digest/registry_digest 与重算值不一致 | 重算比对 | 报告 + 阻止 purge（fail closed）；不得自动改写；**restore 场景重算基准 = 账本快照自身的 owner_version（旧代码产出的账本不得用新版本重算判定），失配走 runbook 人工确认（S6-8）** |
| event gap | run_events 内部空洞（`_find_event_gap` 复用） | 窗口函数扫描 | 置 `agent_runs.event_log_complete=False`（**新增写者：巡检命令，owner=execution.core.v1，Run 行锁内**）→ SSE 对相关 run 返回 **410 `event_history_expired`**（event_log_complete=False 分支，S6-1 item 5）；窗口内空洞仍 409；登记 reconcile |
| unknown ref scheme | external ref 无已登记 scheme | `agent_external_object_refs` scheme 白名单（已有 `unknown_scheme` 阻断） | fail closed 零写（S4 冻结）|
| missing fence/owner scope | conversation 缺已安装 owner 的 fence；inbox/outbox 缺 conversation_id | backfill verify 五维复用（agent_transport_backfill.py:1144-1147）+ missing-fence 扫描 | 登记 reconcile 队列 + 阻止对应 conversation scheduler-enable；**无法可靠回填的行不得被跳过**（R1-AC11） |
| orphan transport 行 | outbox/inbox 行引用不存在的 run/conversation（audit 删除与 claim 并发、异常清理产生；定向复核 P1-4 补入） | 外键/存在性扫描 | 登记具名 issue（`source_run_missing` 基座复用）+ 阻止 + 人工处置（终态行保留证据，不自动删） |

**巡检形态（冻结）**：可重复执行的 CLI（`verify` 命令，退出码 0=零发现/1=有发现/2=不可判定）——**只读为主**，唯一写路径 = event gap 检出时幂等置 `agent_runs.event_log_complete=False`（Run 行锁内短事务，具名 reconcile 登记；三面复审 P2-3 裁决）+ 每个 conversation 的巡检结果以具名 issue 登记表 `agent_transport_scope_reconcile`（state ≠ resolved → gate 阻断 scheduler-enable，既有语义）；**发现即阻止（fail closed），不得静默修复**；digest/tenant 类发现必须人工处置后重跑（restore 场景的重算基准见 S6-8）。

##### S6-7 发布迁移流程（冻结）

1. **顺序（Spec §10 逐字引用，不复制）**：expand（migrations 034-043 全部 expand-only）→ writer capability → batched backfill → verify → canary enable。**V1 不支持 purge 开启时仍有旧 Writer 进程在线**（Spec §10）：Docker Compose 维护窗口切换；滚动发布须实例 capability 门禁——本仓库无该基建，冻结：**scheduler 三重 fail-closed 维持**（registry False + 静态守卫 + 无生产调用方），canary enable 只以测试环境 + tenant 级演练承载，生产启用留 REQ-047 conformance。**旧 Writer 在线模拟（三面复审 P2-7 裁决，AC11 载体）**：演练脚本显式部署**旧 writer 代码变体**（capability/owner_version 失配注入或旧版本 checkout）+ 每阶段 fail-closed 判别点（pre-canary：三重 fail-closed 断言；canary 中：能力失配 gate 失败、scheduler 拒绝执行）；conformance suite 只能断言本进程代码，检测不到远程旧进程——判别点必须在演练脚本内。
2. **writer capability（裁决五，冻结）**：Spec §10.2「writer 上报稳定 capability version」以 **registry owner_version + conformance suite 断言**承载（代码级常量，**不新增列**；043 仅 guard 白名单扩展，与 capability 无关）——conformance suite 断言全部 F 写者的 capability 版本与 registry snapshot 一致；版本变化 = registry digest 漂移（已有 OwnerRegistryChangedError fail closed 语义）。
3. **policy snapshot（裁决六，冻结）**：`retention_policy_snapshot` 只由 claim/rebuild 调用方在时点传入（conversation_purge_scheduler.py:194/334）——**不新增独立回填命令**；purge 侧策略以 claim 时点快照为准（S5-B/S5-C 冻结「运行中 operation 不受配置漂移影响」）；retention worker 侧策略 = 冻结默认 90/365 基线。
4. **backfill**：fence backfill CLI（agent_erasure_backfill.py，幂等/分批/tenant 限流/退出码契约）与 scope backfill CLI（agent_transport_backfill.py，B2-B7）**已有**，实现阶段只补 verify 巡检接线；无法回填行 → 具名 reconcile 队列 + 阻止（R1-AC11）。
5. **回滚（冻结）**：代码回滚只关闭 scheduler 与回退 writer 功能，**不能恢复已清正文**；schema 采用向前修复（expand-only 已就绪，无 DROP/收紧）；scheduler fail-closed 三重机制任何一层失效即视为不满足发布门禁。

##### S6-8 备份恢复门禁（冻结）

1. **恢复顺序（Spec §3 末段逐字引用）**：从旧快照恢复后**服务保持不可对外读写** → **先重放独立 erasure ledger** → **body/ref 扫描为零** → 才开放流量。
2. **账本独立保存（三面复审 P1-7 裁决，S6 新交付）**：S5-SCH-0 持久化账本（operation `agent_conversation_purges` + checkpoint `agent_conversation_purge_owners` + external ref `agent_external_object_refs` + reconcile `agent_transport_scope_reconcile`）**必须连续导出/归档到独立于 DB 备份的存储**（receipt/ack_digest 变更日志）——Spec §3「从**独立保存**的 erasure operation/receipt 账本重放」字面要求；DB 内账本停在备份快照时点，**快照之后完成的 purge 在恢复库中既留有正文又无账本记录**，body scan 永不为零（永久 fail-closed）或按账本裁剪则正文复活——**冻结：快照后 purge 处置 = fail-closed 范围（该部分 conversation 保持服务关闭）+ 人工 reconcile 门禁（runbook 步骤），不得静默放行**。重放 = 对恢复出的旧数据执行账本记录的已完成 purge 清除步骤，**不重复 adapter 调用**——已 ACK 的以账本 receipt/ack_digest 为准标记。
3. **重放执行器（三面复审 P1-8 裁决，S6 新交付）**：重放机制为 **M 类 sanctioned 维护路径**（集合锁 + 显式登记 S6-4；与 retention/audit jobs 互斥——重放期间暂停，冻结为显式声明）；**进行中 operation 处置（冻结）**：一次性 replay 执行器完成剩余 owner 的本地清除（与 purge 同谓词、**无 adapter 调用**；external/runtime 未 acked 项 → `blocked` + reconcile 记录，**不冒充已 erase**）；不依赖生产 scheduler（S6-7.1 冻结生产不可达）；**digest 失配（旧代码账本 vs 新代码重算）**：重算基准 = 账本快照自身 owner_version，失配走 runbook 人工确认（S6-6 digest conflict 处置衔接）。
...
| AC | 内容（Spec §11 引用） | 验证层级 | 证据载体 |
|----|----------------------|---------|---------|
| AC1 | 30/90/365 时钟边界/时区/DB 时钟；恢复截止不重开 | 真实 PG（DB clock 裁决）+ contract-tested（测试注入 now） | S6 retention worker 测试 + 既有 restore deadline 测试（test_restore_recovery.py）+ purge_after 全链路既有测试 |
| AC2 | registry snapshot/digest、未知 owner、版本变化、缺 capability fail closed | 真实 PG（既有）+ conformance suite（静态枚举非真实 PG + fence 判别真实 PG） | S5 既有套件 + S6 conformance suite（writer 集合==registry owner 集合，层级按 S6-4 标注） |
| AC3 | 每个正文 writer 与 purge 共用 owner lock/fence；fence check 后暂停不复活 | 真实 PG（既有 S3E/writer_fence + conformance suite fence 判别）+ 静态枚举（写者集合完整） | 既有 race 测试 + S6 writer conformance（含 binding 缺口族生产不可达标记 + stage_with_created 调用方门禁） |
| AC4 | 分步 ACK、部分失败、重试、Worker kill、lease 接管可恢复 | 真实 PG | S4-F/S5 既有 + S6-F1/F2/F4/F5 新注入 |
| AC5 | purge 后扫描为零；audit envelope 不含可恢复正文 | 真实 PG | S5 scan 既有 + S6 audit prune 前置条件断言 + orphan 巡检 |
| AC6 | payload 到期无 seq gap；prune 只删连续前缀 + 推进 first_available_event_seq；SSE 稳定 410/409 | 真实 PG | S6-F6/F7 + S6-2 判别测试 |
| AC7 | hold 与 purge CAS race、release、**expired hold**、completed 后新增 hold 均真实 PG | 真实 PG | 既有（race/release/completed 拦截）+ S6-1 裁决一 expired hold 新测试（过期不再阻塞 + 未过期仍阻塞） |
| AC8 | 不盲重试正文写 | 真实 PG | S4-F/S4-E 既有 + S6-F8/F11/F14（outbox claim crash / mutate-during-lookup / 重放拒绝） |
| AC9 | 跨 tenant/actor、无 permission hold、owner scope mismatch、伪造 ACK、operation revision 重放拒绝（Spec §11 五项） | 真实 PG（既有） | S4-F 既有（伪造 ACK/revision 重放）+ S5 transport scope 既有（owner scope mismatch）+ I1 既有（无 permission hold）+ S6-6 tenant mismatch 巡检（跨 tenant/actor） |
| AC10 | 无 CoT/secret/正文/ref 入 operation/checkpoint/日志/指标 | 真实 PG sentinel 判别（S4-F 冻结手法） | S4-F 既有 + S6 retention worker 日志/指标纳入 sentinel 判别 |
| AC11 | expand/backfill/enforce/enable 演练证明旧 Writer 在线时 scheduler fail closed；无法回填行不跳过 | 真实 PG（演练脚本，含旧 writer 变体部署 + 每阶段 fail-closed 判别点）+ 静态守卫断言（三重 fail-closed，非真实 PG）+ **基础设施 canary 真实 drill 无法本地执行 → 登记生产门禁** | S6-7 演练 + backfill CLI 既有测试 + 巡检 reconcile 断言 |
| AC12 | 旧备份恢复后重放 erasure ledger → body scan 为零 → 开放流量；未跑 drill 降级 | 真实 PG（重放机制 + 扫描）+ **drill 降级声明** | S6-8 重放测试 + runbook 文档 + 完成声明降级标注 |

**验证层级声明（冻结）**：fake Runtime/external adapter 只标 contract-tested；retention/audit/巡检全部真实 PG；真实基础设施 drill（多实例 canary、备份恢复演练）不可执行项一律登记生产门禁 + 完成声明降级，不冒充已验证。

##### S6-10 边界与停止条件确认（冻结）

- **S5 production erase 不可达边界（冻结，不修改）**：六 owner erase 入口仅允许 participant 定义文件 + `scheduler_composition.py`（sanctioned composition root，7 gate 元素 + CompositionNotReadyError，test_s5i2_production_wiring_boundary.py:41-77）；registry external/runtime 保持 False + FailClosedAdapter；S6 实现不得触碰该守卫与组合根门禁。
...
> **F-matrix M-F8 单独判别 test contract 增强 merged-boundary 收口注解（2026-09-06，pure-docs 治理收口；保留既有 frozen 顺序 + PR #608 19/20 历史口径 + PR-D 边界注解，原文不重写）**：
> F-matrix M-F8 NOT-RED（test-contract / shared-observation gap）已由 PR #610 独立后续 PR 单独闭合（squash mergeCommit `b8daa934a65491c09a6399cbb4a77235452a290c`，mergedAt `2026-09-06T13:59:40Z`；source head `785d7dbe` + FINAL_IMPL_HEAD `2cc609a2`；评分 95 Original；三面独立复审 P0=0/P1=0/P2=0/P3=1，**唯一 P3 = G-1 governance finding = implementation 期间缺活跃任务卡**，登记不关闭，closeout 阶段补登；详见 `docs/03-engineering-governance/04-retrospectives/r1-s6-i3-d-fact-audit.md` §17.11 F-matrix M-F8 单独判别 closeout 标注）：
> 1. **共享 test 拆分为 M-F2 / M-F8 独立判别载体** — 原共享测试 `test_f2_f8_dual_connection_claim_collapses_to_single_writer`（用 M-F2 `_lock_conversation` FOR UPDATE 串行化 claim() 竞争，掩盖 M-F8 `_top_operation` 缺 FOR UPDATE 的独立效应）→ 拆分为：
>    - `test_mf2_lock_conversation_serializes_dual_claim`：M-F2 独立判别（双连接 `asyncio.gather` `ConversationPurgeScheduler.claim` 同 conversation → 期望 `kinds == {ClaimKind.CLAIMED, ClaimKind.HELD}`；M-F2 注入 `_lock_conversation` 失锁 → 双 CLAIMED 单写者破缺）
...
> 5. **零生产代码改动** — 2 文件净 diff 115 insertions(+)/25(-) 仅含 pure test contract 增量 + mutation harness nodeid 重映射 + docstring 同步；无业务代码 / 无 migration 043 / 无 schema / 无 enum / 无 CHECK / 无 S5 状态机 / 无锁序 / 无写者矩阵 / 无 registry / 无 agent_erasure_registry.py / 无 CI 门禁 / 无 KNOWN_ISSUES 改动；零 review-score-log.md 历史评分行 + 零 metrics-snapshot.md 改动；zero-touch production
> 6. **保留 PR #608 19/20 历史口径** — PR #608 历史 `F-matrix 11/12 KILLED + F10 8/8 KILLED = 19/20 behavioral KILLED（不冒充 20/20）` 措辞按当时历史事实保留，本节**不**覆写 PR #608 历史口径为 20/20；PR #608 F-matrix 11/12 + F10 8/8 + PR #610 新增 M-F8 1 KILLED = **PR #610 收口后 F-matrix 12/12 KILLED + F10 8/8 KILLED = 20/20 behavioral KILLED**（仅本节新增事实，不 supersede PR #608 19/20 历史措辞）
> 7. **保留 PR-D 边界注解（2026-09-03）** 与 F-matrix test contract 增强 merged-boundary 注解（2026-09-04）原文不变；本节**不**supersede 任何既有 merged-boundary 注解 / **不**修改 S6-14 frozen 顺序 / **不**改写 plan 既有 frozen 内容
> 8. **直接调 private `_top_operation` 的取舍** — 隔离 M-F2 / M-F8 共享观察的唯一手段；公开 `claim()` 路径先经 `_lock_conversation`（M-F2 标的）会复现 PR #608 的 shared-observation 问题；该取舍已 docstring 化记录在 test 与 PR body 中；方法名 / 签名是 Plan §S6-F8 冻结契约一部分，重构导致测试失败即正确信号
> 9. **F-matrix + F10 mutation 实证（PR #610 收口后口径）** — F-matrix **12/12** KILLED（含 M-F2 / M-F8 重映射后独立真红）+ F10 8/8 KILLED = **20/20 behavioral KILLED**（M-F8 NOT-RED 已闭合，failure mode 为 `pytest.raises DID NOT RAISE Exception` 纯 assertion failure，非 crash / setup / timeout / no-tests / survived）；M-F8 关闭条件：A session 持 row-lock + B session `SET LOCAL lock_timeout` → B 锁等待 → SQLSTATE 55P03 → `pytest.raises` 命中 → assertion 内层 `lock timeout` / `55p03` 文本校验命中
> 10. **post-score Backend 首次 cancelled，单次授权 rerun PASS** — PR #610 post-score Backend 首次 `run 33852851817 job 100951517726` 35m24s 时 CI runner 外部取消（`##[error]The operation was canceled`，非 test failure，进度 46% ≈ 720/999 tests passed，0 failures / 0 errors）；获用户授权单次 `gh run rerun 33852851817 --failed` → rerun `job 101087930930` 21m36s PASS；最终三路 required checks 全 SUCCESS（Backend rerun 21m36s + Engineering docs 17s + Frontend 3m28s）
> 11. **零启动 / 零关闭 / 零重开** — PR-E（release drill）/ C1 Durable Core / S5 production wiring / registry capability flip / 六 erase 入口生产可达 / REQ-047 conformance 全部保持未启动；TD-104（PR-A schema/test alignment 残项 ⚫ 待办）/ TD-032（D1a 1724 + D1b 1052 + D2 1876 + 双 test 1117/2559 双超 1000 行硬限制 🟢 待拆分）/ TD-105（F10 实现承接 🟢 完成）/ TD-106（settlement SUCCESS 不写 ledger/binding 🟢 完成）全部保持登记不关闭不重开；G-1（implementation 期间缺活跃任务卡）作为真实 P3 governance finding 保留（closeout 阶段已在该分支先登记 TASK-R1-S6-FMATRIX-MF8-CLOSEOUT 活跃卡再跨文件修改，避免重复 G-1；不静默删除、不伪造已提前修复）；测试数据库仅 `metaedu_test`（不触碰 `metaedu`）

> **PR-E release drill 五阶段 fail-closed canary contract merged-boundary 收口注解（2026-09-07，pure-docs 治理收口；保留 S6-14 frozen 顺序 + PR #608 19/20 + PR #610 20/20 历史口径 + PR-D / F-matrix / M-F8 边界注解，原文不重写）**：
> PR-E release drill（S6-14 item 4，独立后续 PR）已由 PR #612 完成并 squash merge 入 main（squash mergeCommit `25aefc74a51cd3bcac0d17d54a5dbbcd959292cc`，mergedAt `2026-09-07T05:39:35Z`；implementation baseline main `e2814233` + FINAL_IMPL_HEAD `0a598311` + source head/score commit `536629fd`；评分 95 Original；三面独立复审 P0=0/P1=0/P2=0/P3=2 已闭环 follow-up=无；详见 `docs/03-engineering-governance/04-retrospectives/r1-s6-i3-d-fact-audit.md` §17.12 PR-E release drill merged-boundary 收口标注）：
> 1. **用户裁决 pure test harness 方案** — 不新增 thin composition / production orchestrator；PR-E 只验证五阶段 fail-closed contract，不启用真实生产 canary
> 2. **五阶段 contract drill（真实 PG + 静态守卫）** — Stage 1 expand（head=043 + append-only guard live 写 RAISE + 分支1 tombstone 正向控制）/ Stage 2 writer capability（snapshot digest 一致 + conformance 全过 + stale owner_version/digest fail closed + unknown owner + external/runtime erase 不可用）/ Stage 3 batched backfill（fence 分批游标续跑幂等 + 版本漂移 rollback 零 partial commit + transport phantom 具名 reconcile 阻断不静默跳过）/ Stage 4 verify（干净 exit 0 + event gap exit 1 不自动 resolve + indeterminate exit 2）/ Stage 5 canary enable（三重 fail-closed：registry False + 静态生产 wiring 守卫 + 组合根 CompositionNotReadyError + 旧 writer 变体注入 + M-class shared↔exclusive 双向 lock_timeout 55P03 + 多 replay 串行）
> 3. **restore-before-open 顺序（§S6-8，contract-tested 级）** — blocking replay report / runtime_proof_c_present 强制 closed；replay 干净 + 六 owner scan 零 + S6-6 巡检零 → open_allowed=True
> 4. **mutation 等价判别探针（不改生产代码）** — P1 guard 触发器禁用 / P2 锁 key 失配 / P3 版本匹配对照 / P4 tenant 谓词；附证 reconcile.tenant_id FK `fk_agent_transport_reconcile_tenant` 存在
> 5. **零生产代码改动** — 净 diff 2 文件 786 insertions(+)/2(-) 仅含 pure test contract 增量 + 工作台活跃卡；无业务代码 / 无 migration / 无 schema / 无 enum / 无 CHECK / 无 S5 状态机 / 无锁序 / 无写者矩阵 / 无 registry / 无 CI 门禁 / 无 KNOWN_ISSUES 改动；零 review-score-log.md 历史评分行 + 零 metrics-snapshot.md 改动；zero-touch production
> 6. **PR-E 完成性质（冻结声明）** — PR-E = **production-neutral contract-tested test harness**，验证五阶段 fail-closed contract 在真实 PG / 静态守卫层成立；**不是**生产 release enable、**不是**真实生产 release drill 完成；「canary enable」仅断言测试环境 tenant 级 fail-closed gate 行为
> 7. **生产门禁保持登记（未执行，不冒充已验证）** — 真实 pg_dump / restore / 流量切换、多实例滚动 canary、旧 writer 真实进程部署——本仓库无该基础设施（§S6-8.6 / R1-AC12 字面降级），由生产基础设施负责人承接；restore-before-open 断言为 contract-tested 级别
> 8. **保留历史口径** — PR-D（2026-09-03）/ F-matrix test contract 增强（2026-09-04）/ M-F8 单独判别（2026-09-06）边界注解原文不变；PR #608 19/20 + PR #610 20/20 历史口径保留；本节**不**supersede 任何既有 merged-boundary 注解 / **不**修改 S6-14 frozen 顺序 / **不**改写 plan 既有 frozen 内容
> 9. **runbook §6.4 历史漂移如实保留** — runbook 称 `restore_replay_executor`「pending in `S6I2_PENDING_WRITERS`」，代码现状已 registered=FENCE_M（`S6I2_PENDING_WRITERS=()` 空）；本注解**不**修改 runbook 历史事实源，漂移如实保留不冒充已消除
> 10. **零启动 / 零关闭 / 零重开** — C1 Durable Core / S5 production wiring / D1b/D2 production wiring / registry capability flip（external/runtime 保持 `erase_available=False`）/ 六 erase 入口生产可达 / REQ-047 conformance 全部保持未启动；TD-104（⚫ 待办）/ TD-032（🟢 待拆分）/ TD-105（🟢 完成）/ TD-106（🟢 完成）全部保持登记不关闭不重开；TASK-R1-S6-I3-D 整体仍 🟡 进行中（PR-E 完成 ≠ 整个 TASK 完成）；测试数据库仅 `metaedu_test`（不触碰 `metaedu`）
...
> Status: Frozen — contract correction merged to main + **TD-106 已完成并关闭（2026-08-26）**（**方案 A 已经用户批准（2026-08-25）**；**PR #591 已 squash merge 入 main，mergeCommit `738be6f9`（2026-08-25T08:37:30Z），正式评分 89（Original）落入 Score Log；评审对象 main@`b28f84ab`..`5901e6d7`**；§S6-15 整段含 §S6-15.1/15.2/15.3/15.4/15.5（事实核验 + Phase 1 六场景 + 方案 A/B 决策材料 + 方案 A 裁决 + 实现证据块）已入 main；实现由独立 stacked PR #592 承载（base = #590 分支）已 squash 入 #590 parent（mergeCommit `e345c429`，评分 92 Original 已落入 Score Log），#590 已 squash 入 #586 root（mergeCommit `ee53b172`，评分 94 Original 已落入 Score Log），**#586 root 已 squash merge 入 main（2026-08-26T08:35:12Z，mergeCommit `68fafd81de5b0925be44e935c532cdce898e2935`，source head `cca892b54b39836cc5e98a5e84e3028ba1ddea04`，FINAL_IMPL_HEAD `1a61ba6f`，评分 94 Original 已落入 Score Log，评审对象 main@`ddcb87ca`..`1a61ba6f`，`scripts/check-review-score-submit --base 1a61ba6f --pr 586` PASS，Metrics byte-identical，Ready 三路 required checks 全 SUCCESS）**——TD-106 已闭合；不改代码/schema/migration/enum/CHECK/registry/CI；migration 043 零改动）
...
**#586 bounded integration merged-boundary（2026-08-26，TD-106 已完成并关闭）**：S6-F1..F14 故障矩阵 + TD-106 方案 A bounded integration 已 squash merge 入 main（PR #586，mergeCommit `68fafd81de5b0925be44e935c532cdce898e2935`，source head `cca892b54b39836cc5e98a5e84e3028ba1ddea04`，FINAL_IMPL_HEAD `1a61ba6f`，评分 94 Original 已落入 Score Log）。

- **集成事实链**：`PR #589` (R1-S6-I3-A schema/test alignment bounded repair, squash 入 #586 mergeCommit `f6062466`, 评分 92 Original) + `PR #590` (R1-S6-I3-C S6-F1..F14 故障矩阵 + TD-106 方案 A 实现, squash 入 #586 mergeCommit `ee53b172`, 评分 94 Original; source head `cd5a437c`, FINAL_IMPL_HEAD `d3a12549`) + `PR #592` (TD-106 方案 A 实现, squash 入 #590 mergeCommit `e345c429`, 评分 92 Original) → `PR #586` (squash merge 入 main `68fafd81`, source head `cca892b54b39836cc5e98a5e84e3028ba1ddea04`, FINAL_IMPL_HEAD `1a61ba6f`, 评分 94 Original)。
- **评审对象**（#586 root 三面复审）：`ddcb87ca..1a61ba6f` (main → PR head)，净 diff 19 文件：3 个治理文档（current-work + work-log + td-032 baseline）+ 3 个生产代码（`external_ref_erasure_participant.py` / `runtime_erasure_participant.py` / `settlement.py`）+ 12 个测试与脚手架（`test_s6i3_fault_matrix.py` 306 / `test_s6i3_fault_hold.py` 218 / `test_s6i3_fault_external.py` 358 / `test_s6i3_fault_events.py` 480 / `test_s6i3_fault_scheduler.py` 191 / `test_s6i3_fault_mutation_evidence.py` 71 + `s6i3_seeds.py` 155 + `test_s6_td106_lock_and_cas.py` 389 + `test_s6_td106_settlement_ledger.py` 913 + `scripts/s6_td106_settlement_ledger_mutation_kill.py` 291 + `scripts/s6i3_fault_matrix_mutation_kill.py` 216 + `test_s5_sch_d_settlement_key_alignment.py` 14)。
- **三面复审（#586 root, head=`1a61ba6f`）**：面 A 数据/状态/scope + 面 B 并发/锁序/恢复 + 面 C 测试/mutation/ops/docs → 三面 P0=0/P1=0/P2=0/P3=3。P3 三项观察不修复：(P3-A1) `reconcile_runtime_binding` (runtime_erasure_participant.py:1135) 独立 `status='closed'` UPDATE 不委托模块级 `write_erased_and_close_binding`——预存于 main 非 TD-106 引入（git diff main..HEAD 显示该方法未被改动），B2/E-5-2 契约范围限于 Tx2+settlement ledger 共用函数（已落地），E-3b reconcile 路径不在契约范围；(P3-A2) `settlement.py` 1461 行超 td-032 1000 硬限制——已登记拆分；(P3-A3) scope 收敛 commit `9eef73cd` + governance commit `1a61ba6f` 同根集合（撤回 5 文件 scaffold + 1 内联常量 + 1 测试迁入）。
- **正式评分 94 Original（#586, head=`1a61ba6f`）**：维度 = 范围与需求匹配 15/15 + 实现质量 17/20 + 测试与验证证据 19/20 + 事实源与流程遵守 15/15 + 风险与行为变化控制 15/15 + 可评审性与交接质量 9/10 + 持续改进信号 4/5 = **94**。`scripts/check-review-score-submit --base 1a61ba6f --pr 586` PASS（one Original row, Metrics byte-identical）。
- **TD-106 状态（2026-08-26）**：**🟢 已完成并关闭**——bounded integration 已并入 main，S5-C-1 态 1 落账列闭合；20 项 TD-106 专项 + composition 763 passed/1 skipped + 9/9 mutation 真红 + 9/12 F-matrix mutation 真红 + P1-A/P1-B CLOSED + P2 三项保留未关。migration 043 / schema / enum / CHECK / CI / 门禁脚本 / KNOWN_ISSUES 零改动。
- **scope 收敛（2026-08-26, commit `9eef73cd`）**：撤回 PR-D/E scaffold（`s6i3_ledger_export.py` / `s6i3_restore_replay.py` / `s6i3_release_drill.py` / `restore-before-open.md` / `test_s6i3_fault_matrix_restore_replay.py` 18 项 PR-D/E 测试）——**本 root = F1-F14 故障矩阵 + TD-106 方案 A bounded integration；PR-D/E 拆分独立后续 PR，本 root 不含**。
- **整体仍未完成（不属于本 PR 范围）**：F10 仍 skip（`test_f10_contract_conflict_not_implemented`，TD-105 承接——前置 TD-106 现已完成，可启动 F10 实现 PR）；#590 mutation 9/12 真红 + M-F3/M-F5/M-F8 mutation 停止条件登记保持；PR-D（ledger export executor + restore-before-open runbook）/ PR-E（release drill 真实 canary）/ C1 Durable Core 总验收 / S5 production wiring / registry capability 翻转（external/runtime 保持 `erase_available=False`）/ 六 erase 入口生产可达 均未启动；TD-104（PR-A schema/test alignment 残项）+ REQ-047（R1-S6 implementation conformance 联合闭环）保持后续承接。
- **后续收口顺序（已完成，2026-08-26）**：(1) #590 squash merge → #586 ✅（mergeCommit `ee53b172`）；(2) #586 root 三面复审 ✅ + Ready ✅ + 评分 94 Original ✅；(3) #586 squash merge → main ✅（mergeCommit `68fafd81`）；(4) main closeout ✅（PR closeout squash merge → main 后置 closeout commit）；(5) TD-106 进入 main 后已标记完成 ✅（🟢）。
...
**边界（已变更，2026-08-26 #586 merge 落地后）**：本段不产生任何 schema/migration/enum/CHECK 变更需求；不修改 S5 settlement/participant/terminal guard；settlement T2 现行行为已满足 F10 主体（settlement 落账层，§S6-15.3）；读法甲前 4 环维持确认；**TD-106 已完成并关闭**（bounded integration #586 `68fafd81` 已 squash merge 入 main）；PR-D/PR-E/C1/S5 production wiring/registry capability 翻转均未启动；不新增 Score Log 行（仅 #586 Original 一行）、不修改 Metrics。follow-up **TD-105**（F10 实现承接——前置 TD-106 现已完成，可启动 F10 实现 PR 解除 `test_f10_contract_conflict_not_implemented` skip）+ **REQ-047**（R1-S6 implementation conformance 联合闭环）；TD-106 P2 三项保留未关（runtime per-binding receipt 形参零使用 / settlement↔participant 跨入口双写者并发用例 / M4 receipt 复用深度）。

**merged-boundary（2026-08-26，PR #596 F10 真实 PG 判别 + mutation kill + closeout 入 main）**：F10 真实 PG 判别经 PR #596 承接并入 main（mergeCommit `c0ec008d4299e8a38e4b7f049146a14eb2b87cf8`，source head `f5cb0b34` / implementation HEAD `7cc732ba`，评审对象 `695fed8e..7cc732ba`，净 diff 3 文件 1005+/23-：`packages/server-python/tests/composition/test_s6i3_fault_f10.py` 新增 732 行（8 项真实 PG 测试：test_f10_t1_then_advance_hold_t2_completes / test_f10_t2_unidirectional_advance_passes / test_f10_t2_unidirectional_regression_fails_closed / test_f10_projection_g2_blocked_hold_revision_changed_no_completed / test_f10_rebuild_hold_gated_with_active_hold / test_f10_rebuild_unblocks_after_hold_release_bumps_revision / test_f10_rebuild_unblocks_after_hold_expire_no_bump / test_f10_no_body_resurrection_no_repeated_adapter_replay_idempotent）+ `test_s6i3_fault_hold.py` F10 skip 占位迁出（F9 保留 + F10 迁出至新文件）+ `scripts/s6i3_f10_mutation_kill.py` 新增 269 行（8 项具名 mutation：M1-M5 + M7/M8 真红 + M6 NOT-RED 如实登记））。**F10 routing 四环 + TD-106 不变式保持**——settlement T2 单向 hold 检查（advance 放行 / regression fail-closed）+ fence `erasing→erased` + checkpoint `erasing→acked` + TD-106 per-ref receipt + B2 唯一清除者命中源行 + D8 集合锁 + G2 `blocked_hold_revision_changed` + rebuild G3 HOLD_GATED + release bump / expire no-bump 双路径 + 三联不变式（零正文复活 / 零重复 adapter / 重放幂等）全部验证；M6 completed 绕过最终扫描 NOT-RED（如实登记为 test contract 增强 follow-up——F10 链经 G2 在 priority 2 提前 return blocked_hold_revision_changed，永不到达 priority 3 scan check，判别载体与 F10 测试集正交）；F10 不直接宣称原 operation completed（completed 仍依赖后续 rebuild、新 operation 和最终扫描）。**TD-105 已完成并关闭**——PR #596 评分 97 Original（implementation baseline main `695fed8e` / FINAL_IMPL_HEAD `7cc732ba`，`scripts/check-review-score-submit --base 7cc732ba --pr 596` PASS，Metrics byte-identical）；Ready 三路 required checks 全 SUCCESS（Backend full 15m0s + Engineering docs 18s + Frontend 2m45s）；本 closeout 入 main 后 **TD-105 状态翻 🟢 完成**；PR-D/PR-E/C1/S5 production wiring/registry capability 翻转/六 erase 入口生产可达 仍均未启动；F-matrix 7/12 mutation NOT-RED + M-F3/F5/F8 mutation 停止条件登记 继续保持；M6 NOT-RED 登记为后续独立 test contract 增强 PR；TD-106 P2 三项保留未关（runtime per-binding receipt 形参零使用 / settlement↔participant 跨入口双写者并发用例 / M4 receipt 复用深度）。
...
> **R1-S5 B/C/D root integration merged-boundary（2026-08-19，root PR #577，squash merge `636fc425`，评分 92）**：R1-S5 Scheduler B/C/D root integration 并入 main——SCH-A Claim & Lease（migration 042 + `ConversationPurgeScheduler`，child PR #575 已入 main `36d091a4`）+ SCH-B Owner Execution（`OwnerExecutionOrchestrator`：owner 字典序循环 + 周期 token 重验 + SettlementPort + tick + retry 预算 + checkpoint.attempt 写者矩阵回填）+ SCH-C Rebuild & Seeding（`PurgeRebuildService`：G1/G2→quiesce→rebuild/seeding 单事务原子 + G3 hold 门禁 + lineage，child PR #578 `a8f4d561`）+ SCH-D Settlement（`SettlementService`：六输出态 + 裁决二两阶段——T1 锁内读 → 锁外 adapter I/O → T2 全 token 重验 + CAS 落账，child PR #579 `5033efc5`）+ settlement idempotency key 对齐（逐 ref/binding 以 frozen descriptor 派生 `external_erase_idempotency_key`/`runtime_destroy_idempotency_key`，E-2a 冻结 intent 重验）+ `scheduler_composition.py` 联合组合根（六 owner participant map + concrete SettlementPort + rebuild + coordinator + claim/lease；partial wiring `CompositionNotReadyError` fail closed）。验证：SCH 全套 126 专项 + fresh 隔离 PG composition 665 passed + mutation 8 组 + CI Backend full 2600/1/4 + Frontend 326 unit + 55 E2E + ruff/mypy/docs gates。**merged-boundary 结论**：B/C/D 联合门禁关闭——但**生产 Scheduler 未启用**：六 owner erase 入口生产不可达（`scheduler_composition.py` 无生产调用方 + `test_s5i2_production_wiring_boundary` 静态守卫）、external/runtime registry 保持 `erase_available=False` + FailClosed 槽位、无 production adapter、无后台循环/HTTP/CLI/API。遗留明确排除：S6（retention clocks / 真实故障矩阵）、C1 总验收、生产 erase wiring、REQ-047 conformance follow-up（B/C/D 联合启用门禁核验、participant 三键收窄、settlement wiring 前 idempotency key 对齐——已由本 PR 落地）、TD-093/095/096、td-032（settlement.py 1280 行待拆分）。
...
> 本段冻结 migration 042（Lease Carrier）、租约状态机与统一 CAS 谓词、分阶段门禁边界与 8 项新增反例（SCH-9..16，全归 SCH-A）。首轮三面复审原始计数（保留不覆盖）：数据/状态机 P0=0/P1=0/P2=3/P3=6 + 并发/锁序 P0=0/P1=1/P2=7/P3=6 + 测试/运维/文档 P0=0/P1=1/P2=1/P3=4 → 合计 P0=0/P1=2/P2=11/P3=16，按族 A~I 统一返修一次（本版）。
...
- `updated_at` 时钟源混合：模型 default/`onupdate`（未显式设值时）走应用时钟 `_utcnow`，而 coordinator/restore-cancel 显式写 DB clock（transactional_projection_coordinator.py:483 / erasure_repository.py:870，锁后 `clock_timestamp()`）——同一列多写者、多时钟语义，进一步排除其作租约事实源的可能。
...
- **expand-only 加列**：`ALTER TABLE metaedu.agent_conversation_purges ADD COLUMN lease_expires_at TIMESTAMPTZ NULL`（命名先例 = agent_execution 域 `stream_lease_expires_at`，nullable timestamptz 租约截止）。nullable——`NULL` = 未认领（初始/释放态），既有行天然合法；**不新增 CHECK**——任何 state 相关 CHECK（如「终态 ⇒ NULL」）会误杀「coordinator 终态写 → scheduler 终态观察 release」之间的合法非 NULL 窗口；纯 epoch 不变量 `lease_expires_at IS NOT NULL ⇒ lease_epoch >= 1`（S5-SCH-8）虽可安全落 CHECK（单语句原子写、无窗口），本段选择保持 042 最小列形态、以 SCH-16 反例兜底。
- **partial index**：`ix_agent_purge_lease_active ON metaedu.agent_conversation_purges (tenant_id, lease_expires_at) WHERE lease_expires_at IS NOT NULL AND state NOT IN ('completed','cancelled')`——服务 tenant 并发上限计数谓词（S5-SCH-8）；claim 短事务不得退化为全表 scan（operation 表随 purge_revision 历史增长）。042 落地时无并发写者（scheduler 未启用、erase 入口不可达），普通 `CREATE INDEX` 可接受、`CONCURRENTLY` 非必需；既有行全 NULL 仍需一次性全表扫描，超大表迁移时长由 SCH-A 实现 PR 评估。
...
- `lease_expires_at > clock_timestamp()` → 在租（current-epoch 可 renew——无 `claimant_id` 设计下持新鲜 epoch 读的调度器即可续约，靠 epoch 推进 fence 旧 token、无双写；不可 claim/takeover）；
- `lease_expires_at <= clock_timestamp()` → 已过期（可 claim/takeover；旧持有者 renew 被拒）。

**全部转移 = Conversation-first 短事务 + 行锁内「expected epoch CAS」**：预期不匹配 → 零写退避；成功一律 `lease_epoch = expected + 1`。S5-SCH-1.1 冻结的 lease 上界（默认 10 分钟、大于单 owner 最大执行时长、settlement_deadline ≤ 同值）不变，本段为其提供可判定载体：`lease_expires_at = clock_timestamp() + lease_ttl`（ttl 由该上界派生）。**到期判定表达式必须落在 UPDATE 语句的 WHERE 子句（语句级 `clock_timestamp()` 求值）**；SELECT 仅取行锁与 expected epoch，不得应用侧先判到期再 UPDATE（微秒级滑越 = 「过期后仍续约」，与 SCH-10 相悖）。
...
| release/yield/终态观察 | `lease_epoch = expected` 且 `lease_expires_at IS NOT NULL`（过期旧持有者释放仍合法——过期行 expiry 非 NULL；已被 takeover 则 epoch 已推进 → CAS 拒 → 零写；NULL 态视为已释放 → 零写成功返回，防 fresh/已释放行冗余 epoch+1） | epoch+1；`lease_expires_at = NULL` |
...
- **编排方内存 token 同步义务（冻结）**：acquire/takeover/renew 成功后编排方内存 token 同步为 `expected+1`；所有 participant entry 与 1.3(a) 重验的 `expected_lease_epoch` 一律取最新 token——漏同步则后续 entry 全被 fencing 拒 → fail closed 重入 claim → 撞自己的在租租约 → ≤TTL 空转自愈。
...
- **rebuild 后新 operation 租约获取（冻结；堵 S5-SCH-2 (d) × S5-SCH-9 (i) 接缝）**：rebuild 单事务**并入 lease acquire**——seeding 提交前对新 operation 执行 acquire CAS（NULL → epoch+1、写 expiry），回滚则租约同回滚零残留；S5-SCH-2 (d) 写事务清单据此增补 lease acquire 项。提交即持租约，无「未认领新行被执行 / 被另一 scheduler 并发 claim」窗口。**禁止**不持租约直接继续 owner entry（S5-SCH-9 (i) 为兜底）。
- 语义结论（对齐开工核对裁决）：**expired/NULL 才可 claim；未到期租约不可接管；败者零写**。claim 谓词「无在租 claim」（S5-SCH-1.1）正式化 = 该 operation `lease_expires_at IS NULL OR lease_expires_at <= clock_timestamp()`；「claim 后崩溃 → lease 到期 takeover」（S5-SCH-2 崩溃恢复分支 1）现由 `lease_expires_at` 可判定。**completed/cancelled 行由 conversation 级谓词先排除**（completed ⇒ coordinator 已写 `purged_at`；cancelled ⇒ restore 已改 `state`）——operation 级谓词不做终态特判，**终态行禁止 takeover**，唯一收尾 = 终态观察 release；failed 行在租 = 合法占用，等待 ≤TTL 属冻结恢复语义（S5-SCH-2 分支 1）。
- **tenant 并发上限计数谓词（冻结）**：只统计 `state NOT IN ('completed','cancelled') AND lease_expires_at IS NOT NULL AND lease_expires_at > clock_timestamp()` 的 operation——未认领（NULL）与已过期行**不占 slot**（否则 NULL 堆积锁死 claim、过期未接管行锁死 takeover，liveness 洞）。上限 4（S5-SCH-1.1）不变。**上限 4 为 advisory 目标值（冻结）**：各 claim 只持自己 conversation 的锁，跨 conversation 计数为快照读，并发 claim 可在短窗口瞬时越过，随后续租约到期/release 收敛回上限内——**不引入 tenant 级锁**（避免新增 S1 锁层级）；单 claim 自身必须「计数与 acquire 同事务、计数在前、count ≥ 4 即拒」。crashed-claim 在 TTL 内计入 slot（在租 = 合法占用）；最坏 4 个 crashed claim 阻塞同 tenant 新 claim ≤TTL 属冻结恢复语义（SCH-5 分支 1 测试需时钟注入处理 10 分钟窗口）。
- **写者所有权矩阵纠偏行**（**本行 supersede S5-SCH-2 矩阵 :2187 行**，矩阵其余行不变；lease 转移术语正式化为 acquire/takeover/renew/release，release 为本纠偏新增转移，:2187 的「claim/takeover/续期」不再单独引用）：`operation.lease_epoch` + `operation.lease_expires_at` | 唯一写者 = scheduler（acquire/takeover/renew/release CAS） | 单调由 CAS 保证；其余写者只读重验（`lease_expires_at` 其余写者不读不写）。
- **租约 CAS 事务的 operation 锁查询一律三键（tenant_id+id+conversation_id）**，不得复用现存两键 `get_purge_operation_for_update`（erasure_repository.py:795-807，S5-SCH-5 尚未收窄形态）；renew 心跳的 Conversation 首锁不可省（S1 不变量字面要求，lease 列独占不构成省锁理由）。
- 实现建议（非强制）：ttl 取值给 settlement_deadline 留严格小于余量，避免 deadline 到达瞬间与租约到期边界竞争。
...
- **owner entry 门禁（冻结给 SCH-B 接线）**：B/C/D 联合启用前，owner entry 必须在 **Conversation 行锁内**（S1 不变量统一锁序第一步，与 entry 自身事务同一锁上下文；「锁内」不指 operation 行锁独检）拒绝 (i) `lease_epoch = 0`（未认领——scheduler 未 claim 即进入 owner entry 属接线故障）；(ii) expired lease（`lease_expires_at <= clock_timestamp()`；当前实例不持有有效租约）；(iii) 旧 epoch（token 不匹配）。(i)/(iii) 是 S5-SCH-1.3(a) 周期 token 重验中 `lease_epoch` 检查的显式实例（实现合并为一次检查，不得双规则文本漂移）；(ii) 为 1.3(a) 的**增量项**——1.3(a) 只比 epoch 不比 expiry，无法捕获「token 匹配但时钟越过 expiry」。**本段扩展 1.3(a) 重验元组：`(revision, lease_epoch, purge_revision)` → `(revision, lease_epoch, lease_expires_at, purge_revision)`**（supersede 注：plan :2151 元组以本段为准）。三者任一 → fail closed 重入 claim 判定，不写任何行；renew 失败 → 门禁 (ii) → 重入 claim（takeover）。
- **五份 participant operation 锁查询三键收窄承接决策由本段预决为独立 REQ-047 conformance PR**（S5-SCH-5 原允许 SCH-A 承接的选项不再保留；td-032 测试文件拆分随该项一并执行；本契约不顺手改代码/测试）。
- #571 评分 87、Score Log、Metrics Snapshot 不受本纠偏影响（不重评）。

#### S5-SCH-10 反例矩阵增补（冻结；S5-D 反例矩阵 53 → 61）

新增 8 项全归 SCH-A，复用 S4-F 冻结注入机制（真实 PostgreSQL、双连接 `asyncio.gather`、DB 篡改、崩溃注入；租约反例优先用具名机制 4「手动推进 lease_epoch/attempt/revision 模拟 takeover」与机制 7「跨 tenant/跨 Conversation」）；每项具名 mutation，SCH-A 实现 PR 逐项落地：

| # | 反例 | 触发 | 期望行为 | 判别点（mutation） |
|---|------|------|---------|-------------------|
| SCH-9 | 通用 `updated_at` 变化不得续租 | 非租约写者（coordinator 投影 / restore-cancel）更新 operation 行（`updated_at` 被显式赋值或 onupdate 刷新） | `lease_expires_at` 不变、租约不续期；租约逻辑不读 `updated_at` | 「以 updated_at 判定租约」→红 |
...
| SCH-13 | terminal/expired 不占 tenant slot | 一 operation 已 completed（或 cancelled）且 expiry 未清；另一 lease 已过期 | tenant 计数（上限 4）不含终态与过期行；终态行不可被 takeover；终态观察 release 可收尾 | 「计数含终态/过期行」或「takeover 终态行」→红 |
| SCH-14 | migration 042 往返 | alembic upgrade → downgrade → upgrade；既有 operation 行 | 往返无损；既有行全 NULL = 未认领；无伪造历史租约 | 「backfill/非空默认」→红 |
| SCH-15 | rebuild 后未 claim 即 entry | rebuild 提交后直接 owner entry | S5-SCH-9 门禁 (i) 拒 epoch 0 → 重入 claim acquire 后可判；rebuild 单事务已并入 acquire 则无此路径 | 「rebuild 后跳过 acquire」→红 |
| SCH-16 | 写 expiry 不推进 epoch | acquire/takeover/renew 任一转移动手写 `lease_expires_at` 不推进 epoch | 不变量 `lease_expires_at IS NOT NULL ⇒ lease_epoch >= 1` 恒成立；epoch 0 行不可被 renew 谓词放行（fencing 不破） | 「acquire 写 expiry 不推进 epoch」→红 |
