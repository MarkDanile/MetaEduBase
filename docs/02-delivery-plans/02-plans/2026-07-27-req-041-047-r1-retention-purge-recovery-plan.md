...
- **全量 flake 登记 TD-080（独立处理，不混入 S3-E）**：干净 main（无 S3-E）全量亦多失败（含 test_ai_chat / direct_rag / e2e / turn_bridge / writer_fence 非确定性隔离 flake）。登记内容：clean-main SHA、完整命令、失败签名集合、重复运行分布、疑似共享数据库/全局状态污染、临时隔离措施、完成标准「连续 3 次全量 hermetic 回归无重跑通过」。
