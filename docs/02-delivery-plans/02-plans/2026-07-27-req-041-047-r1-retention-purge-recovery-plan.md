...
- **D2 producer epoch 用 `Conversation.purge_revision`**（**不变量 2**）：新事件在产生同事务快照当时 `Conversation.purge_revision` 写入 `producer_purge_revision`；**不得**用 fence 行 CAS `revision` 冒充。消费/清除阶段 old/missing producer revision -> 只能 tombstone/reconcile，不得复活正文。
- **D3 历史不确定行 -> 具名 reconcile 阻塞 purge（不变量 3）**：backfill 无法可靠回填 owner scope 的行（如 conversation 已物理缺失、aggregate 无法映射唯一 conversation）进入具名 reconcile 状态，**阻止对应 Conversation purge**（fail closed），不得静默丢弃或猜测。
- **D4 suppressed/cancelled envelope tombstone（不变量 4）**：transport 清除 = 清 `payload_inline`/`payload_ref`，**保留** `payload_digest`；禁止 `{}`、空串、伪 ref。与 S1 `ck_agent_*_outbox_payload` 的 `suppressed` 分支一致（清正文保留 digest）。receipt 仅 digest 无正文，清除语义 = 标记 tombstone（无正文可清）。
- **D5 external ref 状态机与 ledger**：`RunEvent.payload_ref`（execution outbox `payload_ref` 同）是 external object 引用。新增 external ref ledger（external.payload.v1 owner）记录 ref/scheme/state；erase 语义：`known DB-local ref` 可实装删除并 ACK；`unknown scheme` / `erase outcome unknown` / `timeout` / `digest mismatch` 一律 **blocked（不变量 5），不得 ACK**。S3-D 已在 execution.core.v1 把现存 `payload_ref`/`runtime_session_ref` 判 `purge_owner_unavailable` blocked 移交 S4。
