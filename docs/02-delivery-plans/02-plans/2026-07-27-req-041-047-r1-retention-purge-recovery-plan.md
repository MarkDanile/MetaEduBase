...
> **round-3 独立复核修订（2026-08-06，P0/P1/P2/P3=0/3/4/0）**：三面计数修正为 `3/12/11/4`（round-1 注记原写 3/9/12/4，污染 TD-092 收敛指标，已改）。三个 P1 为实现前必须冻结的事务/身份协议，round-3 已一并修正（优先于 R1/R2/R4 对应旧陈述）：**S1** turn 路径补 outbox 行 scope 校验（三源，修正 R2 数据事实）；**S2** unknown/stale 的证据提交 + outbox 终态化冻结双事务协议（修正 R4 遗漏的事务边界）；**S3** orphan 分支改为仅 catch-up/discovery 可达（consumer 先锁 Conversation 后 orphan 不可达）。P2 四项一并修（R1 workspace 锁事实、PR HEAD 稳定表述、实现 PR 冻结为两 PR 拆分）。round-4 复审只需一次轻量 diff 核对。
...
**R 复核提示（round-1 修订落点，round-3 复审重点）**：R1 epoch 快照同事务行锁（workspace 复用既有锁、不重复取）；R2 claim 装载原子性 + 非 NULL 成员比对 + **turn/output 均三源**（round-3 S1）；R3 consumer 集合锁目标/owner_key + 先集合锁后 inbox 行；R4 stale/unknown 确定性终态 + tombstone-resolve 双相 + **双事务协议**（round-3 S2）+ fence 状态 gating；R5 catch-up 全谓词 + verify 不用守卫 + C6 两新行 + C8 编号矩阵；R6 措辞；**S3** orphan 仅 catch-up/discovery 可达（round-3）。
...
**unknown/stale 的双事务协议（round-3 S2，冻结）**：当前 dispatcher 在消费异常时回滚消费事务、再走独立失败/terminalize 事务（`agent_control_plane.py:575` 消费事务、`:592` ACK 事务、`:630` 独立 terminalize），消费事务内提交的 tombstone/ledger 会被回滚。**冻结为**——(1) 消费事务返回**具名 deterministic outcome**（如 `ConsumeOutcome.reconcile_epoch_unknown`/`stale_epoch_tombstoned`），在消费事务**内提交** inbox `receipt_tombstone_state='redacted'` + `receipt_tombstone_digest`（B1f）与 `epoch_unresolvable` ledger issue（集合锁临界区内，同事务），消费事务**正常提交**（不 raise、不回滚）；(2) **第二独立事务**按 claim CAS（`event_id`/`payload_digest`/`attempt_count`/`claimant_id`）把 outbox 行置确定性终态并清 claim——workspace turn outbox 终态 + `decision_reason`/`decision_digest`/`decided_at` 字段随 S4-C 落地定义（镜像 `terminalize_output_late_write` 的 `cancelled` + `decision_reason` 语义）；(3) **第二步失败/崩溃后重放**：重放先识别**已提交的 evidence**（inbox receipt tombstone + ledger `epoch_unresolvable` 已存在），直接继续完成第二步 outbox 终态化（幂等），**不重复写证据、不回到 retry**。
...
7. consumer 写 inbox metadata 与 receipt 同事务；unknown/missing epoch 集合锁临界区 tombstone-resolve 双相（先证据后 `epoch_unresolvable`，R4）+ **双事务协议**（round-3 S2：消费事务返回具名 outcome 提交证据、第二事务按 claim CAS 终态化 outbox、重放识别已提交证据幂等续做）。
...
- **实现 PR（round-3 冻结为至少两 PR）**：S4-C 已同时涉及 producer（outbox 写 scope/epoch）、claim/consumer（envelope + 六元 CAS + inbox）、deterministic terminalization（stale/unknown 双事务协议）与 catch-up 四个风险域，超单风险域——**不再「范围过大时」才拆，冻结为至少两个实现 PR**（round-3 P2 修订）：
  1. **Producer propagation + replay/catch-up**：writer 写 scope/epoch（R1/C1/C2）+ 幂等重放 + S4-B catch-up（C5）。验证 C8 项 1/4/5/9。
  2. **Claim/consumer CAS + deterministic terminalization**：envelope 扩展 + 六元 CAS（C3）+ inbox 写 + stale/unknown 双事务协议（round-3 S2）+ consumer 集合锁（R3）。验证 C8 项 2/6/7/8/10。
