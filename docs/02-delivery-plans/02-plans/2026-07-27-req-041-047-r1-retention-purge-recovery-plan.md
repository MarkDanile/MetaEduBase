...
- **多 issue -> 单值投影聚合规则（复核 #5）+ 源行级聚合锁（复核 #1，防集合并发漂移）**：同一源行可有多条 issue 行；行内 `scope_reconcile_state` 投影聚合规则——任一 issue `state <> 'resolved'` 时投影 `'pending'`；全部 issue `resolved` 后才投影 `'reconciled'`；`orphan` 类 issue 存在时投影 `'orphan'`（**优先级最高**，即便其他 issue 未 resolved 也标 orphan，因 Conversation 已删、scope 已无对象可回填）。**并发正确性（复核 #1）**：单条 issue 的 `(id, revision)` CAS 只保护**单条**状态机迁移，保护不了「同一源行的 issue 集合」——并发事务可插入新 issue（不改既有行 revision）导致基于过期集合的投影覆盖（lost-update/skew）。故投影聚合须在**同事务**内先 `SELECT ... FROM agent_transport_scope_reconcile WHERE (tenant_id, owner_key, source_table, source_row_id) = ? FOR UPDATE` **锁定该源行的全部 issue 行**（行锁序列化该源行的聚合），据此集合重算并写行内投影；**新 issue 一律 `INSERT ... ON CONFLICT DO NOTHING`**（唯一键兜底幂等），插入成功后**同事务按当前完整集合重算投影**——新 issue 的 INSERT 会被并发事务持有的 `FOR UPDATE` 行锁阻塞（unique 冲突等待）或插入后触发重算，二者都保证不会用漏掉新 issue 的旧集合覆盖投影。单条 `revision` CAS 仍用于单 issue 的 `open->acknowledged->resolved` 迁移防并发漂移；集合级正确性由源行 `FOR UPDATE` 聚合锁保证（或等价的 `SERIALIZABLE` + 重试，二者取一并冻结为实现约束，禁止只用单条 revision 保护集合）。
