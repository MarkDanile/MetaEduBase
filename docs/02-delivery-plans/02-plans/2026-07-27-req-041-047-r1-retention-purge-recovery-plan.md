...
> | 4 | 超时：可能已生效 -> `unknown`（不自动重试） | S4-E E-3a | `test_*_timeout_marks_unknown`、`test_destroy_timeout_marks_unknown` | 消费证据 |
> | 5 | 超时：可证明未发送 -> `blocked`/`erase_timeout` | S4-E E-3a | `test_*_not_sent_blocks_erase_timeout` | 消费证据 |
> | 6 | `unknown` outcome 禁自动重试 + reconcile 需 evidence | S4-E E-3b | `test_query_blocked_unknown_*`、`test_reconcile_requires_receipt_lookup_capability` | 消费证据 + 互操作确认 |
> | 7 | stale lease_epoch（takeover） | S4-D/E fencing + E-2a 重验 | `test_erase_stale_lease_epoch_fail_closed` | 消费证据 |
> | 8 | stale revision/attempt（Tx2 精确重验） | S4-E Tx2 | `test_erase_tx2_checkpoint_attempt_mismatch_fail_closed`、`test_erase_tx2_checkpoint_intent_mismatch_fail_closed` | 消费证据 |
> | 9 | 重复调用（idempotency key + 共享 fence 串行化） | S4-E-B2/E-C | `test_concurrent_double_*_erase_serializes`、`test_idempotency_key_stable_across_lease_epoch` | **新增跨 Conversation 同一 object 双删互操作** |
> | 10 | 并发 takeover（lease 推进 + checkpoint attempt 承载） | S4-E | 手动推进 lease 模拟（S4-E 先例） | 消费证据 |
> | 11 | receipt/intent mismatch（伪造本地 receipt / digest 篡改） | S4-E E-2b/E-6 | `test_erased_fence_replay_checkpoint_digest_form`、digest mismatch fail-closed | 消费证据 |
...
>
> **F-2. 五方状态一致不变量（冻结）**——每个故障终态断言 operation/checkpoint/fence/Conversation/ledger（runtime 为 binding）五方一致；`reason_code` 冻结为共享常量（`RECEIPT_TOMBSTONE_REASON`、`REASON_EXTERNAL_*`/`REASON_RUNTIME_*` 等 conversation 级 reason 键值，participant 不得自造字符串）。**blocked 三方一致断言**：`checkpoint.reason_code == operation.failure_code`（精确相等，conversation 级 reason 键值）**且** `Conversation.purge_state` 为非 purged（`deleted`，可重试/运维）**且** fence 保持 `active`；conversation 级 reason（如 `purge_blocked_by_external_outcome_unknown`）与行级 `blocked_reason`（如 `outcome_unknown`）是两层字段，断言各自精确匹配：
>
> | 故障终态 | operation（`agent_conversation_purges`） | checkpoint（`agent_conversation_purge_owners`） | fence（`agent_erasure_fences`） | Conversation | external ledger / runtime binding |
> |---------|-----------------------------------------|-----------------------------------------------|-------------------------------|-------------|----------------------------------|
> | 成功 erased（全部 owner ACK 后） | `completed`（`completed_at` 非空） | `acked` + `ack_digest`（final scan digest） | `erased` + `ack_digest` + `acked_at`（`ck_..._ack` 强制） | `purged` | external `erased`+`receipt_digest`；runtime `closed`（ref 清空） |
> | blocked（残留/前置命中/legal hold/adapter 失败） | `blocked` + `failure_code`（= reason 键值） | `blocked` + `reason_code`（= failure_code，scan/intent digest 留证） | `active`（保持，不误进 erased） | `deleted`（非 purged，重试） | external `blocked`+`blocked_reason`；runtime `invalid`（ref 保留，不进窗口） |
> | unknown（outcome 未知，不自动重试） | `blocked` + `failure_code`=`purge_blocked_by_external_outcome_unknown`（不自动重试，S5 后接运维） | `blocked` + `reason_code` 同值 | `active` | `deleted` | external `unknown`+`blocked_reason=outcome_unknown`；runtime `invalid` |
> | fail closed（E-2a 精确重验任一不符） | `blocked` + `failure_code`（不写 receipt） | `blocked`（digest/attempt 不匹配） | `active`（same-purge-instance 门禁保护） | `deleted` | 保持原态（不覆盖、不伪造） |
> | partial ACK（D6） | **`running`/`blocked`（任一 owner 未 ACK 不 completed）** | 已 ACK owner `acked`、未 ACK owner `pending`/`blocked` | 各 owner 各自态 | `deleted` | 各 owner 各自态 |
>
...
> 6. **日志脱敏注入（R1-AC10）**：`caplog` 捕获 purge 路径日志 + 断言 operation/checkpoint/fence/ledger 字段不含正文、ref 原值、Runtime session ref、CoT/secret、自由文本 reason（spec §9.3）；reason 必须落在冻结 code 集合。
> 7. **跨 tenant / 跨 Conversation 注入（R1-AC9）**：双 tenant 同 ref 值、跨 Conversation 同一 external object（idempotency key 去重）、伪造 ACK / operation revision 重放、owner scope mismatch。
>
...
>
> - **final scan 谓词一致性**：outbox 正文事实谓词（`payload_inline IS NOT NULL OR payload_ref IS NOT NULL`，含 `cancelled`）、inbox 未决 receipt、execution Run `output_publish_state`（`pending`/`dead_letter` 残留；`not_required`/`published`/`suppressed` 终态）、external ref scan（`registered`/`blocked`/`unknown`）、runtime binding scan（`active`；排除 `closed`/`invalid`）——五族谓词各自为零才 ACK，混合 conversation 下互不误伤。
> - **锁序一致性（D8 全局链）**：Guard -> Conversation 行锁 -> owner advisory lock -> fence 重验 -> 集合 advisory lock -> 源行；所有 participant 族同序，混合 fault 下无 AB-BA。
> - **capability gate**：4 owner True 放行、external/runtime 2 owner False fail closed（拒绝后 fence/checkpoint/源行零变更）；gate 位置后移变异击杀。
> - **legal hold 不绕过**：active hold -> blocked（`purge_blocked_by_legal_hold`），hold/purge CAS 排序不被 fault 路径绕过。
> - **partial ACK（D6）**：跨 owner 部分 ACK 不 completed、全部 owner ACK 才 completed。
> - **跨 tenant 隔离**：双 tenant 并行 fault 无交叉污染（R1-AC9）。
>
> **F-5. 与 S5/S6 分工与 registry/迁移边界（冻结）**：
>
> - **不实现/不启用 S5**：scheduler claim/租约、operation `lease_epoch` 推进/接管、blocked/unknown 的 HTTP/CLI 接线、inspect/retry/reconcile API 接线、tenant 限流/指数退避、scheduler 并发 claim。S4-F 只验证 participant 侧对 takeover/租约变化的 fail-closed 反应，不实现推进者。
> - **不实现 S6**：真实 PostgreSQL Worker kill、retention clocks（`run_event_retention`/`run_audit_retention`）、seq gap、outbox claim 运维闭环、hold revision 全局联动、备份恢复 drill、body/ref orphan 巡检、owner/writer conformance suite 枚举。S4-F 的故障注入是 participant 契约级（fake adapter + 真实 PG 并发），**不等于 S6 的真实故障注入验收**。
...
> | 反例 | 触发 | 期望行为（fail closed） | 判别点 / 注入 |
> |------|------|------------------------|--------------|
> | 跨 tenant 伪造 ACK（R1-AC9） | tenant A 的 operation/revision 在 tenant B 的 conversation 上重放 | 拒绝：operation 身份/tenant scope 不匹配 fail closed，不写任何终态 | 双 tenant 种子 + 跨 tenant 重放；断言零变更；变异：跳过 tenant 校验 -> 红 |
> | owner scope mismatch（R1-AC9） | 与 conversation owner 不符的 owner 直调 participant | capability gate / owner scope 拒绝 fail closed（`purge_owner_unavailable`），不 bypass | 断言拒绝后 fence/checkpoint/源行零变更；gate 位置后移变异击杀 |
...
> | 跨 Conversation 同一 object 双删 | 同一 external object ref 出现在两个 conversation，两 purge 并发 | 每个 conversation 各自 idempotency key 去重；**同一 object 不重复产生副作用** | 双 conversation 同 ref 种子 + 并发 purge；共享 adapter 总 distinct delete==1 |
> | 混合 conversation 多族同时故障 | 同一 conversation 内 external ref + runtime binding + transport outbox 同时故障 | 各 owner 独立 blocked/unknown/erased，五方状态各自一致，互不误伤；partial ACK 不 completed | 混合种子 + 各 adapter 注入不同故障；断言五方一致矩阵（F-2）+ 未 ACK owner 保持 |
> | 日志脱敏（R1-AC10） | purge 路径触发（含故障），记录日志/operation/checkpoint | operation/checkpoint/fence/ledger 字段不含正文/ref 原值/Runtime session ref/CoT/secret/自由文本 reason | `caplog` 捕获 + 断言字段集；reason 必须落冻结 code 集合；变异：日志带 ref 原值 -> 红 |
> | partial ACK（D6） | 部分 owner 故障 blocked、部分 ACK | operation 不得 `completed`；未 ACK owner checkpoint 保持 pending/blocked | 混合故障种子；断言 operation 非 completed；变异：部分 ACK 判 completed -> 红 |
