...
- **多 issue -> 单值投影聚合规则（复核 #5）+ 源 transport 行锁（复核 #1/#集合锁，防集合并发漂移）**：同一源行可有多条 issue 行；行内 `scope_reconcile_state` 投影聚合规则——任一 issue `state <> 'resolved'` 时投影 `'pending'`；全部 issue `resolved` 后才投影 `'reconciled'`；`orphan` 类 issue 存在时投影 `'orphan'`（**优先级最高**，即便其他 issue 未 resolved 也标 orphan，因 Conversation 已删、scope 已无对象可回填）。**并发正确性（复核 #1）**：单条 issue 的 `(id, revision)` CAS 只保护**单条**状态机迁移，保护不了「同一源行的 issue 集合」——并发事务可插入新 issue（不改既有行 revision）导致基于过期集合的投影覆盖（lost-update/skew）。**锁对象必须是源 transport 行、不是 reconcile 子行**：对 reconcile 子行 `SELECT ... FOR UPDATE` 只锁已存在的行、无范围锁（空集合无行可锁），且唯一键含 `issue_code`、并发插不同 `issue_code` 不发生 unique 冲突，故锁子行 + ON CONFLICT **不成立**。冻结为：**行内投影写在源 transport 行上，故任何「读 issue 集 + 算投影 + 写投影」或「插入新 issue」都须在同一事务内先对该源 transport 行 `SELECT ... FROM <source_table> WHERE (tenant_id, id) = (?, ?) FOR UPDATE`**——该源行**恒存在**（reconcile 行由它派生，覆盖空集合与新增成员场景），行锁序列化该源行的整个「集 + 投影」临界区。写路径（backfill / S4-C / S4-D / resolved 推进）统一流程：同事务内 (1) `FOR UPDATE` 锁源 transport 行；(2) 读该源行当前完整 issue 集；(3) 若需登记新 issue 则 `INSERT ... ON CONFLICT DO NOTHING`（唯一键仅兜底同 `issue_code` 幂等，不承担集合锁）；(4) 按完整集重算投影并写源行行内 `scope_reconcile_state`。单条 `revision` CAS 仍用于单 issue 的 `open->acknowledged->resolved` 迁移防并发漂移；**集合级正确性由源 transport 行 `FOR UPDATE` 保证**（等价的 `SERIALIZABLE` + serialization-failure 重试、或按 `(tenant_id, owner_key, source_table, source_row_id)` 派生的事务级 advisory lock 亦可，三者取一并冻结为实现约束；**禁止**只用单条 revision 或锁 reconcile 子行来保护集合）。
