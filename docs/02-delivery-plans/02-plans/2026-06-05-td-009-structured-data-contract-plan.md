...
- Canonical spec: `docs/02-delivery-plans/01-specs/2026-06-05-td-009-structured-data-contract.md`.
- Technical debt card: `docs/03-engineering-governance/technical-debt.md#td-009`.
...
| `docs/02-delivery-plans/01-specs/2026-06-05-td-009-structured-data-contract.md` | Update plan link/status after implementation plan exists. |
| `docs/03-engineering-governance/current-work.md` | Keep TD-009 progress, next step, and validation results current. |
| `docs/03-engineering-governance/technical-debt.md` | Keep TD-009 fact source, status, and delivery record current. |
...
**Files:**
- Modify: `docs/02-delivery-plans/01-specs/2026-06-05-td-009-structured-data-contract.md`
- Modify: `docs/03-engineering-governance/current-work.md`
- Modify: `docs/03-engineering-governance/technical-debt.md`
- Modify: `docs/02-delivery-plans/02-plans/2026-06-05-td-009-structured-data-contract-plan.md`

- [ ] **Step 1: Update the spec plan link**

In `docs/02-delivery-plans/01-specs/2026-06-05-td-009-structured-data-contract.md`, change the header plan line to:

```md
> 计划：[plans/2026-06-05-td-009-structured-data-contract-plan.md](../02-plans/2026-06-05-td-009-structured-data-contract-plan.md)
```

- [ ] **Step 2: Update TD-009 current-work progress before final validation**

In `docs/03-engineering-governance/current-work.md`, keep TD-009 in “当前进行中” until validation finishes. Use this row shape after implementation and before final status change:
...
After all validation passes, update `docs/03-engineering-governance/technical-debt.md` TD-009 detail status from in-progress to completed using the standard green completed marker.
...
```md
| 2026-06-05 | TD-009 减少前后端契约漂移 | 🟢 完成 | 结构化抽取结果容器契约显式化：shared schema/type/helper、前端 template 窄化、后端写入 shape 测试。 | [Spec](../01-specs/2026-06-05-td-009-structured-data-contract.md) / [Plan](../02-plans/2026-06-05-td-009-structured-data-contract-plan.md) |
```

Keep only the most recent 5 rows in “最近完成”. If adding TD-009 makes 6 rows, move the oldest row into `docs/03-engineering-governance/work-log.md` before deleting it from current-work.
...
M	docs/02-delivery-plans/01-specs/2026-06-05-td-009-structured-data-contract.md
A	docs/02-delivery-plans/02-plans/2026-06-05-td-009-structured-data-contract-plan.md
...
If current-work recent-completed pruning requires a work-log update, include `docs/03-engineering-governance/work-log.md` in the expected diff and explain why.

If commit authorization is present, commit all final TD-009 changes:

```bash
git add docs/03-engineering-governance/current-work.md docs/03-engineering-governance/technical-debt.md docs/02-delivery-plans/01-specs/2026-06-05-td-009-structured-data-contract.md docs/02-delivery-plans/02-plans/2026-06-05-td-009-structured-data-contract-plan.md packages/server-python/app/contexts/document/application/tasks.py packages/server-python/tests/contexts/document/test_structured_data_contract.py packages/shared/src/schemas/document.ts packages/shared/src/schemas/index.ts packages/web/src/services/document.ts packages/web/src/views/resource/FileDetailView.vue
