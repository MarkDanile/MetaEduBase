...
> **Status:** 🟢 完成 — 2026-06-06。Plan checkboxes represent the original execution order; the canonical post-delivery record is in `docs/03-engineering-governance/technical-debt.md#td-029-收口-td-009-的-shared-schema-门禁与-filedetailview-类型错误` 交付记录段。
...
- Canonical spec: `docs/02-delivery-plans/01-specs/2026-06-06-td-029-shared-schema-gate.md`.
- TD-029 卡片：`docs/03-engineering-governance/technical-debt.md#td-029-收口-td-009-的-shared-schema-门禁与-filedetailview-类型错误`。
...
| `docs/03-engineering-governance/technical-debt.md` | TD-009 详情段「验证摘要」中关于 `pnpm --filter @metaedu/web typecheck` 的描述改为事实表述，并指向 TD-029 收口。 |
| `docs/03-engineering-governance/current-work.md` | 任务完成后把 TD-029 从「当前进行中」移到「最近完成」（5 行强约束，必要时归档最旧条目到 work-log）。 |
| `docs/03-engineering-governance/work-log.md` | 必要时新增 TD-029 索引；若 `current-work.md` 最近完成超 5 行，把最旧条目搬到这里。 |
| `docs/02-delivery-plans/01-specs/2026-06-06-td-029-shared-schema-gate.md` | 任务完成后在 spec 顶部把 `计划` 链接更新成 plan 文件相对路径。 |
...
**Files:**
- Modify: `docs/03-engineering-governance/technical-debt.md` (TD-009 详情段 + TD-029 详情段 + 总览表)
- Modify: `docs/03-engineering-governance/current-work.md`
- Modify: `docs/03-engineering-governance/work-log.md` (only if recent-completed needs to evict an old entry)
- Modify: `docs/02-delivery-plans/01-specs/2026-06-06-td-029-shared-schema-gate.md` (header `计划` link)

- [x] **Step 1: Update TD-009 验证摘要 to match reality**

In `docs/03-engineering-governance/technical-debt.md`, find this line inside the TD-009 `交付记录`:
...
In `docs/02-delivery-plans/01-specs/2026-06-06-td-029-shared-schema-gate.md`, find:
...
```md
> 计划：[plans/2026-06-06-td-029-shared-schema-gate-plan.md](../02-plans/2026-06-06-td-029-shared-schema-gate-plan.md)
```

- [x] **Step 3: Update TD-029 task overview + detail status to 🟢 完成**

In `docs/03-engineering-governance/technical-debt.md`, find in the 任务总览 table:

```md
| TD-029 | 收口 TD-009 的 shared schema 门禁与 FileDetailView 类型错误 | 🟡 进行中 | P1 | 前端 / 类型 / 交付 | [Spec](../01-specs/2026-06-06-td-029-shared-schema-gate.md) |
```

Replace with:

```md
| TD-029 | 收口 TD-009 的 shared schema 门禁与 FileDetailView 类型错误 | 🟢 完成 | P1 | 前端 / 类型 / 交付 | [Spec](../01-specs/2026-06-06-td-029-shared-schema-gate.md) / [Plan](../02-plans/2026-06-06-td-029-shared-schema-gate-plan.md) |
```
...
In `docs/03-engineering-governance/current-work.md` 当前进行中区域，把 TD-029 行替换回空行：
...
```md
| 2026-06-06 | TD-029 收口 TD-009 的 shared schema 门禁与 FileDetailView 类型错误 | 🟢 完成 | 去掉 web 对 shared 的 project reference + `FileDetailView` v-for key 用 `String(...)` 收敛 + TD-009 验证摘要校正。 | [Spec](../01-specs/2026-06-06-td-029-shared-schema-gate.md) / [Plan](../02-plans/2026-06-06-td-029-shared-schema-gate-plan.md) |
```
...
```bash
awk '/^## 最近完成/{flag=1; next} /^## /{flag=0} flag && /^\| 2026/{count++} END{print count}' docs/03-engineering-governance/current-work.md
```

If the result is 6, archive the oldest row by:

1. Reading the oldest row text from the bottom of 最近完成 in `docs/03-engineering-governance/current-work.md`.
2. Inserting a matching index row at the top of `docs/03-engineering-governance/work-log.md` 的索引表（紧跟在 `|------|...|` header 行之后）。索引行 schema：

   ```md
   | 2026-06-05 | TD-027 补 `ui-input` / `ui-btn-*` / `ui-tag-*` / `ui-dialog` 共享类（设计系统扩展） | 技术债 / 设计系统 | [#59](https://github.com/MarkDanile/MetaEduBase/pull/59) |  | `docs/03-engineering-governance/technical-debt.md#td-027` |
   ```

   依据被驱逐行的实际任务编号/标题/事实源调整字段。

3. 从 `docs/03-engineering-governance/current-work.md` 最近完成表删除该旧行。
...
```text
M docs/03-engineering-governance/current-work.md
M docs/03-engineering-governance/technical-debt.md
M packages/web/src/views/resource/FileDetailView.vue
M packages/web/tsconfig.json
?? docs/02-delivery-plans/02-plans/2026-06-06-td-029-shared-schema-gate-plan.md
?? docs/02-delivery-plans/01-specs/2026-06-06-td-029-shared-schema-gate.md
```

如果 Step 5 触发了 work-log 归档，额外允许：

```text
M docs/03-engineering-governance/work-log.md
```
...
```bash
git add docs/02-delivery-plans/01-specs/2026-06-06-td-029-shared-schema-gate.md docs/02-delivery-plans/02-plans/2026-06-06-td-029-shared-schema-gate-plan.md docs/03-engineering-governance/current-work.md docs/03-engineering-governance/technical-debt.md
# Append work-log if Step 5 archived a row:
# git add docs/03-engineering-governance/work-log.md
