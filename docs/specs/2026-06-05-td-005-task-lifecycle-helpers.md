...
`docs/engineering/technical-debt.md#td-005-拆分大型后端任务流水线文件` 记录了两份大型
Celery 任务编排文件：
...
- 验证流程按 `docs/engineering/rules/quality-gates.md`：先
  `pytest tests/shared/test_task_lifecycle.py`，再 `pytest -q` 全量，再
  `ruff check app/ tests/`。

### 4. 行为不变声明

按 `docs/engineering/rules/quality-gates.md#行为变化声明检查` 排查：
...
按 `docs/engineering/rules/quality-gates.md#验证矩阵` 选后端 Python 行：
...
完成后需在 `docs/engineering/current-work.md` 把 TD-005 移到「最近完成」并记录
PR 链接，同时在 `docs/engineering/technical-debt.md#td-005-拆分大型后端任务流水线文件`
