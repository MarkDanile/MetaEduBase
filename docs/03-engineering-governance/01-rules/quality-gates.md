...
7. 插件输出：如使用 superpower、compound-engineering-plugin 或其他插件，任务卡片的 `Spec` / `Plan` 必须指向 `docs/02-delivery-plans/01-specs/*` / `docs/02-delivery-plans/02-plans/*`；原始插件路径只能写入 `插件输出`。
