...
| 候选门禁 | 当前状态 | 触发价值 |
|----------|----------|----------|
| `current-work.md` 最近完成最多 20 行，超过后只保留最新 12 行 | 已实现 | 防止工作台无限扩张，同时禁止每个任务完成后的单条搬运 |
...
加入“下一批候选任务”前必须检查该区域仍然只是近期接力池：只允许 1 到 3 个未完成候选，不允许 `🟢 完成` 行。已完成任务应进入“最近完成”或归档到对应事实源；“最近完成”超过 20 行时，必须一次性批量归档，只保留最新 12 行，第 13 行及以后全部移入 `docs/03-engineering-governance/work-log.md` 或对应总账。
