...
| Backlog、current-work、work-log、technical-debt 中任务 ID 唯一且状态不冲突 | 候选 | 降低跨事实源状态漂移 |
| 禁止把 `REQ-xxx-FOLLOWUP` / `TD-xxx-FOLLOWUP` 作为长期任务编号 | 候选 | 保持稳定编号和可检索历史 |
| `Done` 任务在 Backlog / current-work / work-log 之间有最小索引闭环 | 候选 | 防止任务关闭后事实源缺失 |
| 旧 docs 路径残留检查 | 已实现 | 防止目录迁移后链接回退 |
| Markdown 相对链接存在性检查 | 候选 | 防止文档迁移或重命名后断链 |
