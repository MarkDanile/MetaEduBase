...
| TD-091 | 后端全量套件非确定性隔离 flake（干净 main 复现 27 failed，疑共享测试 PG 跨包污染） | ⚫ 待办 | P1 | 后端 / 测试基础设施 / 稳定性 / 隔离 | R1-S3-E round-2 独立复核要求登记（与本 Slice 修复解耦，不混入 S3-E）。**clean-main 对照**：在不含任何 S3-E 改动的干净 main 上跑全量 `pytest -m 'not external_network'` 即复现 **27 failed**，与 S3-E 分支失败集合一致/为超集 -> 属 main 预存问题，非 S3-E 引入。命令：`cd packages/server-python && uv run pytest -m 'not external_network'`（对照基线 `git checkout main` 无 S3-E 工作树）。**失败签名集合**（27 例）：`test_ai_chat` 系列 / `direct_rag` / e2e / `turn_bridge` / `writer_fence`。**重复运行分布**：单文件/小组跑全绿，原组合重跑亦绿，仅全量串行时非确定性失败——典型顺序/隔离污染。**疑似根因**：全量套件共享同一测试 PostgreSQL；`agent_control_plane`/`composition` conftest 有 autouse `TRUNCATE` 清理，`tests/contexts/ai/` 无对应清理 -> 跨包数据残留污染后续用例。**临时隔离措施**：(1) 为 `tests/contexts/ai/` 补 autouse TRUNCATE 或独立 schema；(2) 或全量入口强制 per-package 隔离库；(3) 定位具体污染用例对。**完成标准**：连续 3 次全量 hermetic 回归无重跑通过（0 failed），且干净 main 与本 Slice 分支结果一致。 |
