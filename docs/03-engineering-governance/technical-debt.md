...
| TD-106 | R1-S6 settlement SUCCESS 不写 ledger/binding（实现 vs S5-C-1 冻结契约缺口，pre-existing） | ⚫ 进行中（实现与评审已闭合） | P1 | 后端 / Erasure / Settlement / 契约 | plan §S6-15.5 登记：settlement `_apply_window_outcome` SUCCESS 分支（`settlement.py:1187-1215`）仅写 `fence.erased` + `checkpoint.acked`，**不写 `agent_external_object_refs.erase_state` / 不清 source ref / 不关 runtime binding**——S5-C-1 冻结落账列明文要求「态 1 同事务写 fence + checkpoint + ledger/binding erased + receipt」，ledger 一腿缺失。任何 settlement recovery 完成后 final scan（`external_ref_erasure_participant.py:225-232`，`erase_state='registered'` 计数）非零 → 优先级 3（`projection_calculator.py:491-507`）落 `blocked + purge_blocked_by_external_ref_scan_nonzero` → orchestrator skip acked + retry 要 blocked + rebuild 非 G1/G2 code NOT_DUE/IDEMPOTENT → **永久死锁，completed 不可达**。该缺口与 F10 路由正交，影响一切带 external ref / runtime binding 的 conversation 的 settlement recovery 路径；属 pre-existing（`test_s5_sch_d_settlement.py` 0 处 completed/rebuild 断言；`test_s5_sch_c_rebuild_seeding.py:511-550` 的 inherited-ack→completed 测试种无 ref 合成 acked+erased）。**决策门禁（2026-08-25，plan §S6-15.5）**：Phase 1 六场景事实核验完成并经独立对抗核验确认；方案 A/B 决策材料成型。**裁决（2026-08-25，用户批准方案 A）**：settlement 态 1 SUCCESS 同事务补 ledger/binding erased+receipt（闭合 S5-C-1 态 1 落账列）；方案 B 保留为无法安全落账时的 fail-closed 兜底；显式 descope 不批准；per-ref receipt 必须精确落 ledger 行（不聚合丢失）；source ref 清除遵守 E-5-2「B2 唯一清除者」（委托现有唯一写入路径，不复制第二清除者）；不新增 reason code。**实现与评审已闭合（2026-08-25，PR #592 squash 入 #590 mergeCommit `e345c429` 并评分 92 Original）**：方案 A 实现落地（`SettlementService._close_window_ledger` 严格计数守卫 + 空窗口合法 no-op SUCCESS 不调 adapter/不伪造 receipt/不写 ledger/不清 source + per-ref/binding 落 receipt+清源 ref + D8 集合锁 + B2 唯一清除路径）+ M8/M9 mutation 补齐冻结矩阵第 (5) 项 + 9/9 全真红 + 20 项 TD-106 专项 + composition 781 passed/1 skipped + P1-A/P1-B 定向复核 P0/P1=0。**状态**：实现与评审已闭合并入 integration parent #590（parent 现 head `733f1b2b`，仍 OPEN/Draft），**待 #590/main merged-boundary 才算关闭**——本登记**不**标记完成、不关闭。P2 三项保留未关：(P2-1) runtime per-binding receipt 形参零使用（语义=ACK 证据链输入，不加列）；(P2-2) settlement↔participant 跨入口双写者并发用例（M9 shared-helper stale-CAS 不冒充全覆盖 = 裁决项 2a）；(P2-3) M4 receipt 复用深度。F10 路由链尾（§S6-15.3 新 op→completed）须先解决本缺口；关联 TD-105、REQ-047。 |
