...
| TD-106 | R1-S6 settlement SUCCESS 不写 ledger/binding（实现 vs S5-C-1 冻结契约缺口，pre-existing） | ⚫ 待办 | P1 | 后端 / Erasure / Settlement / 契约 | plan §S6-15.5 登记：settlement `_apply_window_outcome` SUCCESS 分支（`settlement.py:1187-1215`）仅写 `fence.erased` + `checkpoint.acked`，**不写 `agent_external_object_refs.erase_state` / 不清 source ref / 不关 runtime binding**——S5-C-1 冻结落账列明文要求「态 1 同事务写 fence + checkpoint + ledger/binding erased + receipt」，ledger 一腿缺失。任何 settlement recovery 完成后 final scan（`external_ref_erasure_participant.py:225-232`，`erase_state='registered'` 计数）非零 → 优先级 3（`projection_calculator.py:491-507`）落 `blocked + purge_blocked_by_external_ref_scan_nonzero` → orchestrator skip acked + retry 要 blocked + rebuild 非 G1/G2 code NOT_DUE/IDEMPOTENT → **永久死锁，completed 不可达**。该缺口与 F10 路由正交，影响一切带 external ref / runtime binding 的 conversation 的 settlement recovery 路径；属 pre-existing（`test_s5_sch_d_settlement.py` 0 处 completed/rebuild 断言；`test_s5_sch_c_rebuild_seeding.py:511-550` 的 inherited-ack→completed 测试种无 ref 合成 acked+erased）。**决策门禁（2026-08-25，plan §S6-15.5）**：Phase 1 六场景事实核验完成并经独立对抗核验确认（settlement 全文件零 UPDATE refs/bindings；态 4 ACK-lost 不受影响——其 scan-zero 前置仅经 participant Tx2 tri-state-zero 闸门可达；死锁精确条件 = participant Tx2 对整个窗口完全未落账；多 ref 聚合丢 per-ref receipt；external `reconcile_external_ref` gated `blocked/unknown` 对 `registered` raise、runtime `reconcile_runtime_binding` 仅测试调用零生产接线——无线后修复路径）。方案 A/B 决策材料已成型于 plan §S6-15.5：方案 A（实现补齐——settlement 态 1 同事务补 ledger/binding erased+receipt，写域 S5-C-2 已含 ledger/binding 无需契约变更，但需裁决第二写者并发幂等 + T2 集合锁扩展 D8 + E-5-2「B2 唯一清除者」源行越权；无需新增 schema/migration）；方案 B（路由 fail-closed——ledger 腿不可满足时拒发部分 态 1，改落具名 reconcile-only blocked，消除假终态成功分叉，需裁决 fail-close 触发归 态 3 子情形或新增 reason code；自身不产生 completed）。**裁决（2026-08-25，用户批准方案 A）**：settlement 态 1 SUCCESS 同事务补 ledger/binding erased+receipt（闭合 S5-C-1 态 1 落账列）；方案 B 保留为无法安全落账时的 fail-closed 兜底；显式 descope 不批准；per-ref receipt 必须精确落 ledger 行（不聚合丢失）；source ref 清除遵守 E-5-2「B2 唯一清除者」（委托现有唯一写入路径，不复制第二清除者）；不新增 reason code（除非现有事实源无法表达且另行停下请求裁决）。实现由**独立 stacked PR** 承载（`feature/req041-047-r1-s6-i3-td106-settlement-ledger-closure`，base = #590 分支，非 main/非 #591），#591 不承载代码。**保持 P1 open 至实现 PR 合并**。F10 路由链尾（§S6-15.3 新 op→completed）必须先解决本缺口；关联 TD-105、REQ-047。 |
