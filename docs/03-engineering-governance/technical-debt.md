...
| TD-085 | 收口 AI Chat、Skill 与 Agent App 的上下文边界倒置 | 🟡 进行中 | P1 | 后端 / Agent Platform / DDD / 可维护性 | [REQ-059](../01-product-planning/05-requirements/REQ-059-enterprise-agent-platform-kernel.md) / 2026-07-23 源码复核 + 2026-09-15 Phase 0 审计 + 2026-09-17 readiness decision gate + 2026-09-18 Slice A 启动 + 2026-09-20 Slice A 合并（PR #627）+ 2026-09-22 Slice B 合并（PR #629）+ 2026-09-23 Slice B 治理收口合并（PR #630）+ 2026-09-26 Slice C 合并（PR #632，mergeCommit `ff85a47f6ce4e63b95d4d1899776f925ceeec424`） |
...
| TD-093 | R1-S5 settlement 收口健壮性：租约过期校验 + receipt_lookup 异常对称 | ⚫ 待办 | P2 | 后端 / Erasure / Settlement / 健壮性 | R1-S5 root 广域三面复审（面A P2-1 + 面B P2-2）：(1) settlement `_validate_frozen_snapshot` 校验 #3 只查 `lease_epoch>=1 + lease_expires_at IS NOT NULL`，不比较过期——`retry_reconcile.reconcile` 直调 `closeout_erasing` 不经 renew，过期租约可进入（S5-SCH-9 门禁 (ii) 字面未满足；orchestrator 路径由 renew 同事务续租安全）；(2) `_recover_ref` 的 `receipt_lookup` 无 try/except（与 `_replay_ref` 不对称），接线真实 adapter 后 lookup 持续故障 → erasing 无限重试零终态（liveness） |
| TD-094 | R1-S5 scheduler 契约登记缺口：编排方 attempt 写者矩阵 + settlement 锁内 adapter I/O（需契约评审决策） | 🟢 完成 | P1 | 后端 / Erasure / Scheduler / 契约 | R1-S5 root 广域三面复审（面A P0-1 校准 P1 + 面B P1-1）→ **2026-08-18 裁决一/二落地**：裁决一（plan 写者矩阵回填）——S5-SCH-0/2 按列登记 attempt 推进写者（external/runtime = participant Tx1 唯一推进；scan/transport = SCH-B orchestrator entry 前推进，pre-window gate 不推进；rebuild/seeding 仅初始化；coordinator 不写）+ 判别测试（scan owner 每 entry 恰好一次 / external/runtime 无 double increment / pre-window 重入不变 / mutation double increment 转红）；裁决二（settlement 禁持锁 adapter I/O）——S5-SCH-2 补「单事务」语义澄清 + settlement recovery 拆 T1（锁内读）→ 锁外 lookup/replay → T2（重验全 token + CAS 落账），T2 任一重验失败 fail closed 零写、不自动二次 adapter 调用、不创建新 Tx1；`converge_failed_fence` 等无 adapter 路径保持单事务；判别测试（adapter 阻塞时第二连接可取锁 / T1-T2 间 lease_epoch 变化 T2 零写 / 双连接幂等 + T2 CAS 单写者 / mutation 移回锁内转红） |
| TD-095 | R1-S5 root integration mutation 驱动入库 | ⚫ 待办 | P2 | 后端 / 测试基础设施 / Mutation | R1-S5 root 广域三面复审（面C P2-1）：root integration 批次的 6 组 mutation 验证（删 ref 输入 20 转红 / key 加 epoch-attempt 5 转红 / 去 SCH-C/SCH-D wiring / 去 coordinator 触发 / 去 populate_existing / 去 pre-window 豁免）为手工验证，无 `scripts/sch_*_mutation_kill.py` 同级驱动脚本入库（SCH-A/B/C/D 均有）；后续以 `sch_e_mutation_kill.py`（或 sch_root_mutation_kill.py）形式入库 |
| TD-096 | R1-S5 scheduler P3 精度族（8 项） | ⚫ 待办 | P3 | 后端 / Erasure / Scheduler / 词语精度 | R1-S5 root 广域三面复审 P3 合计：(1) `_arbitrate_retry` UPDATE 隐式取 operation 行锁未先取 Conversation（S1 字面，无死锁后果）；(2) tick `_candidate_operations` 全局扫描无 tenant 谓词（partial index 首列不可用，性能注记）；(3) `_verify_cycle_token` 未校验 operation.revision（S5-SCH-9 元组字面，功能等价补偿存在）；(4) rebuild G3 active-hold gate 先于 quiesce 判定（plan 1.5 顺序不字面，零写无后果）；(5) tick 候选集含 failed 终态 op（反复 fail-closed raise 噪音）；(6) `_require_joint_wiring` claim=None 分支无直接测试；(7) 六 owner 实为 5 个 erase 方法名（transport 共用 erase_transport_owner）；(8) joint 测试异常断言二选一措辞 + 零 ref 平凡真 |
| TD-097 | R1-S6-I1 retention worker 健壮性/边界族（P3 精度 + 覆盖补强，10 项） | ⚫ 待办 | P3 | 后端 / Erasure / Retention / 健壮性 | R1-S6-I1 三面复审 P3 合计（`app/composition/retention_workers.py` + 测试）：(1) 计数非按 tenant（S6-2.5「按 tenant 记录」待接线时补）；(2) `_prune_expired_prefix` 前缀 SELECT 无 LIMIT（单 run 大事件量整批物化进 Python）；(3) batch 单事务累计持有至多 `batch_size` 个 Run 行锁（「短事务」意图打折，慢语句锁持有线性放大）；(4) `_lock_run_row` 死读字段（event 路径 `event_log_complete` / audit 路径全字段未用）；(5) `runs_processed` 在并发 erasure 已清 payload（UPDATE 0 行）时仍 +1 计数失真；(6) 健壮性边界无测试：`batch_size=0`、run 不存在跳过、90/365 精确边界、空前缀、outbox `dead_letter`/`claimed` 状态、completed `terminal_output` 随行删除；(7) `seen_hold_skipped` 对「候选后创建→锁内命中→快速释放」的 run 本轮过排除（仅延迟，无正确性影响）；(8) S6-RET-1 未断言仅 expiry（无 prune）时 `event_log_complete` 保持 True；(9) 预 tombstone 头行 prune 路径无直接测试（仅经 inline→expiry→prune 间接覆盖）；(10) R1-AC10 sentinel 判别测试缺失——worker 无 logger/metrics（结构性合规，登记豁免或补轻量断言） |
| TD-098 | R1-S6-I1 audit retention 契约语义张力（剪除证据 fail-closed 字面后果 + outcome_unknown 死路径） | ⚫ 待办 | P2 | 后端 / Erasure / Retention / 契约 | R1-S6-I1 三面复审（面A P1-2）→ 已按冻结字面实现（`first_available_event_seq > 1` → approval 无法判定 → blocked，业务保留 + 人工处置）。**字面后果**：event retention 90 天剪除前缀先于 audit 365 天 → 前缀剪除的 run 在 audit 侧一律保守 blocked，清理能力显著收窄（数据面建议契约修订评审明确语义，S6-2.5「按 tenant」与 S6-3 清理目标联动评估）。**outcome_unknown 不对称**：契约只在「未解决审批」条目冻结剪除 fail-closed，`tool.outcome_unknown` 无该条款——正常流程证据剪除后检查在现存 envelope 上零命中（死路径），需契约修订评审决定是否同构 fail-closed |
| TD-099 | R1-S6-I1 pre-existing 测试兼容升级（043 guard widening 决 A 实施） | ⚫ 待办 | P3 | 后端 / 测试 / 迁移 / 兼容性 | R1-S6-I1 Ready 尝试 Backend full 失败 4 项（迁移 043 与 pre-existing 测试硬编码 039/041 期望冲突），决 A 已升级 test_s3d_run_event_guard / test_s4ea_ref_tombstone_guard / test_alembic_migrations 以匹配 frozen S6-10 行为；migration 043 与 S6-10 契约未改动。**剩余 follow-up**：(1) pre-existing 测试用 `git restore` 兜底还原（mutation script 修复：DB guard 重载 + import 顺序）；(2) 后续 mutation 脚本需以 `importlib` 或 sys.path 显式包路径加载 tests 包（首跑 import 失败已修）；(3) frozen S6-10 widening 与 043 落地需在 S6-I2/I3 或契约修订评审中确认 outcome_unknown 死路径是否同构 fail-closed（见 TD-098） |
| TD-100 | R1-S6-I2 verify 巡检并发写幂等测试补齐（persist_event_gap=True × asyncio.gather） | ⚫ 待办 | P2 | 后端 / Erasure / Inspection / 测试 | R1-S6-I2 PR #584 B 面 P2-1：现有 `test_concurrent_verify_serializable` 仅覆盖 `persist_event_gap=False`（只读路径），未验证双连接同时 `persist_event_gap=True` 时 ledger 唯一约束 + event_log_complete 幂等 UPDATE 协同工作。补 `test_concurrent_verify_write_idempotent`：种 gap → `asyncio.gather(verify_inspection(... persist_event_gap=True) × 2)` → 断言 ledger 行数 = 1 + 两次报告 `event_log_complete_writes = 1` + exit_code 一致。后续按 B 面建议待 S6-I2 收口批次或独立 PR 启动。 |
| TD-101 | R1-S6-I2 missing_fence_or_owner_scope 5a 分支契约张力（部署门禁 vs 巡检实现边界） | ⚫ 待办 | P2 | 后端 / Erasure / Inspection / 契约 | R1-S6-I2 PR #584 B 面 P2-2 + C 面 P2-1 关联：`_scan_missing_fence_or_owner_scope` 当前仅实装 5b（null conversation_id outbox），5a（已安装 owner 缺 fence）以 `_ = (await ...)` 占位跳过并注释"那是部署门禁范围"。`registry capability` 保持 False 现状下 5a 触发条件为零，但契约文档 S6-6 是否要求本分支实装仍需评审决策。建议：(a) 在 S6-6 契约条款中明示 5a 归属（巡检 vs 部署门禁）；(b) 若归属巡检，补实装 + 测试覆盖（C 面 P2-1 关联项）；(c) 若归属部署门禁，移除占位变量 + 添加正式契约引用注释。后续在 S6-I3 或契约修订评审中决策。 |
| TD-102 | R1-S6-I2 digest_conflict 真实 PG 端到端测试补齐（owner_version 漂移 + ingress/ack_digest hex 注入） | ⚫ 待办 | P2 | 后端 / Erasure / Inspection / 测试 | R1-S6-I2 PR #584 C 面 P2-1：`_scan_digest_conflict` 实现完整（`agent_erasure_fences` + owner_version 漂移 + ingress/ack_digest 64-hex 正则），但 `test_s6i2_orphan_inspection.py` 无真实 PG 端到端用例，仅静态枚举层覆盖。补 `test_verify_digest_conflict_owner_version_drift` + `test_verify_digest_conflict_ack_digest_invalid_hex`：种 fence row 注入 owner_version 漂移 + ingress/ack_digest 非 64-hex → 验证 finding 被检出。后续按 C 面建议待 S6-I2 收口批次或独立 PR 启动。 |
| TD-103 | R1-S6-I2 mutation kill baseline 缺失（与 S6-I1 历次 18/18 mutation 比对） | ⚫ 待办 | P3 | 后端 / 测试基础设施 / Mutation / Inspection | R1-S6-I2 PR #584 C 面 P3-3：`current-work.md` 描述"18+ 项 mutation kill（先红后绿，try/finally 恢复）"，实际 PR 内 0 mutation 用例（grep -c mutation = 0）。S6-I1 历次有 `scripts/sch_*_mutation_kill.py` 同级驱动脚本（18/18 通过），S6-I2 未要求 mutation 验证（仅 21 真实 PG 专项测试）；任务卡"验证状态：待补"如实反映此空白。后续以 `s6i2_mutation_kill.py`（参考 S6-I1 模板）形式入库；目标覆盖：(a) 删 `_mark_event_gap_incomplete` Run 行锁 → 竞态转红；(b) 改 ledger issue_class 对齐逻辑 → class_scope CHECK 违规转红；(c) 改 `_register_ledger_issue` ON CONFLICT → 双重登记转红；(d) 改 report_to_dict sentinel → 含正文转红。 |
| TD-104 | R1-S6-I3 schema/test alignment 后续（PR-A 承接：列名对齐 + 负例 fixture 合法化 + 独立 CHECK 拒绝断言） | ⚫ 待办 | P2 | 后端 / Erasure / Restore-Replay / 测试 | R1-S6-I3 PR #586 fresh PG schema-fact audit（alembic head=043）A/E 类登记：(A1) ledger export 引用 `agent_transport_scope_reconcile.observed_at` 不存在（真实列 `created_at`/`resolved_at`）；(A2) checkpoint 快照引用 `intent_digest`/`recorded_at`/`failure_code`/`revision` 不存在（真实列 `capability_digest`/`reason_code`/`created_at`，migration 034 schema）；(A3) registry_snapshot 等 fixture 列名与真实 schema 对齐；(E1) 负例 fixture `INVALID_DIGEST_SHORT` 配 `state='acked'` 违反 `ck_agent_purge_owner_ack`——须改合法 64-hex `ack_digest`，并**另加短 digest 触发 CHECK 拒绝的独立负例**（不靠非法组合构造、不放宽约束）。承接边界：只做 schema/test alignment（bounded fix），不顺手改 C 类状态域语义——C 类已由 R1-S6-I3-B 契约纠偏裁决（plan §S6-11..S6-14）；#586 保持 Draft，PR-A 在其关闭/supersede 决策后另起分支执行。 |
| TD-105 | R1-S6-F10 实现承接（契约纠偏后解除 skip + 真实 PG 判别） | 🟢 完成 | P2 | 后端 / Erasure / Settlement / 测试 | R1-S6-I3-C（PR #590）F10 因「契约冲突」skip；契约核对纠偏（plan §S6-15）澄清冲突源于「T1/T2 锁外阶段」读法歧义，非期望自相矛盾——settlement T2 单向 hold 检查（`settlement.py:849`）放行推进、完成 erase → G2 `blocked_hold_revision_changed` → rebuild。承接边界：待契约纠偏 PR 三面复审确认读法甲（settlement T1/T2）后，另起测试 PR 解除 `test_f10_contract_conflict_not_implemented` skip，按 §S6-15.3 路由表实现真实 PG 判别（settlement 锁外窗口 create hold → T2 完成 fence erased + checkpoint acked + 零复活正文 → 投影 blocked → 释放/过期 hold 后 rebuild HOLD_GATED 消解；rebuild→completed 链尾须先解决 §S6-15.5 settlement SUCCESS ledger 写缺口 = TD-106）。**完整合并链（2026-08-26）**：PR #596 承接 TD-105 实现（source head `f5cb0b34` / implementation HEAD `7cc732ba`，base = main `695fed8e`，评审对象 `695fed8e..7cc732ba`，净 diff 3 文件 1005+/23-：`packages/server-python/tests/composition/test_s6i3_fault_f10.py` 新增 732 行（8 项真实 PG 测试：test_f10_t1_then_advance_hold_t2_completes / test_f10_t2_unidirectional_advance_passes / test_f10_t2_unidirectional_regression_fails_closed / test_f10_projection_g2_blocked_hold_revision_changed_no_completed / test_f10_rebuild_hold_gated_with_active_hold / test_f10_rebuild_unblocks_after_hold_release_bumps_revision / test_f10_rebuild_unblocks_after_hold_expire_no_bump / test_f10_no_body_resurrection_no_repeated_adapter_replay_idempotent）+ `test_s6i3_fault_hold.py` F10 skip 占位迁出（F9 保留 + F10 迁出至 test_s6i3_fault_f10.py）+ `scripts/s6i3_f10_mutation_kill.py` 新增 269 行（8 项 mutation：M1-M5 + M7/M8 真红 + M6 NOT-RED 如实登记））。**验证基线**：fresh PG head=043 + F10 专项 8 passed（5.64s）+ 关联回归 94 passed（F10 + F9 + TD-106 settlement/lock + S5-D + S5-C；32.20s）+ composition 全量 771 passed（204.25s）+ ruff/mypy baseline(0 reg)/git diff --check/engineering-docs --full 全绿 + Ready 三路 required checks 全 SUCCESS（Backend full 15m0s + Engineering docs 18s + Frontend 2m45s）。**三面独立复审**（P0=0/P1=0/P2=0/P3=3）：面 A 契约/状态机/范围 2 项观察（runtime binding 由既有 TD-106 测试覆盖 + completed 链尾 invariant 独立测试集范畴）+ 面 B 并发/锁序/数据完整性 0 项 + 面 C 测试/mutation/运维/事实源 1 项观察（_BlockingLookupAdapter 局部复制可优化）。**正式评分**（Ready 状态）：P0/P1=0；总分 **97 Original**；维度评分：范围与需求匹配 15/15 + 实现质量 19/20 + 测试与验证证据 19/20 + 事实源与流程遵守 15/15 + 风险与行为变化控制 15/15 + 可评审性与交接质量 10/10 + 持续改进信号 4/5；`scripts/check-review-score-submit --base 7cc732ba --pr 596` PASS（one Original row，Metrics byte-identical）。**完整合并链**：PR #596（squash merge 入 main `c0ec008d4299e8a38e4b7f049146a14eb2b87cf8`，source head `f5cb0b34` / implementation HEAD `7cc732ba`，mergedAt 2026-08-26T12:42:58Z，3 路 required checks 全 SUCCESS，**已计入 Score Log +1 Original 行**）。**状态（2026-08-26）**：TD-105 **已完成并关闭**——F10 routing 四环 + TD-106 收口不变式保持 + 7/8 mutation 真红 + M6 NOT-RED 登记为 test contract 增强 follow-up；F10 不直接宣称原 operation completed（completed 仍依赖后续 rebuild、新 operation 和最终扫描）；migration 043 / schema / enum / CHECK / CI / 门禁脚本 / KNOWN_ISSUES 零改动；external/runtime capability 仍 False（保持 `erase_available=False`，**未**翻转）；S5 production wiring / registry capability 翻转 / 六 erase 入口生产可达 均未启动。**P2 保留未关**（不借 P1 修复扩大范围；按已登记项承接）：(P2-1) F-matrix 7/12 mutation NOT-RED + F10 M6 NOT-RED 判别载体（completed 全链路 completed 前置完整性独立测试集）— 待后续独立 test contract 增强 PR；(P2-2) runtime binding F10 不变式（已由既有 TD-106 测试集覆盖，不重复）。**后续未完成边界（TD-105 已关闭但 S6-I3 整体仍 follow-up 保留）**：PR-D（ledger export executor + restore-before-open runbook）/ PR-E（release drill 五阶段 canary）/ C1 Durable Core 总验收 / S5 production wiring / registry capability 翻转 / 六 erase 入口生产可达 均未启动；TD-104（PR-A schema/test alignment 残项）+ REQ-047（R1-S6 implementation conformance 联合闭环）保持后续承接；F-matrix 7/12 + F10 M6 test contract 增强 待后续独立 PR。关联 REQ-047。 |
| TD-106 | R1-S6 settlement SUCCESS 不写 ledger/binding（实现 vs S5-C-1 冻结契约缺口，pre-existing） | 🟢 完成 | P1 | 后端 / Erasure / Settlement / 契约 | plan §S6-15.5 登记：settlement `_apply_window_outcome` SUCCESS 分支（`settlement.py:1187-1215`）仅写 `fence.erased` + `checkpoint.acked`，**不写 `agent_external_object_refs.erase_state` / 不清 source ref / 不关 runtime binding**——S5-C-1 冻结落账列明文要求「态 1 同事务写 fence + checkpoint + ledger/binding erased + receipt」，ledger 一腿缺失。任何 settlement recovery 完成后 final scan（`external_ref_erasure_participant.py:225-232`，`erase_state='registered'` 计数）非零 → 优先级 3（`projection_calculator.py:491-507`）落 `blocked + purge_blocked_by_external_ref_scan_nonzero` → orchestrator skip acked + retry 要 blocked + rebuild 非 G1/G2 code NOT_DUE/IDEMPOTENT → **永久死锁，completed 不可达**。该缺口与 F10 路由正交，影响一切带 external ref / runtime binding 的 conversation 的 settlement recovery 路径；属 pre-existing（`test_s5_sch_d_settlement.py` 0 处 completed/rebuild 断言；`test_s5_sch_c_rebuild_seeding.py:511-550` 的 inherited-ack→completed 测试种无 ref 合成 acked+erased）。**决策门禁（2026-08-25，plan §S6-15.5）**：Phase 1 六场景事实核验完成并经独立对抗核验确认；方案 A/B 决策材料成型。**裁决（2026-08-25，用户批准方案 A）**：settlement 态 1 SUCCESS 同事务补 ledger/binding erased+receipt（闭合 S5-C-1 态 1 落账列）；方案 B 保留为无法安全落账时的 fail-closed 兜底；显式 descope 不批准；per-ref receipt 必须精确落 ledger 行（不聚合丢失）；source ref 清除遵守 E-5-2「B2 唯一清除者」（委托现有唯一写入路径，不复制第二清除者）；不新增 reason code。**完整合并链（2026-08-25 → 2026-08-26）**：方案 A 实现落地（`SettlementService._close_window_ledger` 严格计数守卫 + 空窗口合法 no-op SUCCESS 不调 adapter/不伪造 receipt/不写 ledger/不清 source + per-ref/binding 落 receipt+清源 ref + D8 集合锁 + B2 唯一清除路径）+ M8/M9 mutation 补齐冻结矩阵第 (5) 项 + 9/9 全真红 + 20 项 TD-106 专项 + composition 781 passed/1 skipped + P1-A/P1-B 定向复核 P0/P1=0。merge chain：`PR #592` (squash 入 #590 mergeCommit `e345c429`, 评分 92 Original) → `PR #590` (squash 入 #586 mergeCommit `ee53b172`, 评分 94 Original) → `PR #586` (squash merge 入 main，**mergeCommit `68fafd81de5b0925be44e935c532cdce898e2935`**，**source head `cca892b54b39836cc5e98a5e84e3028ba1ddea04`** + FINAL_IMPL_HEAD `1a61ba6f`，**评分 94 Original 已落入 Score Log**，评审对象 `ddcb87ca..1a61ba6f` main→PR head，`scripts/check-review-score-submit --base 1a61ba6f --pr 586` PASS，Metrics byte-identical，Ready 三路 required checks 全 SUCCESS)。**状态（2026-08-26）**：TD-106 **已完成并关闭**——bounded integration (F1-F14 故障矩阵 + TD-106 方案 A) 已 squash merge 入 main（PR #586 `68fafd81`），main 现包含完整实现；migration 043 / schema / enum / CHECK / CI / 门禁脚本 / KNOWN_ISSUES 零改动；external/runtime capability 仍 False（保持 `erase_available=False`，**未**翻转）；S5 production wiring / registry capability 翻转 / 六 erase 入口生产可达 均未启动。**P2 三项保留未关**（不借 P1 修复扩大范围；按已登记项承接）：(P2-1) runtime per-binding receipt 形参零使用（`write_erased_and_close_binding` 接收形参零使用，语义=ACK 证据链输入，不加列）；(P2-2) settlement↔participant 跨入口双写者并发用例（M9 shared-helper stale-CAS 不冒充全覆盖 = 裁决项 2a）；(P2-3) M4 receipt 复用深度（同 receipt 复用 unit 级）。**后续未完成边界（TD-106 已关闭但 S6-I3 整体仍 follow-up 保留）**：F10 仍 skip（`test_f10_contract_conflict_not_implemented` 保持 skip，TD-105 承接——前置 TD-106 现已完成，可启动 F10 实现 PR）；#590 mutation 9/12 真红 + M-F3/M-F5/M-F8 mutation 停止条件登记保持；PR-D（ledger export executor + restore-before-open runbook）/ PR-E（release drill 五阶段 canary）/ C1 Durable Core 总验收 / S5 production wiring / registry capability 翻转 均未启动；TD-104（PR-A schema/test alignment 残项）+ REQ-047（R1-S6 implementation conformance 联合闭环）保持后续承接；restore_replay_executor 的 PR-D 骨架已随 scope 收敛撤回，S6I2_PENDING_WRITERS 中仍仅 pending。关联 TD-105、REQ-047。 |
...
（Slice A 已合并入 main：PR #627 mergeCommit `8fb60704`、Original 评分 95/100；runtime 子包 LlmProvider port + OpenAIProvider adapter 已抽出；knowledge/application 反向 import 已双向清零；Slice B 已合并入 main：PR #629 mergeCommit `e36a70c1`、Original 评分 95/100，ai_chat_service.py 1054 → 479 行（≤500 达标），prompt/tool-calling 编排入 runtime/application，diagnostics/DTO 留 knowledge/application；Slice C 已合并入 main：PR #632 mergeCommit `ff85a47f`、Original 评分 96/100，skill_runner.py DD/QCC/尽调 persona 硬编码移除并泛化为注入式通用 runner（step_params_mapper / report_persona 构造注入点，默认业务中性），dd_query_runner.py 与 park_investment_dd.yaml 迁至 due_diligence，新增 skill_caller.py 单一装配点保持 due_diligence → skill_registry 单向，internal_query step 类型与 SkillStepResult.query_audit_id 契约保留，CI full hermetic 3028 passed；Slice D/E 未启动；TD-085 仍为整体进行中而非完成；REQ-043 / WS-S2 / WS-S3 状态未推进）
...
- C. 通用 Skill 层被企业尽调反向塑形（下列 3 个行号锚点 :103 / :110 / :538 为 2026-09-15 基线快照；对应 DD/QCC/尽调 persona 硬编码内容已于 Slice C（2026-09-26，PR #632）移除/迁至 due_diligence，历史结论保持有效）：
...
- 2026-09-18：TD-085 readiness PR #625 已 squash merge 入 main `b7339a12`（implementation baseline `e6d876b30182822550c9bc774c349eb524447913`；评分提交 commit `400076aefe6b6563e5480a83eeac37ea7bdbf9d2`；Original 评分 92/100 已入账 `review-score-log.md`；Draft CI 3/3 + Ready CI 3/3 全 SUCCESS；P0=0 / P1=0 / P2=9 绑定既有 TD-085 Slice A/C/D 不触发返修）；4 项 ADR 已入 spec §11（OQ-1 runtime 子包路径 / OQ-2 knowledge 保留 NER-Fusion-Diagnostics / OQ-3 internal_query 是 REQ-046 v2 契约保留 / OQ-4 FencedExecutionPort 复用 + WorkspaceSnapshotPort 新增）；readiness decision gate 已完成；TD-085 状态仍为 🔵 就绪（**未**翻为 🟢 完成）；implementation slices（Slice A / B / C / D / E）尚未启动；REQ-043 仍 ⚫ Candidate / REQ-047 Extended 仍 🟣 Shaping / WS-S2 option B 仍 BLOCKED / WS-S3 仍 BLOCKED；启动 Slice A = 独立后续 PR 单独登记。
- 2026-09-20：TD-085 Slice A PR #627 已 squash merge 入 main `8fb607049aa57c7d06310696dccb03a8f7fe8834`（implementation base `70957a12`；implementation head / SCORE_BASE `5b19178d`；评分提交 commit `b055e380`；Original 评分 95/100 已入账 `review-score-log.md`；Draft Backend iteration + Ready Backend / Frontend / Engineering docs + 评分后 CI 全 SUCCESS；P0=0 / P1=0；P2=1 组合根源码字符串测试绑定 Slice B；P3=3 分别绑定 Slice B 测试维护与 Slice E 兼容性核查）；Slice A 交付：runtime 子包 LlmProvider port + OpenAIProvider adapter + ai_router 组合根单点装配 + knowledge/application 反向 import 双向清零 + 23 runtime 测试 + 35 AI 回归全 pass；Slice B/C/D/E 未启动；TD-085 状态保持 🟡 进行中（未翻 🟢 完成）；REQ-043 仍 ⚫ Candidate / REQ-047 Extended 仍 🟣 Shaping / WS-S2 option B 仍 BLOCKED / WS-S3 仍 BLOCKED。
- 2026-09-22：TD-085 Slice B PR #629 已 squash merge 入 main `e36a70c18e2c9d5c7f5e15a269d92217a401364e`（implementation base `30da367d`；implementation head / SCORE_BASE `16fc3a88`；评分提交 commit `7ae6853a`；Original 评分 95/100 已入账 `review-score-log.md`；Draft CI 3/3 + Ready CI 3/3 + 评分后 CI 3/3 全 SUCCESS，Backend full hermetic 3017 passed；P0=0 / P1=0 / P2=0 / P3=4 均非阻塞并绑定既有归属不新建编号）；Slice B 交付：ai_chat_service.py 1054 → 479 行（≤500 达标）+ runtime/application prompt_builder（124 行，property 协变 Protocol 窄消费面）+ tool_orchestrator（143 行，legacy dict 信封两轮编排逐字保持）+ knowledge/application ai_chat_diagnostics（244 行）+ ai_chat_dto（399 行，dispatch 参数注入）+ 28 个新判别测试 + 组合根 inspect.getsource 测试升级真实构造断言（承接 Slice A P2）+ 既有回归断言零修改；P3 保留未修复：迁移日志前缀不一致 / 「逐字迁移」表述精度 / ai_chat_dto 命名内聚 / AST 扫描 ast.Import 盲区（base 遗留）；Slice C/D/E 未启动；TD-085 状态保持 🟡 进行中（未翻 🟢 完成）；REQ-043 仍 ⚫ Candidate / REQ-047 Extended 仍 🟣 Shaping / WS-S2 option B 仍 BLOCKED / WS-S3 仍 BLOCKED。
- 2026-09-23：TD-085 Slice B post-merge governance closeout PR #630 已 squash merge 入 main `08c84452f3fa846524bd5cd5f357b3d5d93e0d4b`（mergedAt 2026-09-23T01:07:11Z；implementation base `e36a70c1`；implementation head `cf6a7692`；score base `ade13765`；评分提交 commit `8dc88af2`；Original 评分 94/100 已入账 `review-score-log.md`；`scripts/check-review-score-submit --base ade137657752c63043f7f888248c450b46aee746 --pr 630` PASSED；Draft CI 3/3 + 返修后 Draft CI 3/3 + Ready CI 3/3 + 评分后 CI 3/3 全 SUCCESS，均为 scope-aware docs-only 路径，full hermetic PG 证据属实现 PR #629 未在 closeout 重跑）；Slice B 治理事实源（current-work / work-log / technical-debt / plan 4 文件 +11/-52，含评分行最终 5 文件 +12/-52）同步入 main；独立复审发现唯一 P1-1（新增测试计数 30 应为 28）已经 collect-only 重收集 11+8+9=28 返修闭环；Slice B 的 4 项 P3（迁移日志前缀不一致 / 「逐字迁移」表述精度 / ai_chat_dto 命名内聚 / AST 扫描 ast.Import 盲区（base 遗留））保持保留未修复、绑定既有归属不新建编号；Slice C/D/E 未启动；TD-085 状态保持 🟡 进行中（未翻 🟢 完成）；REQ-043 仍 ⚫ Candidate / REQ-047 Extended 仍 🟣 Shaping / WS-S2 option B 仍 BLOCKED / WS-S3 仍 BLOCKED。
- 2026-09-26：TD-085 Slice C PR #632 已 squash merge 入 main `ff85a47f6ce4e63b95d4d1899776f925ceeec424`（mergedAt 2026-09-26T15:10:54Z；implementation base `bc6359b7fe527f89ca21aef2db6d4f80ceb64665`；implementation head / SCORE_BASE `ad286a931e8785c30967e6edce1353d00e9a2423`；评分提交 commit `c7bca96fad4e9cf330f9fd0087bb2ddfdfa089f7`；Original 评分 96/100 已入账 `review-score-log.md`；`scripts/check-review-score-submit --base ad286a931e8785c30967e6edce1353d00e9a2423 --pr 632` PASSED；CI：Draft Round-1 Backend 5 failed——测试隔离根因：测试文件迁移改变目录执行序，e2e 提交的 park_investment_dd v1.0.0 残留被 run-router 409 容忍复用——经 `_clean` fixture teardown 镜像清理返修后，Round-2 / Ready / 评分后三轮 CI 全 SUCCESS，Backend full hermetic 3028 passed / 1 skipped / 10 deselected）；Slice C 交付（ADR-085-3）：skill_runner.py DD/QCC/尽调 persona 业务硬编码归零并泛化为注入式通用 runner（`step_params_mapper` / `report_persona` 构造注入点，默认业务中性），internal_query step 类型与 SkillStepResult.query_audit_id 契约保留（REQ-046 v2 业务方契约非 REQ-045 兼容），dd_query_runner.py 与 park_investment_dd.yaml R100 迁至 due_diligence，新增 skill_caller.py 唯一装配点保持 due_diligence → skill_registry 单向，4 个测试文件迁移；独立复审 P0=0 / P1=0 / P2=1 / P3=3：P2（本节 :103/:110/:538 三个历史行号锚点随 Slice C 失效）已于本 closeout 以 2026-09-15 基线快照注记修复；P3=3 保持保留未修复、绑定 TD-085 后续维护不新建编号（skill_runner.run_skill 未使用 request 参数 / e2e `_clean` teardown 在 aborted-transaction 窄路径下的局限 / v2 `_clean_tables` 仅 pre-clean 的存量顺序污染风险）；Slice D/E 未启动；TD-085 状态保持 🟡 进行中（未翻 🟢 完成）；REQ-043 仍 ⚫ Candidate / REQ-047 Extended 仍 🟣 Shaping / WS-S2 option B 仍 BLOCKED / WS-S3 仍 BLOCKED。
