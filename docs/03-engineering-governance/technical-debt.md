...
| TD-094 | R1-S5 scheduler 契约登记缺口：编排方 attempt 写者矩阵 + settlement 锁内 adapter I/O（需契约评审决策） | ⚫ 待办 | P1 | 后端 / Erasure / Scheduler / 契约 | R1-S5 root 广域三面复审（面A P0-1 校准 P1 + 面B P1-1，修复需修改冻结契约/写者矩阵，不自行裁决）：(1) orchestrator 持久写 checkpoint.attempt（scan 族 entry 前 +1，SCH-B e17d8ec1 返修裁决仅记录于模块 docstring）未登记进 S5-SCH-0/2 写者矩阵（plan :2123/:2191/:2159 字面冲突；无并发写者竞争、无运行时后果）——需 plan 回填登记或改 participant 承担；(2) settlement 窗口态 adapter I/O（lookup/replay）在 entry 事务持完整锁集内执行（E-2「禁持锁做外部 I/O」；S5-SCH-2 冻结 settlement 单事务未授权锁内 I/O）——当前 FailClosedAdapterResolver 不可达（潜伏态），接线真实 adapter 后锁持有不可控——需契约评审：无锁 adapter 调用（改单事务结构）或显式授权锁内 I/O + DB 超时配套 |
