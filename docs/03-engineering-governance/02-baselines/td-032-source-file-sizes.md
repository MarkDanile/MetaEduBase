...
| `packages/server-python/tests/composition/test_agent_erasure_schema.py` | 1486 | 🟢 已登记（待拆分） | REQ-041/047 R1-S1 schema 基座 + 三轮复审反例 + round4 复审 F1/F2/F3/F5/F10 变异杀手反例叠加（fence 单调守卫/registry drift 校验/failures 上界/tombstone 负向分支/purge_revision 校验），单文件聚合至 1486 行超硬限制。本次只登记新增风险，不在 R1-S1 中拆测试。后续应按测试主题拆成聚焦文件：(a) fence CRUD/CAS/版本守卫/单调守卫/并发唯一性；(b) tombstone（Message/Conversation/Run/CompatibilityOutput/outbox 正负分支）；(c) purge operation/checkpoint snapshot 绑定/registry drift；(d) backfill 游标/幂等/参数守卫/failures 上界/CLI 退出码；目标单文件 ≤500 行。 |
