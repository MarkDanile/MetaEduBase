...
| `scripts/validate_req024_p2_real_validation.py` | 1035 | 🟢 已登记（待拆分） | REQ-024 起算 ~600 行；REQ-026 扩 4 scenario 报告 + 关键事实覆盖度（+214 行）→ ~800 行；REQ-027 wrapper 不动此文件；REQ-028 三口径（substring / semantic / llm_judge）+ v3 样例支持（+215 行）→ 1035 行。本次只登记新增风险，不在 REQ-028 任务中拆脚本。后续应按 `load_questions` / `compute_*_coverage` / `render_report_section` / `argparse` 拆为 `validate_req024_p2_real_validation/` 包 4 文件（入口脚本 + 4 个聚焦模块），目标单文件 ≤500 行 |
