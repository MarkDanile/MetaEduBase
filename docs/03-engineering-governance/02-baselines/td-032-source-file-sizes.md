...
| `packages/server-python/tests/composition/test_agent_erasure_schema.py` | 1007 | 🟢 已登记（待拆分） | REQ-041/047 R1-S1 schema 基座 + 两轮复审反例叠加（fence/CAS/并发/tombstone/backfill/CLI 退出码），单文件聚合至 1007 行超硬限制。本次只登记新增风险，不在 R1-S1 中拆测试。后续应按测试主题拆成聚焦文件：(a) fence CRUD/CAS/版本守卫/并发唯一性；(b) tombstone（Message/Conversation/Run/CompatibilityOutput/outbox）；(c) purge operation/checkpoint snapshot 绑定；(d) backfill 游标/幂等/参数守卫/CLI 退出码；目标单文件 ≤500 行。 |
