...
|------|------|------|----|-------------------|----------|
| 2026-06-11 | TD-039 6 键保留集合在 TS 端抽到 `@metaedu/shared/schemas/document` + spec 单一来源落地 | 技术债 / 前端 / 共享 schema / 重构（single source of truth） | [PR TBD] |  | `docs/03-engineering-governance/technical-debt.md#td-039` / `packages/shared/src/schemas/document.ts` / `packages/web/src/views/resource/FileTabsPanel.vue` / `docs/03-engineering-governance/current-work.md` |
