...
| 2026-06-10 | TD-045 `ai_chat_service._call_llm` 漏 await 业务 bug 修复（Slice 3 遗留） | 技术债 / 后端 / AI Chat / 运行时 / async-await | [#185](https://github.com/MarkDanile/MetaEduBase/pull/185) |  | `docs/03-engineering-governance/technical-debt.md#td-045` / `packages/server-python/app/contexts/knowledge/application/ai_chat_service.py` / `packages/server-python/tests/contexts/knowledge/test_ai_chat_service.py` / `packages/server-python/tests/e2e/test_p1_rag_evidence_e2e.py` |
