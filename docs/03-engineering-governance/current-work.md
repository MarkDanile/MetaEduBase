...
|------|------|--------|------|----------|--------|------|
| REQ-021 浅色主题加入克制蓝色点缀 | 🟡 进行中 | P1 | Frontend / Theme / Design System | 分支 `codex/req-021-blue-accent-visual-polish`；主题 token、`.ui-btn-primary`、`app-brand-mark`、主布局 Logo、登录页品牌分栏和设计规则已同步 | 提交 PR 并合并；合并后回填 Done / work-log / 最近完成 | typecheck / lint / build / check-engineering-docs / diff check 通过；`/login` light 浏览器 smoke 通过；dark token 静态核验通过 |
