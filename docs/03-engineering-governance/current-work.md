...
下一步：S2-C 已提交待复审（PR #511，HEAD `250a8255`）——独立 `max` 复审 P0=0/P1=5/P2=2 已全返修并变异击杀：assistant projection 推进 body checkpoint、backfill 经 `ensure_fence_under_owner_lock` 消反向锁序、deleted list/search 映 410 fail-closed、create 带初始 title 推进 title ingress、migration 036 归一 baseline digest、verdict/advance 真正拆分、advance 原语自校验 source_key/watermark/epoch+FOR UPDATE。等待独立 `max` 复核（重点：已有 fence 锁序、assistant receipt 原子性、deleted 查询边界）+ Codex 复审。S2-C 契约注记/plan delta 已先于代码冻结（plan §R1-S2「S2-C 契约注记」2026-07-29，commit `c7bd356c`）：ingress_checkpoint 以 `workspace.core.v1` 能力类别为 canonical source key——`body_messages` 用 message `seq` 连续水位、`title` 用 Conversation `revision`，epoch 取 `purge_revision`，digest 走 shared `canonical_digest`；正文/正文写/checkpoint 同事务 commit，不用 `last_body_write_at`/fence revision 冒充；backfill 改造为逐 Conversation 经 `create_fence_under_owner_lock` 补 Conversation 行锁/owner lock 防 AB-BA。验证：S2-C 专项 18 + 复审专项全绿、全量 1846 passed、变异 M-P1-1/3/4/5、M-P2-6/7 击杀、ruff 0、mypy baseline 0 回归、migration 036 round-trip、docs gate + diff-check 通过。模型 Sol `xhigh` 主实现。

S2-C round-2 返修（独立 `max` round 2 P0=0/P1=3/P2=2）已落地：migration 036 重写为精确 legacy pair（checkpoint={} AND digest=LEGACY，不依赖 revision、未知 digest/非空 checkpoint 不动）+ downgrade 同还原两列；新增 036 数据矩阵测试**直接跑迁移模块真实 upgrade()/downgrade()**（RecordingBind 复用迁移 SQL，rev1/rev>1 legacy 归一、未知 digest 不覆盖、非空 checkpoint 不动、downgrade 两列正确），变异 M3/M4/M5（去 legacy-digest 守卫/漏还 digest/误依赖 revision）全击杀；`_to_conversation` 对 deleted 投影 redacted recovery envelope（title/created_by/archived_by/deleted_by→None、title_source→NONE，保留 id/state/revision/purge_after/deleted_at），变异 M1/M2 击杀；新增 P2-4 预建 committed fence 下 writer/backfill 无反向等待并发回归；过期陈述（require docstring、plan §3 初始 title/§4 deleted 投影、本文件 036 声称）已修订。等待独立 `max` 复核（重点：migration 数据矩阵、deleted recovery envelope）+ Codex 复审，**未合并**。
