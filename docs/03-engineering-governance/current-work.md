...
当前进展：M1-M4 已实现完成 + 前九轮独立复核全部修复闭合（第一轮 #1-#7 / 第二轮 #1-#7 / 第三轮 6 项 / 第四轮 4 项 / 第五轮 5 项 / 第六轮 3 项 / 第七轮 2 项 / 第八轮 5 项 / 第九轮 2 项：epoch orphan 独立解析 + external ref 绑定收敛）。**第十轮独立复审（当前）**：P1×1 + P2×1——#1 orphan+ref 合法行被绑定 verify 误判（生产登记 orphan 绑定 NULL，verify expected 却取幽灵 conv；改 expected 按完整来源解析 LATERAL：仅 resolved && !conflict 取源 conv，其余含 orphan 一律 NULL）；#2 RunEvent 绑定未验证且不重扫（verify 跳过 run_events + 扫描只选未登记；改 run_events expected = 行内 conv + 扫描含 blocked 错误绑定进入 heal，outbox 同缺口一并补）。十轮修复全部落地并有判别力回归测试（TDD red/green 已验证）。
下一步：PR #530 已建，当前 HEAD 见 Git 历史（本工作台不写死 SHA），第十轮修复随本次提交推送，等待第十一轮独立 max/Codex 定向复审，不自行合并。erase_available 保持 False。
验证状态：`uv run ruff check`（changed files 0 错误）/ `scripts/check_mypy_baseline.py`（243 historical，0 回归）/ `scripts/check-engineering-docs --full` 通过（32 allowlisted）/ `git diff --check` 干净；`pytest tests/composition/test_agent_transport_*.py` S4-B 专项 70 passed（含第十轮 2 新测试）；`pytest tests/composition/test_agent_erasure_migration_roundtrip.py test_agent_erasure_schema.py` 迁移 round-trip 54 passed；第十轮 2 测试 TDD red/green 已验证（旧逻辑 2 红、修复后全绿）；全量回归 `pytest tests/ -q -m "not external_network"` 2102 passed / 0 failed / 4 deselected（6m36s）。
