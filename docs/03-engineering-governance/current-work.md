...
下一步：round-1 三面首轮复审（P0/P1/P2/P3=3/12/11/4）按根因族 R1-R6 一次返修；round-2 落点修正；round-3（0/3/4/0）S1 turn 三源 CAS、S2 双事务协议、S3 orphan 可达性；round-4（0/3/1/0）S2 重写为状态表（unknown/stale 拆开、inbox 冻结 rejected、workspace outbox 无 migration 证据）；round-5（0/3/0/0）三项定向修正——stale 统一走双事务不 raise、epoch_unresolvable ledger 生命周期（resolved+证据+重放匹配）、状态表按 workspace/execution 展开 + execution decision_actor_id；round-6（0/1/1/0）重放锁后检查 outbox 精确终态三分支（ledger resolved 不代替 Tx2 终态）+ CI 状态同步。契约 P0/P1 清零达成，待一次轻量表格核对后开实现 PR（冻结为 producer propagation + replay/catch-up 与 claim/consumer CAS + deterministic terminalization 两 PR）。
验证状态：docs-only——`scripts/check-engineering-docs` + `git diff --check` 通过；PR #535 required CI 曾因基建/取消重跑被标 CANCELLED（非测试失败），待最新 HEAD 重跑 required checks 全绿后回填；不实现 migration 041、不启用 S5 scheduler。
