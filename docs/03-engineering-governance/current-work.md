...
当前进展：M1-M4 已实现完成 + 前八轮独立复核全部修复闭合（第一轮 #1-#7 / 第二轮 #1-#7 / 第三轮 6 项 / 第四轮 4 项 / 第五轮 5 项 / 第六轮 3 项 / 第七轮 2 项 / 第八轮 5 项：epoch-only 收敛 + 表↔issue 绑定 + external ref 绑定 + inbox 源 NULL + epoch class 精确）。**第九轮独立复审（当前）**：P1×1 + P2×1——#1 epoch 的 orphan 判据由 ledger 自证（伪 conversation_deleted_orphan + epoch orphan 让 live Conversation 的未知 epoch 通过 verify 且不触发 purge gate；改 verify epoch 维按来源矩阵 LATERAL 独立解析 expected class）；#2 external ref 错误绑定不收敛（ON CONFLICT DO NOTHING + verify 只查 ref 存在；改 blocked 记录安全 UPDATE 修正绑定 + verify 校验 expected conversation_id，冲突行豁免）。九轮修复全部落地并有判别力回归测试（TDD red/green 已验证）。
下一步：PR #530 已建，当前 HEAD 见 Git 历史（本工作台不写死 SHA），第九轮修复随本次提交推送，等待第十轮独立 max/Codex 定向复审，不自行合并。erase_available 保持 False。
验证状态：`uv run ruff check`（changed files 0 错误）/ `scripts/check_mypy_baseline.py`（243 historical，0 回归）/ `scripts/check-engineering-docs --full` 通过（32 allowlisted）/ `git diff --check` 干净；`pytest tests/composition/test_agent_transport_*.py` S4-B 专项 68 passed（含第九轮 2 新测试）；`pytest tests/composition/test_agent_erasure_migration_roundtrip.py test_agent_erasure_schema.py` 迁移 round-trip 54 passed；第九轮 2 测试 TDD red/green 已验证（旧逻辑 2 红、修复后全绿）；全量回归 `pytest tests/ -q -m "not external_network"` 2099 passed / 0 failed / 4 deselected（7m01s；1 个 S3-E 单测在共享库偶发失败、隔离与文件级重跑均通过，确认非本改动回归）。
