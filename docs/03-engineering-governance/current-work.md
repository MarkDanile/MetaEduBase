...
交接备注：S3-A 已合并（PR #515）；S3-B 已合并（PR #517）；S3-C fenced port 在 composition 层（commit-12 加 GuardLockPort 拆分跨边界）。run_query_service.request_cancel 锁链严格：Guard -> Conv -> fenced writer 内部 AgentRun FOR UPDATE + cancel intent CAS，与 S3-D `Conversation -> owner/fence -> AgentRun` 同序无 AB-BA。fenced_ingest_runtime_event 形态已就位（Runtime adapter 推迟到 S4）。
