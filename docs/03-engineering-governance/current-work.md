...
| 日期 | 任务 | 状态 | 摘要 | 事实源 |
|------|------|------|------|--------|
| 2026-06-12 | TD-051 `document_chunks` 结构元数据治理 + 历史数据重建 | 🟢 完成 | 7 slice 全完成：sections 保留到 structured_data（S1）；chunk_document 读真实 sections 而非 regex 猜（S2）；`_enforce_size_limit` 传递 `char_start/end`（S3）；chunk size 策略文档化 + `MIN_CHUNK_CHARS` 常量（ S4）；`rebuild_document_chunks` 独立 rebuild 入口（ S5）；`cleanup_orphan_chunks` 清理孤儿 chunk（ S6）；11 个新 pytest 锁死 chunker 行为（ S7）。67 passed，ruff clean。 | [Spec](../02-delivery-plans/01-specs/2026-06-12-td-051-document-chunks-metadata-governance.md) / [Plan](../02-delivery-plans/02-plans/2026-06-12-td-051-document-chunks-metadata-governance-plan.md) / 分支 `refactor/td-051-document-chunks-metadata` |
