...
当前进展：M1-M4 已实现完成 + 前七轮独立复核全部修复闭合（第一轮 #1-#7 / 第二轮 #1-#7 / 第三轮 6 项 / 第四轮 4 项 / 第五轮 5 项 / 第六轮 3 项 / 第七轮 2 项：issue_code↔class 绑定 CHECK + verify 精确匹配 + 工作台稳定表述）。**第八轮独立复审（当前）**：P1×4 + P2×1——#1 epoch-only 行（scope 已填 + epoch NULL）不被扫描、无法收敛（新增扫描分支补登 epoch_unresolvable）；#2 source_table 与 issue_code 未绑定（新增 CHECK `ck_agent_transport_reconcile_source_issue` + verify 按表精确匹配）；#3 非 resolved 行 external ref 错误继承行内 scope（改仅 resolved/conflict bind，其余 NULL）；#4 inbox 源 outbox scope NULL 被 discovery/verify 跳过（mismatch 含源 scope NULL + had_scope+source_missing 分支登记）；#5 epoch 的 tenant_scope/orphan 互换（verify epoch 维按 orphan/resolved/其余三分支精确匹配）。八轮修复全部落地并有判别力回归测试（TDD red/green 已验证）。
下一步：PR #530 已建，当前 HEAD 见 Git 历史（本工作台不写死 SHA），第八轮修复随本次提交推送，等待第九轮独立 max/Codex 定向复审，不自行合并。erase_available 保持 False。
验证状态：`uv run ruff check`（changed files 0 错误）/ `scripts/check_mypy_baseline.py`（243 historical，0 回归）/ `scripts/check-engineering-docs --full` 通过（32 allowlisted）/ `git diff --check` 干净；`pytest tests/composition/test_agent_transport_*.py` S4-B 专项 66 passed（含第八轮 5 新测试 + 1 DB CHECK 测试）；`pytest tests/composition/test_agent_erasure_migration_roundtrip.py test_agent_erasure_schema.py` 迁移 round-trip 54 passed；第八轮 5 测试 TDD red/green 已验证（旧逻辑 4 红、修复后全绿）；全量回归 `pytest tests/ -q -m "not external_network"` 2098 passed / 0 failed / 4 deselected（6m45s）。
