...
下一步：S2-D/E round-3 返修已提交待复审（PR #513，复审代码/测试基线见 Git 历史）--独立 `max` round 3 P0/P1/P2=0/4/2，6 项已按 Sol `xhigh` 返修：P1-1 blocked 重试状态一致（`_mark_operation_running` 推进 scheduled/blocked->running + 清 failure_code + bump revision，是 failure_code 唯一清除点）、P1-2 operation revision replay fencing（`expected_operation_revision` 必填 + `_load_verified_operation` revision CAS + 状态变化 bump revision）、P1-3 erased repair 安全（erased 重放校验 scan=0；`_repair_checkpoint_if_pending` 校验 operation 非终态）、P1-4 actor secret 强度+版本契约（`actor_erasure_secret_version` 混入 HMAC key + lifespan/构造期双重 >=32 字符强度校验）、P1-5 移除公开 `now` 参数（始终用 `clock_timestamp()`）、P2 owner_version 去硬编码（`_record_blocked`/`_repair_checkpoint_if_pending` 用 `fence.owner_version`）。返修落点见 plan §R1-S2「S2-D/E round-3 复审修订」块。验证：S2-D/E round-3 专项 40 passed（含 7 场景表驱动 fencing 反例 +operation revision CAS+ + erased 非零 scan fail closed + erased 终态 operation fail closed + blocked 重试状态一致 + secret 强度/版本/启动校验 + now 不被接受 + owner_version 去硬编码）、8 项 round-3 变异全击杀、ruff 0、mypy 0 回归、docs gate + diff-check 通过。等待独立 `max` 只读复核，**未合并**。遗留前置（不提前做）：S3 purge worker claim 顺序 Guard->Conversation row->owner lock->fence->operation；dispatch_output 对 `LateBodyWriteRejectedError` 分类为 deterministic；hold 生效中 restore 测试随 hold slice 补齐（spec R1-AC7）。
