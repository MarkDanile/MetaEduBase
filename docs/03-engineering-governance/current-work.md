...
当前进展：round-2 复审（P0/P1/P2/P3=0/1/2/0）返修完成。P1 非-claimed 状态不再无条件幂等成功——仅完整匹配既有 late-write 终态（cancelled + late_body_write_rejected + digest 匹配 + Run suppressed）才 no-op，pending/dead_letter/published/其他 cancelled 一律 fail closed（修 takeover 回 pending 后 stale worker 静默吞掉）；P2 P1 反例改跑真实 ExecutionErasureParticipant（发现真实根因：purge-fenced output projection 走 InvalidConversationStateError transient 路径 + redacted CompatibilityOutput read 抛 RunConflictError，均在 fence 裁决前 -> 新增 InvalidConversationStateError→LateBodyWriteRejectedError 重分类 + 新 LateOutputReadRejectedError 统一 deterministic）；删除 plan/工作台错误的 S4 defer（真实 eraser 对 completed+pending 无 CHECK 冲突，系原子 suppressed+清字段）；P2 工作台改稳定非自指表述。新增 takeover-pending 反例，P1 round-2 变异 KILLED。Plan §R1-S3「S3-E round-2 复审修订」delta 已冻结。
下一步：全量回归绿 -> 提交 -> 三路 CI -> 独立 max/Codex 轻量复核；登记全量 flake 为 TD-091（独立于本 Slice，TD-080 编号已占用）。不自行合并。
验证状态：S3-E 全部测试（§8 2 + 无旁路 2 + race 6 + backfill 1 + round-1/2 4 = 15 例）全绿；P1/P2 变异逐项 KILLED；control-plane+composition 回归进行中；全量回归进行中。三路 CI 基线以 PR #524 当前 HEAD 为准（见 Git 历史，不钉漂移 SHA/时长）；本地全量套件存在 main 预存的非确定性隔离 flake（干净 main 对照 27 failed：test_ai_chat/direct_rag/e2e/turn_bridge/writer_fence），与本 Slice 无关，已登记 TD-091 独立处理。
