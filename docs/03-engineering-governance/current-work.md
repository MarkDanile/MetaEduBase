...
交接备注：S3-A 已合并（PR #515）；S3-B 已合并（PR #517）；S3-C fenced port 在 composition 层（commit-5 加 Protocol 拆分）；erase_available 保持 False（S3-D 翻）。commit 5 把 cancel_intent CAS 下推后，repository 层 transition_run/commit_terminal 接受可选 cancel_intent_revision 参数（向后兼容默认 None）。
