...
|------|------|--------|------|----------|--------|------|
| DOC-060 增强 `scripts/engineering/checks/` 任务卡 vs 代码语义校验（任务卡残留量 + 任务卡完成状态 2 类不符） | 🟡 进行中 | P2 | 工程脚本 / 质量门禁 | DOC-057 收口已完成（[PR #204](https://github.com/MarkDanile/MetaEduBase/pull/204) / merge `f1a8bd0`），按用户选择"DOC-057 完成后继续推 DOC-060"开始本任务。任务卡详见 [technical-debt.md#doc-060](technical-debt.md#doc-060)。修复要求：在 `scripts/engineering/checks/_common.py` 加 `check_task_card_claim_vs_code` 通用函数 + 新建 `scripts/engineering/checks/task_card_claims.py` 注册 2 个 check（`task_card_stale_completion` + `task_card_stale_residual`） + 新增 pytest 锁定。 | 1. 开分支 `docs/doc-060-task-card-claim-vs-code`；2. 实现脚本 + pytest；3. 跑门禁 / git diff --check / pytest；4. commit + push + 开 PR；5. 合 main + 按翻完成硬条件收口 | 待跑：脚本新增 check 后 `python3 scripts/check-engineering-docs` 退出码不新增失败项 + `git diff --check` clean + `pytest tests/engineering/` 全过 |
...
| 日期 | 任务 | 状态 | 摘要 | 事实源 |
|------|------|------|------|--------|
| 2026-06-12 | DOC-057 历史"验证通过声明缺可复核证据"格式收口（technical-debt 总账版；与 2026-06-10 归档的"DOC-057 建立并行开发模式与集成收口规则"是同名不同事） | 🟢 完成 | 1 docs-only PR（#204 / merge `f1a8bd0` / 分支 `docs/doc-057-validation-claim-evidence`，已删）收口。修复要求在 main 上已自然满足（DOC-058 PR #202 / TD-049 PR #200 等历史任务合 main 时补齐 current-work.md L37-L40 的 `gh pr view` / `退出码 0` 命中；`scripts/check-engineering-docs` 当前 0 条 `validation-claim` issue）；本轮仅按任务卡交付项收口：技术债总账 L148 翻 🟢 完成 + L1948 任务卡补 PR 链接 + work-log 索引行追加 DOC-057 行 + current-work 当前进行中翻完成。`git diff --check` clean；`scripts/check-engineering-docs` 退出码 1（6 条 pre-existing 警告与本任务无关）；0 业务代码 / 0 测试代码 / 0 脚本变更；`gh pr view 204` state=MERGED + merge commit `f1a8bd0` 已在 main。 | [DOC-057](technical-debt.md#doc-057) / PR #204（merge `f1a8bd0`） |
| 2026-06-12 | DOC-058 显式加"任务分支未合 main 不得翻 🟢 完成；`gh pr view <PR>` state 必须为 MERGED"规则（workbench.md + git-workflow.md + quality-gates.md） | 🟢 完成 | 1 docs-only PR（#202 / merge `8b0ceb8` / 分支 `docs/doc-058-branch-merge-required-to-complete`）收口。`workbench.md#状态同步规则` 末尾追加硬规则段（任务分支未合 main 不得翻完成）；`git-workflow.md#完整交付闭环` 6 阶段后追加"翻完成前硬条件"段（state=MERGED / pr checks 无阻塞 / main pull --ff-only / merge-base 4 条）；`quality-gates.md#完成门禁#3` 状态项补"任务分支未合 main 视为未走完 Git 阶段"。`scripts/check-engineering-docs` 退出码 0（本任务新增 0 警告）；20 pytest passed 零回归；`git diff --check` clean；0 业务代码 / 0 测试代码 / 0 脚本变更（DOC-059 负责脚本维度）。`gh pr view 202` state=MERGED，merge commit `8b0ceb8` 已在 main。 | [DOC-058](technical-debt.md#doc-058) / PR #202（merge `8b0ceb8`） |
