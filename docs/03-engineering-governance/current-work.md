...
当前进展：实现完成 + 首轮三面返修（族 A~E：blocked+NULL dirty-data、case-E 非 active fence 分态、六项 item 5 ingress、re-added 全函数分派、ACK-lost 门禁、registry 一致性守卫、幂等谓词补 hold/registry）；19 专项全绿；composition 全量 602 passed（**存在环境级偶发 flaky：Docker Desktop macOS 卷 + asyncpg 快速连接下 INSERT+COMMIT 偶发全局丢失，非本 PR 逻辑缺陷，登记 infra 核实**）；mutation kill 驱动入库；ruff/mypy 0 regressions。
下一步：独立定向复核（P1 清零核验）→ 通过后 stacked Draft PR（base = SCH-B root）+ checks 全绿后停止。
验证状态：composition 602 passed（137s，偶发 flaky）；mypy 243 historical / 0 regressions；docs gates exit 0；三面首轮原始计数保留（P0=0/P1=9/P2=11/P3=10）
