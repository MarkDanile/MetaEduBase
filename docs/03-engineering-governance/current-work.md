...
下一步：进入 S2-C（list/get/search/history 对 deleted/purged fail-closed + title/create 接 fence/Guard + ingress checkpoint 真实 source key + backfill 命令锁序补齐）。S2-C 契约注记/plan delta 已先于代码冻结（plan §R1-S2「S2-C 契约注记」2026-07-29）：ingress_checkpoint 以 `workspace.core.v1` 能力类别为 canonical source key——`body_messages` 用 message `seq` 连续水位、`title` 用 Conversation `revision`，epoch 取 `purge_revision`，digest 走 shared `canonical_digest`；正文/正文写/checkpoint 同事务 commit，不用 `last_body_write_at`/fence revision 冒充；backfill 改造为逐 Conversation 经 `create_fence_under_owner_lock` 补 Conversation 行锁/owner lock 防 AB-BA。模型 Sol `xhigh` 主实现。
