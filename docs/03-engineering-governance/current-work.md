...
下一步：S2-D/E round-5 返修已提交待复审（PR #513，复审代码/测试基线见 Git 历史）--独立 `max` round 5 P0/P1/P2=0/2/1，3 项已按 Sol `xhigh` 返修：P1-1 ACKed checkpoint 绕过 operation 修复（`_repair_checkpoint_if_pending` ACKed digest 验证后不早 return，fall through 到统一 operation 修复块，ACKed+blocked/scheduled 矛盾组合修复）、P1-2 冻结 version 未冻结 secret（新增 migration 037 `system_key_fingerprints` 表 + `_actor_erasure_key_fingerprint` + `validate_production_actor_erasure_key_fingerprint` 启动期 upsert 持久化 fingerprint，首次锁定 / 不一致 fail closed 检测 secret 静默替换；构造器生产禁覆盖 audit_secret/audit_secret_version）、P2 模块文案与 V1 冻结契约冲突（docstring + error message 统一改为 migration 落地前不可轮换）。返修落点见 plan §R1-S2「S2-D/E round-5 复审修订」块。验证：S2-D/E round-5 专项 52 passed（+6 round-5 测试）、6 项 round-5 变异全击杀、ruff 0、mypy 0、migration roundtrip+schema 56 passed、agent_control_plane+composition+identity jwt 234 passed、全量回归通过、docs gate + diff-check 通过。等待独立 `max` 只读复核，**未合并**。遗留前置（不提前做）：S3 purge worker claim 顺序 Guard->Conversation row->owner lock->fence->operation；dispatch_output 对 `LateBodyWriteRejectedError` 分类为 deterministic；hold 生效中 restore 测试随 hold slice 补齐（spec R1-AC7）；digest version 持久化 migration（round-4 P1-4 V1 冻结解除前置，round-5 fingerprint 为过渡强制）。
