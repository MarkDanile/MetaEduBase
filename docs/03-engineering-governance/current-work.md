...
状态：🟡 进行中（Draft PR #592；定向复核 P0/P1=0、P1-A/P1-B CLOSED，实现与复核闭合**待合并**；TD-106 合并前不标完成）
...
下一步：定向复核 P0/P1=0 已过 → 冻结 FINAL_IMPL_HEAD → `gh pr ready 592` → 等 Backend full/Engineering docs/Frontend 三路 required checks 全绿 → 正式评分（新增唯一 `#592 Original` 行，记录 stacked base=aa889db8 + FINAL_IMPL_HEAD，不改历史行/Metrics；`scripts/check-review-score-submit --base FINAL_IMPL_HEAD --pr 592`）。评分后仍不合并、不建 closeout；不触碰 #590/#591/migration 043/schema/enum/CHECK/门禁脚本/KNOWN_ISSUES/CI；不启动 PR-D/E/C1/S5 production wiring/registry capability 翻转。
