...
下一步：S2-D/E round-2 返修已提交待复审（PR #513，复审代码/测试基线见 Git 历史）--独立 `max` round 2 P0/P1/P2/P3=0/5/4/1，5 个 P1 阻塞项已按 Sol `xhigh` 返修：P1-1 capability gate 放行（registry `workspace.core.v1 erase_available=True`，`require_capability` 不再被绕过）、P1-2 actor HMAC secret 隔离（独立 `settings.actor_erasure_secret`，非 jwt_secret，生产 fail-fast、版本固定）、P1-3 operation/checkpoint 完整 fencing（`purge_operation_id` 必填；校验 conversation_id/purge_revision/lease_epoch/registry_digest/hold_revision_snapshot + checkpoint owner_version/capability_digest CAS；跨 Conversation 误 ACK 防护）、P1-4 operation 投影 + erased fence 恢复（scheduled->running 投影；erased fence 幂等重放先于 purge 前置，`_repair_checkpoint_if_pending` 补 ACK 丢失）、P1-5 final scan + 幂等清除完整性（scan 含 archived_by/deleted_by；`_redact_messages` 选 `or_(body_state=present, author_id IS NOT NULL)` 清所有 author_id 残留）。P2-1 scan tenant 谓词、P2-2 blocked 路径 scan digest、P2-3 PostgreSQL `clock_timestamp()`、P2-4 `purge_blocked_by_legal_hold` reason code。返修落点见 plan §R1-S2「S2-D/E round-2 复审修订」块。验证：S2-D/E round-2 专项 32 passed（含 6 场景表驱动 fencing 反例 + erased fence pending checkpoint 恢复 + scan archived_by/deleted_by + 已 redacted author_id 残留清除 + DB 时钟 + tenant 谓词 + 生产 fail-fast）、全量 1881 passed、18 项变异全击杀、ruff 0、mypy 0 回归、docs gate + diff-check 通过。等待独立 `max` 只读复核，**未合并**。遗留前置（不提前做）：S3 purge worker claim 顺序 Guard->Conversation row->owner lock->fence->operation；dispatch_output 对 `LateBodyWriteRejectedError` 分类为 deterministic；hold 生效中 restore 测试随 hold slice 补齐（spec R1-AC7）。
