...
下一步：S2-D/E round-4 返修已提交待复审（PR #513，复审代码/测试基线见 Git 历史）--独立 `max` round 4 P0/P1/P2=0/5/0，5 项已按 Sol `xhigh` 返修：P1-1 legal-hold 路径 operation revision CAS（`_record_blocked` 新增 `expected_revision` 透传，legal-hold 路径裁决 stale revision 零状态变更 fail closed）、P1-2 legal-hold blocked 投影 + reason change bump（legal-hold 同事务投影 `Conversation.purge_state=blocked`；`_record_blocked` 已 blocked + reason 变化更新 failure_code + bump revision）、P1-3 erased repair 矛盾事实 fail closed（ACKed checkpoint 验证 ack_digest/checkpoint_digest 与 fence/scan 一致；blocked operation 推进 running 不只清 failure_code；erased 重放修复 purge_state 三方一致）、P1-4 actor digest V1 版本冻结（表未持久化 digest version，生产 `actor_erasure_secret_version` 冻结 1 禁止轮换，直至 migration 落地；lifespan/构造期双重校验 version!=1 -> RuntimeError）、P1-5 agent author digest 全类型（`_redact_messages` 对所有 author_id is not None 计算 digest，不再限定 author_type=="user"，兑现 plan:255 契约）。返修落点见 plan §R1-S2「S2-D/E round-4 复审修订」块。验证：S2-D/E round-4 专项 46 passed（+7 round-4 测试）、8 项 round-4 变异全击杀（含唯一锚点修正）、ruff 0、mypy 0、agent_control_plane+composition+identity jwt 228 passed、全量 `pytest -m 'not external_network'` 1895 passed / 0 failed、docs gate + diff-check 通过。等待独立 `max` 只读复核，**未合并**。遗留前置（不提前做）：S3 purge worker claim 顺序 Guard->Conversation row->owner lock->fence->operation；dispatch_output 对 `LateBodyWriteRejectedError` 分类为 deterministic；hold 生效中 restore 测试随 hold slice 补齐（spec R1-AC7）；digest version 持久化 migration（round-4 P1-4 V1 冻结解除前置）。
