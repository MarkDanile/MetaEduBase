...
|------|------|--------|------|----------|--------|------|
| REQ-002 模板化结构抽取能力的配置与复用体验 | 🟣 Shaping | P2 | 需求 / 结构抽取 / 配置体验 | 现状盘点完成：模板 CRUD / AI 初始化 / 嵌套字段编辑器 / doc_type 冲突检查均已落地；塑形文件 `docs/01-product-planning/05-requirements/REQ-002-template-config-and-reuse.md` 已建；backlog `下一步` 已更新；范围划为"配置效率 / 复用机制 / 可观测性 / 可维护性"四象限 + 待回答问题。当前在 `docs/req-002-template-shaping` 分支走塑形收口。 | 待用户对"待回答问题"做选择后再拆分为子任务 / spec / plan；当前候选保留塑形状态 | 待运行：`scripts/check-engineering-docs`（塑形收口后跑） |
