...
交接备注：S3-A 已合并（PR #515）；S3-B 已合并（PR #517）；S3-C fenced port 在 composition 层（不违反跨上下文边界）；erase_available 保持 False（S3-D 翻）；不进 S4；不启用 purge scheduler。fenced_* wrapper 不强制 Guard 持锁（hotfix-2 修复 deadlock），由 caller 路径（dispatch_turn 的 consume_turn_event、direct_rag 的 _acquire_write_guard）按 Spec §6.1 自然持 Guard；fence verdict + advance 仍由 wrapper 统一提供。
