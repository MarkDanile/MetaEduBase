...
### TASK-R1-S6-I3-D: 独立 ledger export/archive + restore replay executor + restore-before-open runbook（**事实审计 + D1a bounded/严格化完成；D1b/D2/PR-D 整体未启动** — 不宣称 executor/archive/runbook 已交付）

状态：🟡 进行中（2026-08-27，**D1a bounded + 严格化已完成 + Draft PR 三路 required checks 全绿**；**禁止本轮进入 Ready / 评分 / 合并 / closeout**；用户裁决 5 项已落实：runtime per-binding proof = c / D1b = 专用 MinIO archive bucket / D2 = A advisory lock / D1a-D1b-D2 独立 PR / 顺序 D1a → D1b → D2）
最新交接（2026-08-27）：**任务卡登记 + 独立分支 + Draft PR + 事实审计 + D1a 实施 + D1a P1 bounded/严格化**。本任务卡严格声明：当前已完成 **D1a bounded read-only ledger snapshot codec**（只读 codec + bounded segment + decoder 多类 fail-closed + reconstruct），**尚未启动** ledger export executor / archive sink / restore replay executor / restore-before-open runbook。Plan §S6-8 / §S6-12 / §S6-13 / §S6-14 已冻结的 ledger export 与 restore replay 路由表 + 判定方式为唯一事实源；旧 PR-D scaffold（`s6i3_ledger_export.py` / `s6i3_restore_replay.py` / `restore-before-open.md` 等）已随 #586 scope 收敛撤回，本任务从冻结契约与当前 main 重新推导，**禁止 cherry-pick / 恢复 / 复制旧 `c07c031c` scaffold**。`S6I2_PENDING_WRITERS` 中 `restore_replay_executor` 仅字符串 pending 登记（PR #584 I3 merge `ad7ac3e5` 后），待独立 PR-D 落地后转 `registered`。
分支：`feature/req041-047-r1-s6-i3-d-ledger-restore-replay`（从 main `aff54883` 创建）
当前执行模式：D1a bounded/严格化已落 → 本轮禁止 Ready/评分/合并/closeout；剩余 D1b/D2/PR-D executor/archive/runbook 仍属本任务后续接力
最近接手工具：Claude Code
...
下一步（D1a P1 已落实；D1b/D2/PR-D 仍未启动）：
...
- **Phase 1 D1a only（已落 + P1 bounded/严格化已落）**：
  ...
  - **D1a P1 bounded/严格化（2026-08-27 用户裁决，本轮已落）**：
  ...
  - **真实 PG 测试覆盖**：28 项 D1a 专项（15 旧 + 11 新 + 2 mutation 配套）—— bounded limit / RR-RR-RO / RR-RC 拒绝 / RR-RR-RW 拒绝 / 两 op 同 owner_key 保留 / operation.state 越界 / checkpoint.state 越界 / ACK 64-hex 缺失 / records 乱序 / unknown kind / runtime proof 缺失 / runtime proof True / count 篡改 / cross-tenant 篡改 / ref_value 不泄露（precondition: external_ref 非空）/ 其余原 15 项
  - **Mutation kill**：10/10 真红（M1 tenant WHERE bypass / M2 ref_value 泄露 / M3 digest bypass / M4 count bypass / M5 schema_version bypass / M6 kind-table bypass / **M7 改用反转 records（替代原 sorted 绕过，因 SELECT 已 ORDER BY id，原 mutation 不可观察）** / M8 duplicate identity bypass / M9 cross-tenant bypass / M10 runtime proof false-positive）
  - **本地验证**：D1a 专项 28/28 pass；mutation kill 10/10 KILLED；ruff clean / mypy baseline 0 / git diff --check clean；**本地 composition 全量测试进行中**（pending）
