...
当前进展：M1-M4 已实现完成。M1 migration 040 + ORM（B1 四表 scope 列 + reconcile/external 两 ledger + inbox tombstone，13 测试）-> M2 版本化 aggregate advisory-lock（前缀 metaedu.agent.transport.agg.v1 与 guard/owner 分域 + D8 锁序，7 测试）-> M3 分批/tenant 限流/可恢复 backfill（B2 来源矩阵 + B3 epoch 保持 NULL + B4 三态 reconcile 集合锁 + B5 external ledger + B7 四维 verify，8 测试）-> M4 run_events 独立 ref 路径 + 并发/中断/跨源测试（3 测试）。第二轮独立复核 7 项（P1×4/P2×2/P3×1）已全部修复：#1 040 downgrade 一律 fail-closed（单步+全链，删全链放行，B8 原义）+ #2 external ref 按 ref_value 匹配（scan+verify）+ #3 取消跨调用游标仅 tenant 起点重扫 + #4 已带 scope 与源解析值冲突登记 conversation_scope/ambiguous_mapping（带解析值 B）+ #5 verify 第四维投影↔ledger 一致性 + 空集 ``WHEN count(*)=0 THEN NULL`` 修复（recompute+verify）+ #6 B2 类型约束 mismatch 行路由到 tenant_scope/ambiguous_mapping（删 verify type_filter，B7:817/B3:778 无类型豁免）。根治本地全量回归交叉污染：test_s3d_run_event_guard 手动 stamp 039->当前 head、structured_data head 断言 039->040、040 schema 测试 tenant 自给自足。
下一步：PR #530 已建，第二轮独立复核 7 项已全部修复并补真实反例回归测试（#1-#6 各一 + 多表重扫 + 投影漂移 + 零 issue 干净行），Backend 全量回归 2074 passed / 0 failed、ruff/mypy/docs gate/diff-check 全绿、5 修复点变异全 KILLED，待下一轮独立 max/Codex 定向复审（含上述真实反例），不自行合并。erase_available 保持 False。
验证状态：ruff check / mypy baseline（0 回归）/ docs gate / git diff --check 全绿；迁移 round-trip 53 passed（含 13 个全链降级<040 的预存迁移测试，新 fail-closed 未误伤）；S4-B 专项 42 passed；第二轮修复点变异验证全 KILLED（#1 040 ledger 守卫 / #2 ref_value 匹配 / #4 冲突登记 / #5 空集守卫 / #6 mismatch 路由）；全量回归（fresh 库 pytest tests/）2074 passed / 0 failed / 4 skipped。
