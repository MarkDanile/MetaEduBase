...
验证状态：纯文档；docs gate + `git diff --check` 通过；三路 CI 全绿（对应 PR #528 当前 HEAD，见 Git 历史，不钉漂移 SHA/时长）。round-1 复审（0/5/4/0）、定向复审（0/4/3/0）、两轮轻量复核（0/2/2/0、0/1/1/0）、两轮定向复核（0/1/0/0、0/1/0/0）均已就地修订并落成 round-1..6 注记：gate 改 state<>'resolved'、issue_code 拆 scope/epoch 类 + class/scope CHECK + 多 issue 投影聚合 + 事务级 advisory lock 集合锁（不依赖 reconcile 子行或源 transport 行存在）、epoch_unresolvable 按 scope 状态三分归类、ref_scheme allowlist 冻结为空、041 guard 覆盖全持 ref 旧状态、弃 UUID max 高水位改 S4-C 幂等全量重扫、verify 分 scope/epoch 双维度各匹配具名 issue、ledger 增 revision 乐观锁、issue_code/owner_key/blocked_reason 封闭枚举 CHECK、downgrade fail-closed。
