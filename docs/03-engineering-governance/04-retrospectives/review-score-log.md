...
| 指标 | 当前值 | 说明 |
|------|--------|------|
| 已记录评审数 | 5 | 1 条 Original + 4 条 Backfilled。 |
| 平均评分 | 81 | `总分` 平均值；样本含回溯评分。 |
| 一次关闭率 | 20% | 评分 >= 80 且无必修 follow-up 的任务数 / 已记录评审数。 |
| 返工率 | 80% | 有必修 follow-up 的任务数 / 已记录评审数。 |
| 流程违规率 | 80% | 存在事实源、spec / plan、PR、验证声明或状态同步扣分的任务数 / 已记录评审数。 |
| 规则转化率 | 40% | TD-032 / REQ-008 相关问题已转为规则、文档或脚本门禁改进。 |
