<template>
  <div class="p-8 max-w-[1200px] mx-auto">
    <div class="flex items-start justify-between mb-8 animate-slide-up">
      <div>
        <h1 class="text-[28px] font-semibold tracking-tight" style="font-family: var(--font-display)">校本资源</h1>
    ...
    <div v-else-if="items.length === 0" class="py-20 text-center animate-slide-up stagger-1">
      <div class="w-16 h-16 mx-auto mb-4 rounded-[var(--radius-lg)] bg-gradient-to-br from-[var(--color-bg-mesh-2)] to-[var(--color-bg-mesh-4)] flex items-center justify-center">
        <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="var(--color-accent)" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"/><polyline points="17 8 12 3 7 8"/><line x1="12" y1="3" x2="12" y2="15"/></svg>
      </div>
      <p class="text-[var(--color-ink-tertiary)] text-[14px]">暂无资源</p>
    ...
    <div v-else class="space-y-3">
      <div
        ...
        :key="item.id"
        class="liquid-card p-5 group animate-slide-up"
        :class="[`stagger-${Math.min(i + 1, 5)}`]"
      >
        <div class="flex items-center justify-between gap-4">
          <div class="flex items-center gap-3 flex-1 min-w-0">
            <div class="w-10 h-10 rounded-lg flex items-center justify-center flex-shrink-0" :class="typeIconClass(item.resource_type)">
              <div v-html="typeIcon(item.resource_type)" />
            </div>
            <div class="flex-1 min-w-0">
              <div class="flex items-center gap-2">
                <span class="font-medium text-[15px] text-[var(--color-ink)] truncate">{{ item.title }}</span>
              ...
              <div class="flex items-center gap-3 mt-1 text-[12px] text-[var(--color-ink-tertiary)]">
          ...
          <div class="flex items-center gap-2 opacity-0 group-hover:opacity-100 transition-opacity duration-200">
            <button @click="downloadResource(item)" class="liquid-btn liquid-btn-ghost text-[12px] py-1.5 px-3">
              <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"/><polyline points="7 10 12 15 17 10"/><line x1="12" y1="15" x2="12" y2="3"/></svg>
              下载
            </button>
            <button @click="deleteResource(item.id)" class="liquid-btn liquid-btn-ghost text-[12px] py-1.5 px-3 !text-[var(--color-danger)] hover:!bg-[rgba(239,68,68,0.06)]">
              <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><polyline points="3 6 5 6 21 6"/><path d="M19 6v14a2 2 0 0 1-2 2H7a2 2 0 0 1-2-2V6m3 0V4a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v2"/></svg>
              删除
            </button>
          </div>
        </div>
        <p v-if="item.description" class="text-[12px] text-[var(--color-ink-tertiary)] mt-2 ml-[52px] truncate">{{ item.description }}</p>
      </div>
    </div>
    ...
    <div v-if="showUploadDialog" class="liquid-dialog-overlay" @click.self="showUploadDialog = false">
      <div class="liquid-dialog">
        <h3 class="text-[18px] font-semibold mb-6" style="font-family: var(--font-display)">上传资源</h3>
        <form @submit.prevent="uploadResource" class="space-y-5">
          <div>
            <label class="block text-[13px] font-medium text-[var(--color-ink-secondary)] mb-1.5 ml-1">文件</label>
            <div class="liquid-input flex items-center cursor-pointer" @click="($refs.fileInput as HTMLInputElement)?.click()">
              ...
              <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="var(--color-ink-tertiary)" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"/><polyline points="17 8 12 3 7 8"/><line x1="12" y1="3" x2="12" y2="15"/></svg>
            </div>
          ...
          <div>
            <label class="block text-[13px] font-medium text-[var(--color-ink-secondary)] mb-1.5 ml-1">类型</label>
          ...
          <div>
            <label class="block text-[13px] font-medium text-[var(--color-ink-secondary)] mb-1.5 ml-1">专业域（可选）</label>
...
function typeIcon(type: string) {
  const icons: Record<string, string> = {
    document: '<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/><polyline points="14 2 14 8 20 8"/></svg>',
...
function typeIconClass(type: string) {
  const map: Record<string, string> = {
    document: "bg-gradient-to-br from-[var(--color-bg-mesh-2)] to-[var(--color-bg-mesh-4)] text-[var(--color-tag-green-text)]",
    video: "bg-gradient-to-br from-[var(--color-bg-mesh-1)] to-[var(--color-bg-mesh-2)] text-[var(--color-accent)]",
    image: "bg-gradient-to-br from-[var(--color-bg-mesh-3)] to-[var(--color-bg-mesh-1)] text-[var(--color-tag-amber-text)]",
    audio: "bg-gradient-to-br from-[var(--color-bg-mesh-4)] to-[var(--color-bg-mesh-2)] text-[var(--color-success)]",
    other: "bg-gradient-to-br from-[var(--color-bg-mesh-1)] to-[var(--color-bg-mesh-3)] text-[var(--color-tag-purple-text)]",
  };
...
function typeTagClass(type: string) {
  const map: Record<string, string> = {
    document: "liquid-tag-blue",
    video: "liquid-tag-purple",
    image: "liquid-tag-amber",
