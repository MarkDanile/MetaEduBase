...
import { FileSpreadsheet, Cpu, GitBranch, RefreshCw } from "lucide-vue-next";
import type { Component } from "vue";
import LoadingSpinner from "@/components/LoadingSpinner.vue";
import { DS_TASK_STEPS, TASK_STATUS_MAP } from "@/constants/pipeline";
import type { TaskDTO } from "@/services/document";

// --- Status helpers (private to this component) ---
// Capture `tasks` at script-scope; the template invokes these helpers without
// explicit task-list argument to keep call-sites short and byte-equivalent with
// the legacy inline helpers in DatabaseView.
const propsForHelpers = defineProps<{
  tasks: TaskDTO[];
  polling: boolean;
  loading: boolean;
}>();
...
const { tasks } = propsForHelpers;

function taskByType(type: string): TaskDTO | undefined {
  return tasks.find((t) => t.task_type === type);
}
