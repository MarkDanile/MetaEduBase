"""REQ-057 Task 1: adapter registry routing tests.
...
``direct_db``, ``mcp``) to the right concrete adapter, raise
``ValueError`` for anything else, and the MCP adapter must raise
:class:`CapabilityUnavailableError` from :meth:`MCPAdapter.query`
rather than returning ``[]`` (which would masquerade as "query
succeeded, no data").
...
The MCP query case constructs the adapter directly with no session
since MCP V1 never touches the session.
"""
...
async def test_factory_routes_mcp(db_session):
    """type=mcp -> MCPAdapter (V1 skeleton, query raises)."""
    adapter = await default_adapter_factory(
        db_session,
        {
            "type": "mcp",
            "server_url": "http://mcp.local",
            "tool_name": "query_bills",
        },
    )
...
async def test_mcp_query_raises_capability_unavailable():
    """MCP V1 must raise, not return [] (no masquerade as empty success)."""
    adapter = MCPAdapter(
