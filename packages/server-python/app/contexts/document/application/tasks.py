...
async def _create_task(session: AsyncSession, tenant_id: uuid.UUID, file_id: uuid.UUID, task_type: str) -> uuid.UUID:
    task_id = uuid.uuid4()
    now = datetime.now(UTC).replace(tzinfo=None)
    await session.execute(
        text(
...
def parse_document(file_id_str: str, tenant_id_str: str):
    ...
    async def _do(session: AsyncSession):
        ...
        try:
            ...
            await session.execute(
                text("UPDATE metaedu.files SET structured_data = :data::jsonb, status = 'processing', updated_at = :now WHERE id = :fid"),
                {"data": json.dumps({"full_text": parsed.full_text, "section_count": len(parsed.sections)}), "now": datetime.now(UTC).replace(tzinfo=None), "fid": file_id},
            )
            ...
            await session.execute(
                text("UPDATE metaedu.files SET status = 'failed', updated_at = :now WHERE id = :fid"),
...
def chunk_document(file_id_str: str, tenant_id_str: str):
    ...
    async def _do(session: AsyncSession):
        ...
        try:
            # Read parsed data
            result = await session.execute(
                text("SELECT structured_data FROM metaedu.files WHERE id = :fid AND tenant_id = :tid"),
                {"fid": file_id, "tid": tenant_id},
            )
            row = result.mappings().first()
            ...
            for chunk in chunks:
                ...
                await session.execute(
                    ...
                    ),
                    {
                        ...
                        "content": chunk.content, "stitle": chunk.section_title,
                        "spath": chunk.section_path, "cstart": chunk.char_start,
                        "cend": chunk.char_end, "now": now,
                    },
                )
                ...
                try:
                    async with httpx.AsyncClient(timeout=30.0) as client:
                        resp = await client.post(
                            ...
                            json={"model": settings.embedding_model, "input": [chunk["content"][:8192]]},
                        )
...
def index_tsvector(file_id_str: str, tenant_id_str: str):
    ...
    async def _do(session: AsyncSession):
        ...
        try:
            ...
            await session.execute(
                text("UPDATE metaedu.files SET status = 'processed', updated_at = :now WHERE id = :fid"),
                {"now": datetime.now(UTC).replace(tzinfo=None), "fid": file_id},
            )
...
def extract_template(file_id_str: str, tenant_id_str: str):
    ...
    async def _do(session: AsyncSession):
        ...
        try:
            ...
            existing_data = existing["structured_data"] if existing and existing["structured_data"] else {}
            existing_data["template"] = template_data

            await session.execute(
                text("UPDATE metaedu.files SET structured_data = :data::jsonb, updated_at = :now WHERE id = :fid"),
                {"data": json.dumps(existing_data), "now": datetime.now(UTC).replace(tzinfo=None), "fid": file_id},
            )
...
def extract_knowledge_graph(file_id_str: str, tenant_id_str: str):
    ...
    async def _do(session: AsyncSession):
        ...
        try:
            ...
            if api_key:
                import httpx
                chunks_text = "\n".join(f"[{c['section_title'] or '段落'}] {c['content'][:500]}" for c in chunks)
                prompt = (
                ...
                for entity in kg_data.get("entities", []):
                    ...
                    await session.execute(
                        ...
                        {"id": node_id, "tid": tenant_id, "title": name, "path": str(node_id)[:8], "fid": file_id, "now": datetime.now(UTC).replace(tzinfo=None)},
                    )
                    if vec_str:
                        await session.execute(
